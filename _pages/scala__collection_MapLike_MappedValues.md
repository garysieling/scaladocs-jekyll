...
### `def grouped(size: Int): Iterator[Map[A, C]]`                            ###
...
### `def slice(from: Int, until: Int): Map[A, C]`                            ###
...
### `def sliding(size: Int): Iterator[Map[A, C]]`                            ###
...
### `def sliding(size: Int, step: Int): Iterator[Map[A, C]]`                 ###
...
### `def takeWhile(p: ((A, C)) ⇒ Boolean): Map[A, C]`                        ###
...
### `def thisCollection: Iterable[(A, C)]`                                   ###
...
### `def toCollection(repr: Map[A, C]): Iterable[(A, C)]`                    ###
...
### `def view(from: Int, until: Int): IterableView[(A, C), Map[A, C]]`       ###
...
### `def view: IterableView[(A, C), Map[A, C]]`                              ###
...
### `def zipAll[B, A1 >: (A, C), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[Map[A, C], (A1, B), That]): That` ###
...
### `def zipWithIndex[A1 >: (A, C), That](implicit bf: CanBuildFrom[Map[A, C], (A1, Int), That]): That` ###
...
### `def zip[A1 >: (A, C), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[Map[A, C], (A1, B), That]): That` ###
...
### `def +[B1 >: C](kv1: (A, B1), kv2: (A, B1), kvs: (A, B1)*): Map[A, B1]`  ###
...
### `def ++[B1 >: C](xs: GenTraversableOnce[(A, B1)]): Map[A, B1]`           ###
...
### `def filterKeys(p: (A) ⇒ Boolean): Map[A, C]`                            ###
...
### `def filterNot(p: ((A, C)) ⇒ Boolean): Map[A, C]`                        ###
...
### `def newBuilder: Builder[(A, C), Map[A, C]]`                             ###
...
### `def updated[B1 >: C](key: A, value: B1): Map[A, B1]`                    ###
...
--------------------------------------------------------------------------------
        Instance Constructors From scala.collection.MapLike.MappedValues
--------------------------------------------------------------------------------
...
### `def ++:[B >: (A, C), That](that: Traversable[B])(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def ++:[B >: (A, C), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def ++[B >: (A, C), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def collect[B, That](pf: PartialFunction[(A, C), B])(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def dropWhile(p: ((A, C)) ⇒ Boolean): Map[A, C]`                        ###
...
### `def filter(p: ((A, C)) ⇒ Boolean): Map[A, C]`                           ###
...
### `def flatMap[B, That](f: ((A, C)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def groupBy[K](f: ((A, C)) ⇒ K): immutable.Map[K, Map[A, C]]`           ###
...
### `def map[B, That](f: ((A, C)) ⇒ B)(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def partition(p: ((A, C)) ⇒ Boolean): (Map[A, C], Map[A, C])`           ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, C)) ⇒ B)(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, C), B) ⇒ B)(implicit bf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def scan[B >: (A, C), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[Map[A, C], B, That]): That` ###
...
### `def span(p: ((A, C)) ⇒ Boolean): (Map[A, C], Map[A, C])`                ###
...
### `def splitAt(n: Int): (Map[A, C], Map[A, C])`                            ###
...
### `def toTraversable: Traversable[(A, C)]`                                 ###
...
### `def withFilter(p: ((A, C)) ⇒ Boolean): FilterMonadic[(A, C), Map[A, C]]` ###
...
### `def flatten[B](implicit asTraversable: ((A, C)) ⇒ GenTraversableOnce[B]): Iterable[B]` ###
...
### `def transpose[B](implicit asTraversable: ((A, C)) ⇒ GenTraversableOnce[B]): Iterable[Iterable[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: ((A, C)) ⇒ (A1, A2, A3)): (Iterable[A1], Iterable[A2], Iterable[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: ((A, C)) ⇒ (A1, A2)): (Iterable[A1], Iterable[A2])` ###
...
### `def -(elem1: A, elem2: A, elems: A*): Map[A, C]`                        ###
...
### `def --(xs: GenTraversableOnce[A]): Map[A, C]`                           ###
...
* Implicit information
  * This member is added by an implicit conversion from MappedValues [C] to
    CollectionsHaveToParArray [MappedValues [C], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (MappedValues [C]) ⇒
