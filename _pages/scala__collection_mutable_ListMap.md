...
Copies the elements of this mutable list map to an array. Fills the given array
 `xs` with at most `len` elements of this mutable list map, starting at position
 `start` . Copying will stop once either the end of the current mutable list map
...
Applies a function `f` to all elements of this mutable list map.
...
order as this mutable list map.
...
Returns a mutable list map formed from this mutable list map and another
...
* thisElem
  * the element to be used to fill up the result if this mutable list map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable list map.
* returns
  * a new mutable list map containing pairs consisting of corresponding elements
    of this mutable list map and `that` . The length of the returned collection
    is the maximum of the lengths of this mutable list map and `that` . If this
    mutable list map is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this mutable list map, `thatElem`
...
Zips this mutable list map with its indices.

* returns
  * A new mutable list map containing pairs consisting of all elements of this
    mutable list map paired with their index. Indices start at `0` .
...
* returns
  * a new mutable list map containing pairs consisting of corresponding elements
    of this mutable list map and `that` . The length of the returned collection
    is the minimum of the lengths of this mutable list map and `that` .
...
* that
  * the traversable to append.
* returns
  * a new mutable list map which contains all elements of this mutable list map
...
Returns a new mutable list map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the mutable list map is the most specific superclass encompassing the element
...
mutable list map on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the mutable list map.
* returns
  * a new mutable list map resulting from applying the given partial function
...
* returns
  * a new mutable list map resulting from applying the given collection-valued
    function `f` to each element of this mutable list map and concatenating the
    results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, ListMap[A, B]]`       ###
...
* f
  * the function to apply to each element.
* returns
  * a new mutable list map resulting from applying the given function `f` to
    each element of this mutable list map and collecting the results.
...
Copies the elements of this mutable list map to an array. Fills the given array
 `xs` with values of this mutable list map. Copying will stop once either the
end of the current mutable list map is reached, or the end of the target array
is reached.
...
Copies the elements of this mutable list map to an array. Fills the given array
 `xs` with values of this mutable list map, beginning at index `start` . Copying
will stop once either the end of the current mutable list map is reached, or the
...
* returns
  * the first element of this mutable list map with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable list map with the smallest value measured
    by function f.
...
Converts this mutable list map to a map. This method is unavailable unless the
...
Converts this mutable list map of traversable collections into a mutable list
...
* returns
  * a new mutable list map resulting from concatenating all element mutable list
    maps.
...
### `def ++=(xs: TraversableOnce[(A, B)]): ListMap.this.type`                ###
...
### `def +=(elem1: (A, B), elem2: (A, B), elems: (A, B)*): ListMap.this.type` ###
...
### `def --=(xs: TraversableOnce[A]): ListMap.this.type`                     ###
...
### `def -=(elem1: A, elem2: A, elems: A*): ListMap.this.type`               ###
...
### `def mapResult[NewTo](f: (ListMap[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.ListMap
--------------------------------------------------------------------------------
...
* Definition Classes
  * ListMap → MapLike → Builder → Growable
* Annotations
  * @ deprecatedOverriding (message =..., since = "2.11.0")
...
### `def retain(p: (A, B) ⇒ Boolean): ListMap.this.type`                     ###
...
### `def transform(f: (A, B) ⇒ B): ListMap.this.type`                        ###
