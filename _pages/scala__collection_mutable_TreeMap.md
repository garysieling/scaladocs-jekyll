...
Copies the elements of this mutable tree map to an array. Fills the given array
 `xs` with at most `len` elements of this mutable tree map, starting at position
 `start` . Copying will stop once either the end of the current mutable tree map
...
order as this mutable tree map.
...
Returns a mutable tree map formed from this mutable tree map and another
...
* thisElem
  * the element to be used to fill up the result if this mutable tree map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable tree map.
* returns
  * a new mutable tree map containing pairs consisting of corresponding elements
    of this mutable tree map and `that` . The length of the returned collection
    is the maximum of the lengths of this mutable tree map and `that` . If this
    mutable tree map is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this mutable tree map, `thatElem`
...
Zips this mutable tree map with its indices.

Note: might return different results for different runs, unless the underlying
collection type is ordered.

* returns
  * A new mutable tree map containing pairs consisting of all elements of this
    mutable tree map paired with their index. Indices start at `0` .
...
* returns
  * a new mutable tree map containing pairs consisting of corresponding elements
    of this mutable tree map and `that` . The length of the returned collection
    is the minimum of the lengths of this mutable tree map and `that` .
...
--------------------------------------------------------------------------------
               Value Members From scala.collection.SortedMapLike
--------------------------------------------------------------------------------
...
* that
  * the traversable to append.
* returns
  * a new mutable tree map which contains all elements of this mutable tree map
...
Returns a new mutable tree map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the mutable tree map is the most specific superclass encompassing the element
...
mutable tree map on which the function is defined.
...
* returns
  * a new mutable tree map resulting from applying the given collection-valued
    function `f` to each element of this mutable tree map and concatenating the
    results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, TreeMap[A, B]]`       ###
...
* f
  * the function to apply to each element.
* returns
  * a new mutable tree map resulting from applying the given function `f` to
    each element of this mutable tree map and collecting the results.
...
Copies the elements of this mutable tree map to an array. Fills the given array
 `xs` with values of this mutable tree map. Copying will stop once either the
end of the current mutable tree map is reached, or the end of the target array
is reached.
...
Copies the elements of this mutable tree map to an array. Fills the given array
 `xs` with values of this mutable tree map, beginning at index `start` . Copying
will stop once either the end of the current mutable tree map is reached, or the
...
* returns
  * the first element of this mutable tree map with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable tree map with the smallest value measured
    by function f.
...
Converts this mutable tree map to a map. This method is unavailable unless the
...
Converts this mutable tree map of traversable collections into a mutable tree
...
* returns
  * a new mutable tree map resulting from concatenating all element mutable tree
    maps.
...
### `def ++=(xs: TraversableOnce[(A, B)]): TreeMap.this.type`                ###
...
### `def +=(elem1: (A, B), elem2: (A, B), elems: (A, B)*): TreeMap.this.type` ###
...
### `def --=(xs: TraversableOnce[A]): TreeMap.this.type`                     ###
...
### `def -=(elem1: A, elem2: A, elems: A*): TreeMap.this.type`               ###
...
### `def mapResult[NewTo](f: (TreeMap[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
### `def retain(p: (A, B) ⇒ Boolean): TreeMap.this.type`                     ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.TreeMap
--------------------------------------------------------------------------------


### `new TreeMap()(implicit ord: Ordering[A])`                               ###
...
* Definition Classes
  * TreeMap → MapLike → Builder → Growable

(defined at scala.collection.mutable.TreeMap)


### `def -=(key: A): TreeMap.this.type`                                      ###
...
Applies a function `f` to all elements of this mutable tree map.
...
* Attributes
  * protected[this]
* Definition Classes
  * TreeMap → SortedMap → SortedMap → MapLike → MapLike →
    GenericTraversableTemplate → TraversableLike → HasNewBuilder
...
Creates a ranged projection of this map. Any mutations in the ranged projection
will update the original map and vice versa.

Only entries with keys between this projection's key range will ever appear as
elements of this map, independently of whether the entries are added through the
original map or through this view. That means that if one inserts a key-value in
a view whose key is outside the view's bounds, calls to `get` or `contains` will
_not_ consider the newly added entry. Mutations are always reflected in the
original map, though.

* from
  * the lower bound (inclusive) of this projection wrapped in a `Some` , or
     `None` if there is no lower bound.
* until
  * the upper bound (exclusive) of this projection wrapped in a `Some` , or
...
### `def transform(f: (A, B) ⇒ B): TreeMap.this.type`                        ###
