...
### `def newAppended[B >: A](that: GenTraversable[B]): Reversed.Transformed[B]` ###
...
### `def newFlatMapped[B](f: (A) ⇒ GenTraversableOnce[B]): Reversed.Transformed[B]` ###
...
### `def newForced[B](xs: ⇒ GenSeq[B]): Reversed.Transformed[B]`             ###
...
### `def newMapped[B](f: (A) ⇒ B): Reversed.Transformed[B]`                  ###
...
### `def newPatched[B >: A](_from: Int, _patch: GenSeq[B], _replaced: Int): Reversed.Transformed[B]` ###
...
### `def newPrepended[B >: A](that: GenTraversable[B]): Reversed.Transformed[B]` ###
...
### `def newZippedAll[A1 >: A, B](that: GenIterable[B], _thisElem: A1, _thatElem: B): Reversed.Transformed[(A1, B)]` ###
...
### `def newZipped[B](that: GenIterable[B]): Reversed.Transformed[(A, B)]`   ###
...
--------------------------------------------------------------------------------
      Value Members From scala.collection.mutable.IndexedSeqView.Reversed
--------------------------------------------------------------------------------
...
* Definition Classes
  * Reversed → Transformed → IndexedSeqView → IndexedSeqLike → SeqLike
* Exceptions thrown
  * IndexOutOfBoundsException if the index is not valid.

(defined at scala.collection.mutable.IndexedSeqView.Reversed)
...
### `def transform(f: (A) ⇒ A): Reversed.this.type`                          ###
