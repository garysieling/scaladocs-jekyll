...
Copies the elements of this mutable sorted set to an array. Fills the given
array `xs` with at most `len` elements of this mutable sorted set, starting at
position `start` . Copying will stop once either the end of the current mutable
sorted set is reached, or the end of the target array is reached, or `len`
...
Applies a function `f` to all elements of this mutable sorted set.
...
order as this mutable sorted set.
...
### `def toCollection(repr: SortedSet[A]): collection.Iterable[A]`           ###
...
### `def zipAll[B, A1 >: A, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[SortedSet[A], (A1, B), That]): That` ###

[use case]

Returns a mutable sorted set formed from this mutable sorted set and another
...
* thisElem
  * the element to be used to fill up the result if this mutable sorted set is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable sorted set.
* returns
  * a new mutable sorted set containing pairs consisting of corresponding
    elements of this mutable sorted set and `that` . The length of the returned
    collection is the maximum of the lengths of this mutable sorted set and
     `that` . If this mutable sorted set is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this mutable
    sorted set, `thatElem` values are used to pad the result.
...
### `def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[SortedSet[A], (A1, Int), That]): That` ###

[use case]

Zips this mutable sorted set with its indices.

Note: might return different results for different runs, unless the underlying
collection type is ordered.

* returns
  * A new mutable sorted set containing pairs consisting of all elements of this
    mutable sorted set paired with their index. Indices start at `0` .
...
### `def zip[A1 >: A, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[SortedSet[A], (A1, B), That]): That` ###
...
* returns
  * a new mutable sorted set containing pairs consisting of corresponding
    elements of this mutable sorted set and `that` . The length of the returned
    collection is the minimum of the lengths of this mutable sorted set and
     `that` .
...
### `def map[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[SortedSet[A], B, That]): That` ###
...
* returns
  * a new mutable sorted set resulting from applying the given function `f` to
    each element of this mutable sorted set and collecting the results.
...
--------------------------------------------------------------------------------
           Abstract Value Members From scala.collection.SortedSetLike
--------------------------------------------------------------------------------
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[SortedSet[A], B, That]): That` ###
...
### `def ++:[B >: A, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[SortedSet[A], B, That]): That` ###
...
* that
  * the traversable to append.
* returns
  * a new mutable sorted set which contains all elements of this mutable sorted
...
Returns a new mutable sorted set containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the mutable sorted set is the most specific superclass encompassing the
...
### `def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[SortedSet[A], B, That]): That` ###
...
* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the mutable sorted set.
* returns
  * a new mutable sorted set resulting from applying the given partial function
...
### `def flatMap[B, That](f: (A) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[SortedSet[A], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new mutable sorted set resulting from applying the given collection-valued
    function `f` to each element of this mutable sorted set and concatenating
    the results.
...
Copies the elements of this mutable sorted set to an array. Fills the given
array `xs` with values of this mutable sorted set. Copying will stop once either
the end of the current mutable sorted set is reached, or the end of the target
array is reached.
...
Copies the elements of this mutable sorted set to an array. Fills the given
array `xs` with values of this mutable sorted set, beginning at index `start` .
Copying will stop once either the end of the current mutable sorted set is
...
* returns
  * the first element of this mutable sorted set with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable sorted set with the smallest value
...
Converts this mutable sorted set to a map. This method is unavailable unless the
...
* returns
  * a map of type `immutable.Map[T, U]` containing all key/value pairs of type
     `(T, U)` of this mutable sorted set.
...
Converts this mutable sorted set of traversable collections into a mutable
...
* returns
  * a new mutable sorted set resulting from concatenating all element mutable
    sorted set s.
...
### `def ++=(xs: TraversableOnce[A]): AbstractSortedSet.this.type`           ###
...
### `def +=(elem1: A, elem2: A, elems: A*): AbstractSortedSet.this.type`     ###
...
--------------------------------------------------------------------------------
     Instance Constructors From scala.collection.mutable.AbstractSortedSet
--------------------------------------------------------------------------------


### `new AbstractSortedSet()`                                                ###

(defined at scala.collection.mutable.AbstractSortedSet)
...
### `def mapResult[NewTo](f: (SortedSet[A]) ⇒ NewTo): Builder[A, NewTo]`     ###
...
The factory companion object that builds instances of class `mutable.Set` . (or
its `Iterable` superclass where class `mutable.Set` is not a `Seq` .)
...
### `abstract def +=(elem: A): AbstractSortedSet.this.type`                  ###
...
--------------------------------------------------------------------------------
         Concrete Value Members From scala.collection.mutable.SortedSet
--------------------------------------------------------------------------------
...
* Definition Classes
  * SortedSet → SortedSet → SetLike → GenericSetTemplate
...
* Implicit information
  * This member is added by an implicit conversion from AbstractSortedSet [A] to
    CollectionsHaveToParArray [AbstractSortedSet [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (AbstractSortedSet [A]) ⇒
