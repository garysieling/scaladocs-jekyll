...
Finds index of first occurrence of some value in this mutable list.
...
* returns
  * the index of the first element of this mutable list that is equal (as
...
Finds index of first occurrence of some value in this mutable list after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this mutable list that is equal
...
Finds index of last occurrence of some value in this mutable list.
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this mutable list that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this mutable list before or at a
given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this mutable list that is equal
    (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this mutable list to an array. Fills the given array
 `xs` with at most `len` elements of this mutable list, starting at position
 `start` . Copying will stop once either the end of the current mutable list is
...
### `def grouped(size: Int): Iterator[MutableList[A]]`                       ###
...
### `def sliding(size: Int): Iterator[MutableList[A]]`                       ###
...
### `def sliding(size: Int, step: Int): Iterator[MutableList[A]]`            ###
...
### `def takeRight(n: Int): MutableList[A]`                                  ###
...
### `def zipAll[B, A1 >: A, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[MutableList[A], (A1, B), That]): That` ###

[use case]

Returns a mutable list formed from this mutable list and another iterable
...
* thisElem
  * the element to be used to fill up the result if this mutable list is shorter
    than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable list.
* returns
  * a new mutable list containing pairs consisting of corresponding elements of
    this mutable list and `that` . The length of the returned collection is the
    maximum of the lengths of this mutable list and `that` . If this mutable
    list is shorter than `that` , `thisElem` values are used to pad the result.
    If `that` is shorter than this mutable list, `thatElem` values are used to
    pad the result.
...
### `def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[MutableList[A], (A1, Int), That]): That` ###

[use case]

Zips this mutable list with its indices.

* returns
  * A new mutable list containing pairs consisting of all elements of this
    mutable list paired with their index. Indices start at `0` .
...
### `def zip[A1 >: A, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[MutableList[A], (A1, B), That]): That` ###
...
* returns
  * a new mutable list containing pairs consisting of corresponding elements of
    this mutable list and `that` . The length of the returned collection is the
    minimum of the lengths of this mutable list and `that` .
...
### `def toCollection(repr: MutableList[A]): collection.LinearSeq[A]`        ###
...
### `def dropRight(n: Int): MutableList[A]`                                  ###
...
Applies a function `f` to all elements of this mutable list.
...
### `def slice(from: Int, until: Int): MutableList[A]`                       ###
...
### `def span(p: (A) ⇒ Boolean): (MutableList[A], MutableList[A])`           ###
...
### `def takeWhile(p: (A) ⇒ Boolean): MutableList[A]`                        ###
...
### `def +:[B >: A, That](elem: B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

A copy of the mutable list with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original mutable list is not modified, so you will want to capture the
result.
...
* returns
  * a new mutable list consisting of `elem` followed by all elements of this
    mutable list.
...
A copy of this mutable list with an element appended.
...
* returns
  * a new mutable list consisting of all elements of this mutable list followed
    by `elem` .
...
### `def combinations(n: Int): Iterator[MutableList[A]]`                     ###
...
### `def diff[B >: A](that: GenSeq[B]): MutableList[A]`                      ###

[use case]

Computes the multiset difference between this mutable list and another sequence.
...
* returns
  * a new mutable list which contains all elements of this mutable list except
...
### `def intersect[B >: A](that: GenSeq[B]): MutableList[A]`                 ###

[use case]

Computes the multiset intersection between this mutable list and another
sequence.
...
* returns
  * a new mutable list which contains all elements of this mutable list which
...
### `def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

A copy of this mutable list with an element value appended until a given target
length is reached.
...
* returns
  * a new mutable list consisting of all elements of this mutable list followed
    by the minimal number of occurrences of `elem` so that the resulting mutable
...
### `def patch[B >: A, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

Produces a new mutable list where a slice of elements in this mutable list is
replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original mutable list
* returns
  * a new mutable list consisting of all elements of this mutable list except
...
### `def permutations: Iterator[MutableList[A]]`                             ###
...
### `def reverseMap[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
* returns
  * a new mutable list resulting from applying the given function `f` to each
    element of this mutable list and collecting the results in reversed order.
...
### `def sortBy[B](f: (A) ⇒ B)(implicit ord: math.Ordering[B]): MutableList[A]` ###
...
### `def sortWith(lt: (A, A) ⇒ Boolean): MutableList[A]`                     ###
...
### `def sorted[B >: A](implicit ord: math.Ordering[B]): MutableList[A]`     ###
...
### `def union[B >: A, That](that: GenSeq[B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this mutable list and
...
* that
  * the sequence to add.
* returns
  * a new mutable list which contains all elements of this mutable list followed
...
### `def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

A copy of this mutable list with one single replaced element.
...
* returns
  * a copy of this mutable list with the element at position `index` replaced by
     `elem` .
...
### `def view(from: Int, until: Int): SeqView[A, MutableList[A]]`            ###
...
### `def view: SeqView[A, MutableList[A]]`                                   ###
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
### `def ++:[B >: A, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
### `def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

Returns a new mutable list containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
mutable list is the most specific superclass encompassing the element types of
the two operands.
...
### `def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
mutable list on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the mutable list.
* returns
  * a new mutable list resulting from applying the given partial function `pf`
...
### `def dropWhile(p: (A) ⇒ Boolean): MutableList[A]`                        ###
...
### `def filter(p: (A) ⇒ Boolean): MutableList[A]`                           ###
...
### `def filterNot(p: (A) ⇒ Boolean): MutableList[A]`                        ###
...
### `def flatMap[B, That](f: (A) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this mutable
list and using the elements of the resulting collections.
...
list. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new mutable list resulting from applying the given collection-valued
    function `f` to each element of this mutable list and concatenating the
    results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, MutableList[A]]`           ###
...
### `def inits: Iterator[MutableList[A]]`                                    ###
...
### `def map[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
* returns
  * a new mutable list resulting from applying the given function `f` to each
    element of this mutable list and collecting the results.
...
### `def partition(p: (A) ⇒ Boolean): (MutableList[A], MutableList[A])`      ###
...
### `def scanLeft[B, That](z: B)(op: (B, A) ⇒ B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (A, B) ⇒ B)(implicit bf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
### `def scan[B >: A, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[MutableList[A], B, That]): That` ###
...
### `def splitAt(n: Int): (MutableList[A], MutableList[A])`                  ###
...
### `def tails: Iterator[MutableList[A]]`                                    ###
...
### `def withFilter(p: (A) ⇒ Boolean): FilterMonadic[A, MutableList[A]]`     ###
...
Copies the elements of this mutable list to an array. Fills the given array
 `xs` with values of this mutable list. Copying will stop once either the end of
the current mutable list is reached, or the end of the target array is reached.
...
Copies the elements of this mutable list to an array. Fills the given array
 `xs` with values of this mutable list, beginning at index `start` . Copying
will stop once either the end of the current mutable list is reached, or the end
...
* returns
  * the first element of this mutable list with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable list with the smallest value measured by
    function f.
...
Converts this mutable list to a map. This method is unavailable unless the
...
### `def flatten[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): MutableList[B]` ###

[use case]

Converts this mutable list of traversable collections into a mutable list formed
...
* returns
  * a new mutable list resulting from concatenating all element mutable lists.
...
### `def genericBuilder[B]: Builder[B, MutableList[B]]`                      ###
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): MutableList[MutableList[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (A) ⇒ (A1, A2, A3)): (MutableList[A1], MutableList[A2], MutableList[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (A) ⇒ (A1, A2)): (MutableList[A1], MutableList[A2])` ###
...
### `def ++=(xs: TraversableOnce[A]): MutableList.this.type`                 ###
...
### `def +=(elem1: A, elem2: A, elems: A*): MutableList.this.type`           ###
...
### `def mapResult[NewTo](f: (MutableList[A]) ⇒ NewTo): Builder[A, NewTo]`   ###
...
--------------------------------------------------------------------------------
             Value Members From scala.collection.mutable.LinearSeq
--------------------------------------------------------------------------------
...
(defined at scala.collection.mutable.LinearSeq)


--------------------------------------------------------------------------------
        Instance Constructors From scala.collection.mutable.MutableList
--------------------------------------------------------------------------------
...
* returns
  * the element of this mutable list at index `idx` , where `0` indicates the
    first element.
...
### `def companion: GenericCompanion[MutableList]`                           ###

The factory companion object that builds instances of class
 `mutable.MutableList` . (or its `Iterable` superclass where class
 `mutable.MutableList` is not a `Seq` .)

* Definition Classes
  * MutableList → LinearSeq → LinearSeq → Seq → Iterable → Traversable → Seq →
    GenSeq → Iterable → GenIterable → Traversable → GenTraversable →
    GenericTraversableTemplate

(defined at scala.collection.mutable.MutableList)


### `var first0: LinkedList[A]`                                              ###
...
### `def newBuilder: Builder[A, MutableList[A]]`                             ###

The builder that builds instances of type `mutable.MutableList` [A]
...
* returns
  * a mutable list consisting of all elements of this mutable list except the
    first one.
...
### `final def tailImpl(tl: MutableList[A]): Unit`                           ###
...
### `def transform(f: (A) ⇒ A): MutableList.this.type`                       ###
...
* Implicit information
  * This member is added by an implicit conversion from MutableList [A] to
    CollectionsHaveToParArray [MutableList [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (MutableList [A]) ⇒
