...
* returns
  * the index of the first element of this vector that is equal (as determined
...
Finds index of first occurrence of some value in this vector after or at some
start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this vector that is equal (as
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this vector that is equal (as determined by
...
Finds index of last occurrence of some value in this vector before or at a given
end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this vector that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this vector to an array. Fills the given array `xs` with
at most `len` elements of this vector, starting at position `start` . Copying
will stop once either the end of the current vector is reached, or the end of
...
Returns a vector formed from this vector and another iterable collection by
...
* thisElem
  * the element to be used to fill up the result if this vector is shorter than
     `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    vector.
* returns
  * a new vector containing pairs consisting of corresponding elements of this
    vector and `that` . The length of the returned collection is the maximum of
    the lengths of this vector and `that` . If this vector is shorter than
     `that` , `thisElem` values are used to pad the result. If `that` is shorter
    than this vector, `thatElem` values are used to pad the result.
...
* returns
  * A new vector containing pairs consisting of all elements of this vector
...
* returns
  * a new vector containing pairs consisting of corresponding elements of this
    vector and `that` . The length of the returned collection is the minimum of
    the lengths of this vector and `that` .
...
* returns
  * a new vector which contains all elements of this vector except some of
...
* returns
  * a new vector which contains all elements of this vector which also appear in
...
A copy of this vector with an element value appended until a given target length
is reached.
...
* elem
  * the padding value
* returns
  * a new vector consisting of all elements of this vector followed by the
    minimal number of occurrences of `elem` so that the resulting vector has a
...
Produces a new vector where a slice of elements in this vector is replaced by
another sequence.
...
* returns
  * a new vector consisting of all elements of this vector except that
...
* returns
  * a new vector resulting from applying the given function `f` to each element
    of this vector and collecting the results in reversed order.
...
Produces a new sequence which contains all elements of this vector and also all
...
* that
  * the sequence to add.
* returns
  * a new vector which contains all elements of this vector followed by all
    elements of `that` .
...
vector on which the function is defined.
...
* returns
  * a new vector resulting from applying the given partial function `pf` to each
...
The type of the resulting collection is guided by the static type of vector.
...
* f
  * the function to apply to each element.
* returns
  * a new vector resulting from applying the given collection-valued function
     `f` to each element of this vector and concatenating the results.
...
* returns
  * a new vector resulting from applying the given function `f` to each element
    of this vector and collecting the results.
...
Copies the elements of this vector to an array. Fills the given array `xs` with
values of this vector. Copying will stop once either the end of the current
vector is reached, or the end of the target array is reached.
...
Copies the elements of this vector to an array. Fills the given array `xs` with
values of this vector, beginning at index `start` . Copying will stop once
either the end of the current vector is reached, or the end of the target array
is reached.
...
* returns
  * the first element of this vector with the largest value measured by function
    f.
...
* f
  * The measuring function.
* returns
  * the first element of this vector with the smallest value measured by
    function f.
...
Converts this vector to a map. This method is unavailable unless the elements
...
Converts this vector of traversable collections into a vector formed by the
elements of these traversable collections.

The resulting collection's type will be guided by the static type of vector. For
example:
...
Returns a new vector containing the elements from the left hand operand followed
by the elements from the right hand operand. The element type of the vector is
...
Also, the original vector is not modified, so you will want to capture the
result.
...
* returns
  * the element of this vector at index `idx` , where `0` indicates the first
    element.
...
The factory companion object that builds instances of class `Vector` . (or its
 `Iterable` superclass where class `Vector` is not a `Seq` .)

* Definition Classes
  * Vector → IndexedSeq → IndexedSeq → Seq → Iterable → Traversable → Seq →
...
* returns
  * a vector consisting of all elements of this vector except the first `n`
    ones, or else the empty vector, if this vector has less than `n` elements.
...
* returns
  * a vector consisting of all elements of this vector except the last `n` ones,
    or else the empty vector, if this vector has less than `n` elements.
...
* returns
  * a vector containing the elements greater than or equal to index `from`
    extending up to (but not including) index `until` of this vector.
...
Splits this vector into two at a given position. Note: `c splitAt n` is
equivalent to (but possibly more efficient than) `(c take n, c drop n)` .

* n
  * the position at which to split.
* returns
  * a pair of vectors consisting of the first `n` elements of this vector, and
    the other elements.
...
* returns
  * a vector consisting only of the first `n` elements of this vector, or else
    the whole vector, if it has less than `n` elements.
...
* n
  * the number of elements to take
* returns
  * a vector consisting only of the last `n` elements of this vector, or else
    the whole vector, if it has less than `n` elements.
...
* returns
  * a copy of this vector with the element at position `index` replaced by
     `elem` .
...
* Implicit information
  * This member is added by an implicit conversion from Vector [A] to
...
* Implicit information
  * This member is added by an implicit conversion from Vector [A] to
    CollectionsHaveToParArray [Vector [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (Vector [A]) ⇒
