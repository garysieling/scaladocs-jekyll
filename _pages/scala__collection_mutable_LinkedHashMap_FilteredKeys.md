...
### `def +[B1 >: B](kv: (A, B1)): collection.Map[A, B1]`                     ###
...
### `def drop(n: Int): collection.Map[A, B]`                                 ###
...
### `def dropRight(n: Int): collection.Map[A, B]`                            ###
...
### `def grouped(size: Int): Iterator[collection.Map[A, B]]`                 ###
...
### `def slice(from: Int, until: Int): collection.Map[A, B]`                 ###
...
### `def sliding(size: Int): Iterator[collection.Map[A, B]]`                 ###
...
### `def sliding(size: Int, step: Int): Iterator[collection.Map[A, B]]`      ###
...
### `def take(n: Int): collection.Map[A, B]`                                 ###
...
### `def takeRight(n: Int): collection.Map[A, B]`                            ###
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): collection.Map[A, B]`             ###
...
### `def toCollection(repr: collection.Map[A, B]): collection.Iterable[(A, B)]` ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), collection.Map[A, B]]` ###
...
### `def zipAll[B, A1 >: (A, B), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[collection.Map[A, B], (A1, B), That]): That` ###
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[collection.Map[A, B], (A1, Int), That]): That` ###
...
### `def zip[A1 >: (A, B), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[collection.Map[A, B], (A1, B), That]): That` ###
...
### `def +[B1 >: B](kv1: (A, B1), kv2: (A, B1), kvs: (A, B1)*): collection.Map[A, B1]` ###
...
### `def ++[B1 >: B](xs: GenTraversableOnce[(A, B1)]): collection.Map[A, B1]` ###
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): collection.Map[A, B]`             ###
...
### `def newBuilder: Builder[(A, B), collection.Map[A, B]]`                  ###
...
--------------------------------------------------------------------------------
            Value Members From scala.collection.MapLike.FilteredKeys
--------------------------------------------------------------------------------
...
* Definition Classes
  * FilteredKeys → MapLike → GenMapLike

(defined at scala.collection.MapLike.FilteredKeys)
...
* Definition Classes
  * FilteredKeys → IterableLike → GenericTraversableTemplate → TraversableLike →
...
### `def ++:[B >: (A, B), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def ++:[B >: (A, B), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def ++[B >: (A, B), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def collect[B, That](pf: PartialFunction[(A, B), B])(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def dropWhile(p: ((A, B)) ⇒ Boolean): collection.Map[A, B]`             ###
...
### `def filter(p: ((A, B)) ⇒ Boolean): collection.Map[A, B]`                ###
...
### `def flatMap[B, That](f: ((A, B)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, collection.Map[A, B]]` ###
...
### `def inits: Iterator[collection.Map[A, B]]`                              ###
...
### `def map[B, That](f: ((A, B)) ⇒ B)(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def partition(p: ((A, B)) ⇒ Boolean): (collection.Map[A, B], collection.Map[A, B])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[collection.Map[A, B], B, That]): That` ###
...
### `def span(p: ((A, B)) ⇒ Boolean): (collection.Map[A, B], collection.Map[A, B])` ###
...
### `def splitAt(n: Int): (collection.Map[A, B], collection.Map[A, B])`      ###
...
### `def tails: Iterator[collection.Map[A, B]]`                              ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), collection.Map[A, B]]` ###
...
### `def flatten[B](implicit asTraversable: ((A, B)) ⇒ GenTraversableOnce[B]): collection.Iterable[B]` ###
...
### `def transpose[B](implicit asTraversable: ((A, B)) ⇒ GenTraversableOnce[B]): collection.Iterable[collection.Iterable[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: ((A, B)) ⇒ (A1, A2, A3)): (collection.Iterable[A1], collection.Iterable[A2], collection.Iterable[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: ((A, B)) ⇒ (A1, A2)): (collection.Iterable[A1], collection.Iterable[A2])` ###
...
### `def -(elem1: A, elem2: A, elems: A*): collection.Map[A, B]`             ###
...
### `def --(xs: GenTraversableOnce[A]): collection.Map[A, B]`                ###
...
--------------------------------------------------------------------------------
 Instance Constructors From scala.collection.mutable.LinkedHashMap.FilteredKeys
--------------------------------------------------------------------------------


### `new FilteredKeys(p: (A) ⇒ Boolean)`                                     ###

(defined at scala.collection.mutable.LinkedHashMap.FilteredKeys)


--------------------------------------------------------------------------------
     Value Members From scala.collection.mutable.LinkedHashMap.FilteredKeys
--------------------------------------------------------------------------------
...
* Implicit information
  * This member is added by an implicit conversion from FilteredKeys to
    CollectionsHaveToParArray [LinkedHashMap.FilteredKeys, T] performed by
    ...
    LinkedHashMap.FilteredKeys) ⇒ GenTraversableOnce [T] is in scope.
* Definition Classes
