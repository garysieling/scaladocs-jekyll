...
* returns
  * the index of the first element of this array stack that is equal (as
...
Finds index of first occurrence of some value in this array stack after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this array stack that is equal
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this array stack that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this array stack before or at a
given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this array stack that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this array stack to an array. Fills the given array `xs`
with at most `len` elements of this array stack, starting at position `start` .
Copying will stop once either the end of the current array stack is reached, or
...
### `def zipWithIndex[A1 >: T, That](implicit bf: CanBuildFrom[ArrayStack[T], (A1, Int), That]): That` ###
...
* returns
  * A new array stack containing pairs consisting of all elements of this array
...
### `def zip[A1 >: T, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[ArrayStack[T], (A1, B), That]): That` ###

[use case]

Returns a array stack formed from this array stack and another iterable
...
* returns
  * a new array stack containing pairs consisting of corresponding elements of
    this array stack and `that` . The length of the returned collection is the
    minimum of the lengths of this array stack and `that` .
...
### `def zipAll[B, A1 >: T, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[ArrayStack[T], (A1, B), That]): That` ###
...
* that
  * The iterable providing the second half of each result pair
* thisElem
  * the element to be used to fill up the result if this array stack is shorter
    than `that` .
* thatElem
...
* returns
  * a new array stack containing pairs consisting of corresponding elements of
    this array stack and `that` . The length of the returned collection is the
    maximum of the lengths of this array stack and `that` . If this array stack
    is shorter than `that` , `thisElem` values are used to pad the result. If
     `that` is shorter than this array stack, `thatElem` values are used to pad
    the result.
...
### `def +:[B >: T, That](elem: B)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###
...
Also, the original array stack is not modified, so you will want to capture the
result.
...
* returns
  * a new array stack consisting of `elem` followed by all elements of this
    array stack.
...
* returns
  * a new array stack consisting of all elements of this array stack followed by
     `elem` .
...
### `def diff[B >: T](that: GenSeq[B]): ArrayStack[T]`                       ###
...
* returns
  * a new array stack which contains all elements of this array stack except
...
### `def intersect[B >: T](that: GenSeq[B]): ArrayStack[T]`                  ###

[use case]

Computes the multiset intersection between this array stack and another
sequence.

* that
  * the sequence of elements to intersect with.
* returns
  * a new array stack which contains all elements of this array stack which also
...
### `def padTo[B >: T, That](len: Int, elem: B)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

A copy of this array stack with an element value appended until a given target
length is reached.
...
* returns
  * a new array stack consisting of all elements of this array stack followed by
    the minimal number of occurrences of `elem` so that the resulting array
    stack has a length of at least `len` .
...
### `def patch[B >: T, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

Produces a new array stack where a slice of elements in this array stack is
...
* returns
  * a new array stack consisting of all elements of this array stack except that
...
### `def reverseMap[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this array
stack and collecting the results in reversed order.
...
* returns
  * a new array stack resulting from applying the given function `f` to each
    element of this array stack and collecting the results in reversed order.
...
### `def union[B >: T, That](that: GenSeq[B])(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this array stack and also
...
* that
  * the sequence to add.
* returns
  * a new array stack which contains all elements of this array stack followed
...
### `def updated[B >: T, That](index: Int, elem: B)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###
...
* returns
  * a copy of this array stack with the element at position `index` replaced by
     `elem` .
...
### `def ++:[B >: T, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###
...
Returns a new array stack containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
array stack is the most specific superclass encompassing the element types of
the two operands.
...
### `def collect[B, That](pf: PartialFunction[T, B])(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
array stack on which the function is defined.
...
* returns
  * a new array stack resulting from applying the given partial function `pf` to
...
### `def flatMap[B, That](f: (T) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this array
stack and using the elements of the resulting collections.
...
stack. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new array stack resulting from applying the given collection-valued
    function `f` to each element of this array stack and concatenating the
    results.
...
### `def map[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[ArrayStack[T], B, That]): That` ###
...
* returns
  * a new array stack resulting from applying the given function `f` to each
    element of this array stack and collecting the results.
...
Copies the elements of this array stack to an array. Fills the given array `xs`
with values of this array stack. Copying will stop once either the end of the
current array stack is reached, or the end of the target array is reached.
...
Copies the elements of this array stack to an array. Fills the given array `xs`
with values of this array stack, beginning at index `start` . Copying will stop
once either the end of the current array stack is reached, or the end of the
...
* returns
  * the first element of this array stack with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this array stack with the smallest value measured by
    function f.
...
Converts this array stack to a map. This method is unavailable unless the
...
### `def flatten[B](implicit asTraversable: (T) ⇒ GenTraversableOnce[B]): ArrayStack[B]` ###

[use case]

Converts this array stack of traversable collections into a array stack formed
...
--------------------------------------------------------------------------------
         Instance Constructors From scala.collection.mutable.ArrayStack
--------------------------------------------------------------------------------
...
* Definition Classes
  * ArrayStack → SeqLike → GenSeqLike → Function1
* Exceptions thrown
...
The factory companion object that builds instances of class `ArrayStack` . (or
its `Iterable` superclass where class `ArrayStack` is not a `Seq` .)

* Definition Classes
  * ArrayStack → IndexedSeq → IndexedSeq → Seq → Iterable → Traversable → Seq →
...
Empties the stack, passing all elements on it in LIFO order to the provided
function.
...
* Definition Classes
  * ArrayStack → IndexedSeqOptimized → IterableLike → GenericTraversableTemplate
    → TraversableLike → GenTraversableLike → TraversableOnce →
...
Evaluates the expression, preserving the contents of the stack so that any
changes the evaluation makes to the stack contents will be undone after it
completes.
...
* Definition Classes
  * ArrayStack → IndexedSeqLike → SeqLike
* Exceptions thrown
...
* Implicit information
  * This member is added by an implicit conversion from ArrayStack [T] to
...
* Implicit information
  * This member is added by an implicit conversion from ArrayStack [T] to
    CollectionsHaveToParArray [ArrayStack [T], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (ArrayStack [T]) ⇒
