...
Finds index of first occurrence of some value in this synchronized queue.

* elem
  * the element value to search for.
* returns
  * the index of the first element of this synchronized queue that is equal (as
...
Finds index of first occurrence of some value in this synchronized queue after
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this synchronized queue that is
...
Finds index of last occurrence of some value in this synchronized queue.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this synchronized queue that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this synchronized queue before
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this synchronized queue that is
    equal (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this synchronized queue to an array. Fills the given
array `xs` with at most `len` elements of this synchronized queue, starting at
position `start` . Copying will stop once either the end of the current
synchronized queue is reached, or the end of the target array is reached, or
...
Returns a synchronized queue formed from this synchronized queue and another
...
* thisElem
  * the element to be used to fill up the result if this synchronized queue is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    synchronized queue.
* returns
  * a new synchronized queue containing pairs consisting of corresponding
    elements of this synchronized queue and `that` . The length of the returned
    collection is the maximum of the lengths of this synchronized queue and
     `that` . If this synchronized queue is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this
    synchronized queue, `thatElem` values are used to pad the result.
...
Zips this synchronized queue with its indices.

* returns
  * A new synchronized queue containing pairs consisting of all elements of this
    synchronized queue paired with their index. Indices start at `0` .
...
* returns
  * a new synchronized queue containing pairs consisting of corresponding
    elements of this synchronized queue and `that` . The length of the returned
    collection is the minimum of the lengths of this synchronized queue and
     `that` .
...
### `def toCollection(repr: Queue[A]): collection.LinearSeq[A]`              ###
...
Applies a function `f` to all elements of this synchronized queue.
...
order as this synchronized queue.
...
A copy of the synchronized queue with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original synchronized queue is not modified, so you will want to
capture the result.
...
* returns
  * a new synchronized queue consisting of `elem` followed by all elements of
    this synchronized queue.
...
A copy of this synchronized queue with an element appended.
...
* returns
  * a new synchronized queue consisting of all elements of this synchronized
    queue followed by `elem` .
...
### `def combinations(n: Int): Iterator[Queue[A]]`                           ###
...
Computes the multiset difference between this synchronized queue and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new synchronized queue which contains all elements of this synchronized
    queue except some of occurrences of elements that also appear in `that` . If
    an element value `x` appears _n_ times in `that` , then the first _n_
...
Computes the multiset intersection between this synchronized queue and another
sequence.
...
* returns
  * a new synchronized queue which contains all elements of this synchronized
    queue which also appear in `that` . If an element value `x` appears _n_
...
A copy of this synchronized queue with an element value appended until a given
...
* returns
  * a new synchronized queue consisting of all elements of this synchronized
    queue followed by the minimal number of occurrences of `elem` so that the
    resulting synchronized queue has a length of at least `len` .
...
Produces a new synchronized queue where a slice of elements in this synchronized
queue is replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original synchronized queue
* returns
  * a new synchronized queue consisting of all elements of this synchronized
    queue except that `replaced` elements starting from `from` are replaced by
     `patch` .
...
### `def permutations: Iterator[Queue[A]]`                                   ###
...
synchronized queue and collecting the results in reversed order.
...
* returns
  * a new synchronized queue resulting from applying the given function `f` to
    each element of this synchronized queue and collecting the results in
    reversed order.
...
### `def sortBy[B](f: (A) ⇒ B)(implicit ord: math.Ordering[B]): Queue[A]`    ###
...
### `def sorted[B >: A](implicit ord: math.Ordering[B]): Queue[A]`           ###
...
Produces a new sequence which contains all elements of this synchronized queue
...
* returns
  * a new synchronized queue which contains all elements of this synchronized
    queue followed by all elements of `that` .
...
A copy of this synchronized queue with one single replaced element.
...
* returns
  * a copy of this synchronized queue with the element at position `index`
    replaced by `elem` .
...
### `def view(from: Int, until: Int): SeqView[A, Queue[A]]`                  ###
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[Queue[A], B, That]): That` ###
...
Returns a new synchronized queue containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the synchronized queue is the most specific superclass encompassing the
...
synchronized queue on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the synchronized queue.
* returns
  * a new synchronized queue resulting from applying the given partial function
...
### `def filterNot(p: (A) ⇒ Boolean): Queue[A]`                              ###
...
Builds a new collection by applying a function to all elements of this
synchronized queue and using the elements of the resulting collections.
...
synchronized queue. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new synchronized queue resulting from applying the given collection-valued
    function `f` to each element of this synchronized queue and concatenating
    the results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, Queue[A]]`                 ###
...
* returns
  * a new synchronized queue resulting from applying the given function `f` to
    each element of this synchronized queue and collecting the results.
...
### `def partition(p: (A) ⇒ Boolean): (Queue[A], Queue[A])`                  ###
...
### `def scanLeft[B, That](z: B)(op: (B, A) ⇒ B)(implicit bf: CanBuildFrom[Queue[A], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (A, B) ⇒ B)(implicit bf: CanBuildFrom[Queue[A], B, That]): That` ###
...
### `def scan[B >: A, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[Queue[A], B, That]): That` ###
...
### `def withFilter(p: (A) ⇒ Boolean): FilterMonadic[A, Queue[A]]`           ###
...
Copies the elements of this synchronized queue to an array. Fills the given
array `xs` with values of this synchronized queue. Copying will stop once either
the end of the current synchronized queue is reached, or the end of the target
array is reached.
...
Copies the elements of this synchronized queue to an array. Fills the given
array `xs` with values of this synchronized queue, beginning at index `start` .
Copying will stop once either the end of the current synchronized queue is
...
* returns
  * the first element of this synchronized queue with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this synchronized queue with the smallest value
...
Converts this synchronized queue to a map. This method is unavailable unless the
...
* returns
  * a map of type `immutable.Map[T, U]` containing all key/value pairs of type
     `(T, U)` of this synchronized queue.
...
Converts this synchronized queue of traversable collections into a synchronized
queue formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of
synchronized queue. For example:
...
* returns
  * a new synchronized queue resulting from concatenating all element
    synchronized queues.
...
### `def genericBuilder[B]: Builder[B, Queue[B]]`                            ###
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): Queue[Queue[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (A) ⇒ (A1, A2, A3)): (Queue[A1], Queue[A2], Queue[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (A) ⇒ (A1, A2)): (Queue[A1], Queue[A2])` ###
...
### `def +=(elem1: A, elem2: A, elems: A*): SynchronizedQueue.this.type`     ###
...
### `def +=:(elem: A): SynchronizedQueue.this.type`                          ###
...
--------------------------------------------------------------------------------
          Deprecated Value Members From scala.collection.mutable.Queue
--------------------------------------------------------------------------------


### `def extractFirst(start: LinkedList[A], p: (A) ⇒ Boolean): Option[LinkedList[A]]` ###

Return the proper suffix of this list which starts with the first element that
satisfies `p` . That element is unlinked from the list. If no element satisfies
 `p` , return None.
...
* Annotations
  * @ deprecated
* Deprecated
  * _(Since version 2.11.0)_ extractFirst inappropriately exposes implementation
    details. Use dequeue or dequeueAll.
...
* Definition Classes
  * Queue → MutableList → Cloneable → AnyRef
* Note
...
### `def companion: GenericCompanion[Queue]`                                 ###

The factory companion object that builds instances of class `mutable.Queue` .
(or its `Iterable` superclass where class `mutable.Queue` is not a `Seq` .)

* Definition Classes
  * Queue → MutableList → LinearSeq → LinearSeq → Seq → Iterable → Traversable →
...
### `def newBuilder: Builder[A, Queue[A]]`                                   ###
...
* Attributes
  * protected[this]
* Definition Classes
  * Queue → MutableList → GenericTraversableTemplate → TraversableLike →
    HasNewBuilder
...
* returns
  * a mutable queue consisting of all elements of this mutable queue except the
    first one.

* Definition Classes
  * Queue → MutableList → LinearSeqOptimized → TraversableLike →
    GenTraversableLike
...
### `def transform(f: (A) ⇒ A): SynchronizedQueue.this.type`                 ###
...
--------------------------------------------------------------------------------
     Instance Constructors From scala.collection.mutable.SynchronizedQueue
--------------------------------------------------------------------------------


### `new SynchronizedQueue()`                                                ###

(defined at scala.collection.mutable.SynchronizedQueue)


--------------------------------------------------------------------------------
         Value Members From scala.collection.mutable.SynchronizedQueue
--------------------------------------------------------------------------------


### `def ++=(xs: TraversableOnce[A]): SynchronizedQueue.this.type`           ###

Adds all elements provided by a `TraversableOnce` object at the end of the
queue. The elements are prepended in the order they are given out by the
iterator.

* xs
  * a traversable object
* returns
  * the synchronized queue itself.

* Definition Classes
  * SynchronizedQueue → Growable
...
* Definition Classes
  * SynchronizedQueue → MutableList → Builder → Growable
...
Returns all elements in the queue which satisfy the given predicate, and removes
those elements from the queue.
...
Returns the first element in the queue which satisfies the given predicate, and
...
* Definition Classes
  * SynchronizedQueue → GenSeqLike → Equals → AnyRef → Any
...
* Implicit information
  * This member is added by an implicit conversion from SynchronizedQueue [A] to
    CollectionsHaveToParArray [SynchronizedQueue [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (SynchronizedQueue [A]) ⇒
