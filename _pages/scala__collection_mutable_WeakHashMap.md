...
Copies the elements of this weak hash map to an array. Fills the given array
 `xs` with at most `len` elements of this weak hash map, starting at position
 `start` . Copying will stop once either the end of the current weak hash map is
...
### `def drop(n: Int): WeakHashMap[A, B]`                                    ###
...
### `def dropRight(n: Int): WeakHashMap[A, B]`                               ###
...
### `def grouped(size: Int): Iterator[WeakHashMap[A, B]]`                    ###
...
### `def slice(from: Int, until: Int): WeakHashMap[A, B]`                    ###
...
### `def sliding(size: Int): Iterator[WeakHashMap[A, B]]`                    ###
...
### `def sliding(size: Int, step: Int): Iterator[WeakHashMap[A, B]]`         ###
...
### `def take(n: Int): WeakHashMap[A, B]`                                    ###
...
### `def takeRight(n: Int): WeakHashMap[A, B]`                               ###
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): WeakHashMap[A, B]`                ###
...
### `def toCollection(repr: WeakHashMap[A, B]): collection.Iterable[(A, B)]` ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), WeakHashMap[A, B]]` ###
...
### `def view: IterableView[(A, B), WeakHashMap[A, B]]`                      ###
...
### `def zipAll[B, A1 >: (A, B), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[WeakHashMap[A, B], (A1, B), That]): That` ###

[use case]

Returns a weak hash map formed from this weak hash map and another iterable
...
* thisElem
  * the element to be used to fill up the result if this weak hash map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    weak hash map.
* returns
  * a new weak hash map containing pairs consisting of corresponding elements of
    this weak hash map and `that` . The length of the returned collection is the
    maximum of the lengths of this weak hash map and `that` . If this weak hash
    map is shorter than `that` , `thisElem` values are used to pad the result.
    If `that` is shorter than this weak hash map, `thatElem` values are used to
    pad the result.
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[WeakHashMap[A, B], (A1, Int), That]): That` ###
...
* returns
  * A new weak hash map containing pairs consisting of all elements of this weak
...
### `def zip[A1 >: (A, B), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], (A1, B), That]): That` ###
...
* returns
  * a new weak hash map containing pairs consisting of corresponding elements of
    this weak hash map and `that` . The length of the returned collection is the
    minimum of the lengths of this weak hash map and `that` .
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): WeakHashMap[A, B]`                ###
...
### `def ++:[B >: (A, B), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
### `def ++:[B >: (A, B), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
### `def ++[B >: (A, B), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###

[use case]

Returns a new weak hash map containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
weak hash map is the most specific superclass encompassing the element types of
the two operands.
...
### `def collect[B, That](pf: PartialFunction[(A, B), B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
weak hash map on which the function is defined.
...
* returns
  * a new weak hash map resulting from applying the given partial function `pf`
...
### `def dropWhile(p: ((A, B)) ⇒ Boolean): WeakHashMap[A, B]`                ###
...
### `def filter(p: ((A, B)) ⇒ Boolean): WeakHashMap[A, B]`                   ###
...
### `def flatMap[B, That](f: ((A, B)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this weak hash
...
The type of the resulting collection is guided by the static type of weak hash
...
* returns
  * a new weak hash map resulting from applying the given collection-valued
    function `f` to each element of this weak hash map and concatenating the
    results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, WeakHashMap[A, B]]`   ###
...
### `def inits: Iterator[WeakHashMap[A, B]]`                                 ###
...
### `def map[B, That](f: ((A, B)) ⇒ B)(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new weak hash map resulting from applying the given function `f` to each
    element of this weak hash map and collecting the results.
...
### `def partition(p: ((A, B)) ⇒ Boolean): (WeakHashMap[A, B], WeakHashMap[A, B])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[WeakHashMap[A, B], B, That]): That` ###
...
### `def span(p: ((A, B)) ⇒ Boolean): (WeakHashMap[A, B], WeakHashMap[A, B])` ###
...
### `def splitAt(n: Int): (WeakHashMap[A, B], WeakHashMap[A, B])`            ###
...
### `def tails: Iterator[WeakHashMap[A, B]]`                                 ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), WeakHashMap[A, B]]` ###
...
Copies the elements of this weak hash map to an array. Fills the given array
 `xs` with values of this weak hash map. Copying will stop once either the end
of the current weak hash map is reached, or the end of the target array is
reached.
...
Copies the elements of this weak hash map to an array. Fills the given array
 `xs` with values of this weak hash map, beginning at index `start` . Copying
will stop once either the end of the current weak hash map is reached, or the
...
* returns
  * the first element of this weak hash map with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this weak hash map with the smallest value measured by
    function f.
...
Converts this weak hash map to a map. This method is unavailable unless the
...
--------------------------------------------------------------------------------
      Value Members From scala.collection.convert.Wrappers.JMapWrapperLike
--------------------------------------------------------------------------------


### `def +=(kv: (A, B)): WeakHashMap.this.type`                              ###
...
(defined at scala.collection.convert.Wrappers.JMapWrapperLike)


### `def -=(key: A): WeakHashMap.this.type`                                  ###
...
Converts this weak hash map of traversable collections into a weak hash map
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of weak hash
map. For example:
...
### `def ++=(xs: TraversableOnce[(A, B)]): WeakHashMap.this.type`            ###
...
### `def +=(elem1: (A, B), elem2: (A, B), elems: (A, B)*): WeakHashMap.this.type` ###
...
### `def --=(xs: TraversableOnce[A]): WeakHashMap.this.type`                 ###
...
### `def -=(elem1: A, elem2: A, elems: A*): WeakHashMap.this.type`           ###
...
### `def mapResult[NewTo](f: (WeakHashMap[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
### `def -(elem1: A, elem2: A, elems: A*): WeakHashMap[A, B]`                ###
...
### `def --(xs: GenTraversableOnce[A]): WeakHashMap[A, B]`                   ###
...
### `def newBuilder: Builder[(A, B), WeakHashMap[A, B]]`                     ###
...
### `def retain(p: (A, B) ⇒ Boolean): WeakHashMap.this.type`                 ###
...
### `def transform(f: (A, B) ⇒ B): WeakHashMap.this.type`                    ###
...
--------------------------------------------------------------------------------
        Instance Constructors From scala.collection.mutable.WeakHashMap
--------------------------------------------------------------------------------
...
* Definition Classes
  * WeakHashMap → JMapWrapper → JMapWrapperLike → Map → Map → MapLike
...
* Implicit information
  * This member is added by an implicit conversion from WeakHashMap [A, B] to
    CollectionsHaveToParArray [WeakHashMap [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (WeakHashMap [A, B]) ⇒
