...
Returns a immutable map formed from this immutable map and another iterable
...
* thisElem
  * the element to be used to fill up the result if this immutable map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable map.
* returns
  * a new immutable map containing pairs consisting of corresponding elements of
    this immutable map and `that` . The length of the returned collection is the
    maximum of the lengths of this immutable map and `that` . If this immutable
    map is shorter than `that` , `thisElem` values are used to pad the result.
    If `that` is shorter than this immutable map, `thatElem` values are used to
    pad the result.
...
* returns
  * A new immutable map containing pairs consisting of all elements of this
    immutable map paired with their index. Indices start at `0` .
...
* returns
  * a new immutable map containing pairs consisting of corresponding elements of
    this immutable map and `that` . The length of the returned collection is the
    minimum of the lengths of this immutable map and `that` .
...
--------------------------------------------------------------------------------
           Concrete Value Members From scala.collection.MapProxyLike
--------------------------------------------------------------------------------


### `def addString(b: StringBuilder, start: String, sep: String, end: String): StringBuilder` ###

Appends all elements of this map to a string builder using start, end, and
...
the string `end` . Inside, the string representations (w.r.t. the method
 `toString` ) of all elements of this map are separated by the string `sep` .
...
(defined at scala.collection.MapProxyLike)
...
* returns
  * a new map consisting of all elements of this map that do not satisfy the
    given predicate `p` . The order of the elements is preserved.

* Definition Classes
  * MapProxyLike → TraversableProxyLike → MapLike → TraversableLike →
    GenTraversableLike
...
* that
  * the traversable to append.
* returns
  * a new immutable map which contains all elements of this immutable map
...
* returns
  * the first element of this immutable map with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this immutable map with the smallest value measured by
    function f.
...
Returns a new immutable map containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
immutable map is the most specific superclass encompassing the element types of
the two operands.
...
immutable map on which the function is defined.
...
* returns
  * a new immutable map resulting from applying the given partial function `pf`
...
Copies the elements of this immutable map to an array. Fills the given array
 `xs` with values of this immutable map. Copying will stop once either the end
of the current immutable map is reached, or the end of the target array is
reached.
...
Copies the elements of this immutable map to an array. Fills the given array
 `xs` with values of this immutable map, beginning at index `start` . Copying
will stop once either the end of the current immutable map is reached, or the
...
Copies the elements of this immutable map to an array. Fills the given array
 `xs` with at most `len` elements of this immutable map, starting at position
 `start` . Copying will stop once either the end of the current immutable map is
...
* returns
  * a new immutable map resulting from applying the given collection-valued
    function `f` to each element of this immutable map and concatenating the
    results.
...
* f
  * the function to apply to each element.
* returns
  * a new immutable map resulting from applying the given function `f` to each
    element of this immutable map and collecting the results.
...
Converts this immutable map to a map. This method is unavailable unless the
...
Converts this immutable map of traversable collections into a immutable map
...
--------------------------------------------------------------------------------
           Concrete Value Members From scala.collection.immutable.Map
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
         Concrete Value Members From scala.collection.immutable.MapLike
--------------------------------------------------------------------------------
...
### `def transform[C, That](f: (A, B) ⇒ C)(implicit bf: CanBuildFrom[Map[A, B], (A, C), That]): That` ###
...
--------------------------------------------------------------------------------
        Concrete Value Members From scala.collection.immutable.MapProxy
--------------------------------------------------------------------------------
...
(defined at scala.collection.immutable.MapProxy)
...
* Definition Classes
  * MapProxy → MapProxyLike → Map → MapLike → MapLike → GenMapLike
...
* Definition Classes
  * MapProxy → MapProxyLike → MapLike → Subtractable → GenMapLike
...
* Definition Classes
  * MapProxy → MapProxyLike → Map → MapLike → MapLike → GenMap
...
* Implicit information
  * This member is added by an implicit conversion from MapProxy [A, B] to
    CollectionsHaveToParArray [MapProxy [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (MapProxy [A, B]) ⇒
