...
### `def andThen[C](k: (V) ⇒ C): PartialFunction[Long, C]`                   ###
...
### `def applyOrElse[A1 <: Long, B1 >: V](x: A1, default: (A1) ⇒ B1): B1`    ###
...
### `def orElse[A1 <: Long, B1 >: V](that: PartialFunction[A1, B1]): PartialFunction[A1, B1]` ###
...
### `def runWith[U](action: (V) ⇒ U): (Long) ⇒ Boolean`                      ###
...
### `def copyToArray[B >: (Long, V)](xs: Array[B], start: Int, len: Int): Unit` ###
...
### `def exists(p: ((Long, V)) ⇒ Boolean): Boolean`                          ###
...
### `def find(p: ((Long, V)) ⇒ Boolean): Option[(Long, V)]`                  ###
...
### `def foldRight[B](z: B)(op: ((Long, V), B) ⇒ B): B`                      ###
...
### `def forall(p: ((Long, V)) ⇒ Boolean): Boolean`                          ###
...
### `def grouped(size: Int): Iterator[LongMap[V]]`                           ###
...
### `def reduceRight[B >: (Long, V)](op: ((Long, V), B) ⇒ B): B`             ###
...
### `def sameElements[B >: (Long, V)](that: GenIterable[B]): Boolean`        ###
...
### `def slice(from: Int, until: Int): LongMap[V]`                           ###
...
### `def sliding(size: Int): Iterator[LongMap[V]]`                           ###
...
### `def sliding(size: Int, step: Int): Iterator[LongMap[V]]`                ###
...
### `def takeWhile(p: ((Long, V)) ⇒ Boolean): LongMap[V]`                    ###
...
### `def thisCollection: collection.Iterable[(Long, V)]`                     ###
...
### `def toCollection(repr: LongMap[V]): collection.Iterable[(Long, V)]`     ###
...
### `def toIterable: collection.Iterable[(Long, V)]`                         ###
...
### `def toIterator: Iterator[(Long, V)]`                                    ###
...
### `def toStream: immutable.Stream[(Long, V)]`                              ###
...
### `def view(from: Int, until: Int): IterableView[(Long, V), LongMap[V]]`   ###
...
### `def view: IterableView[(Long, V), LongMap[V]]`                          ###
...
### `def zipAll[B, A1 >: (Long, V), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[LongMap[V], (A1, B), That]): That` ###
...
### `def zipWithIndex[A1 >: (Long, V), That](implicit bf: CanBuildFrom[LongMap[V], (A1, Int), That]): That` ###
...
### `def zip[A1 >: (Long, V), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[LongMap[V], (A1, B), That]): That` ###
...
### `def filterKeys(p: (Long) ⇒ Boolean): collection.Map[Long, V]`           ###
...
### `def filterNot(p: ((Long, V)) ⇒ Boolean): LongMap[V]`                    ###
...
### `def isDefinedAt(key: Long): Boolean`                                    ###
...
### `def keySet: collection.Set[Long]`                                       ###
...
### `def keys: collection.Iterable[Long]`                                    ###
...
### `def mapValues[C](f: (V) ⇒ C): collection.Map[Long, C]`                  ###
...
### `def toBuffer[C >: (Long, V)]: Buffer[C]`                                ###
...
### `def ++:[B >: (Long, V), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def ++:[B >: (Long, V), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def ++[B >: (Long, V), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def collect[B, That](pf: PartialFunction[(Long, V), B])(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def dropWhile(p: ((Long, V)) ⇒ Boolean): LongMap[V]`                    ###
...
### `def filter(p: ((Long, V)) ⇒ Boolean): LongMap[V]`                       ###
...
### `def flatMap[B, That](f: ((Long, V)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def groupBy[K](f: ((Long, V)) ⇒ K): immutable.Map[K, LongMap[V]]`       ###
...
### `def map[B, That](f: ((Long, V)) ⇒ B)(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def partition(p: ((Long, V)) ⇒ Boolean): (LongMap[V], LongMap[V])`      ###
...
### `def scanLeft[B, That](z: B)(op: (B, (Long, V)) ⇒ B)(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((Long, V), B) ⇒ B)(implicit bf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def scan[B >: (Long, V), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[LongMap[V], B, That]): That` ###
...
### `def span(p: ((Long, V)) ⇒ Boolean): (LongMap[V], LongMap[V])`           ###
...
### `def splitAt(n: Int): (LongMap[V], LongMap[V])`                          ###
...
### `def toTraversable: collection.Traversable[(Long, V)]`                   ###
...
### `def withFilter(p: ((Long, V)) ⇒ Boolean): FilterMonadic[(Long, V), LongMap[V]]` ###
...
### `def aggregate[B](z: ⇒ B)(seqop: (B, (Long, V)) ⇒ B, combop: (B, B) ⇒ B): B` ###
...
### `def collectFirst[B](pf: PartialFunction[(Long, V), B]): Option[B]`      ###
...
### `def copyToArray[B >: (Long, V)](xs: Array[B]): Unit`                    ###
...
### `def copyToArray[B >: (Long, V)](xs: Array[B], start: Int): Unit`        ###
...
### `def copyToBuffer[B >: (Long, V)](dest: Buffer[B]): Unit`                ###
...
### `def count(p: ((Long, V)) ⇒ Boolean): Int`                               ###
...
### `def foldLeft[B](z: B)(op: (B, (Long, V)) ⇒ B): B`                       ###
...
### `def fold[A1 >: (Long, V)](z: A1)(op: (A1, A1) ⇒ A1): A1`                ###
...
### `def maxBy[B](f: ((Long, V)) ⇒ B)(implicit cmp: Ordering[B]): (Long, V)` ###
...
### `def minBy[B](f: ((Long, V)) ⇒ B)(implicit cmp: Ordering[B]): (Long, V)` ###
...
### `def reduceLeftOption[B >: (Long, V)](op: (B, (Long, V)) ⇒ B): Option[B]` ###
...
### `def reduceLeft[B >: (Long, V)](op: (B, (Long, V)) ⇒ B): B`              ###
...
### `def reduceOption[A1 >: (Long, V)](op: (A1, A1) ⇒ A1): Option[A1]`       ###
...
### `def reduceRightOption[B >: (Long, V)](op: ((Long, V), B) ⇒ B): Option[B]` ###
...
### `def reduce[A1 >: (Long, V)](op: (A1, A1) ⇒ A1): A1`                     ###
...
### `def toIndexedSeq: immutable.IndexedSeq[(Long, V)]`                      ###
...
### `def toMap[T, U](implicit ev: <:<[(Long, V), (T, U)]): immutable.Map[T, U]` ###
...
### `def toSet[B >: (Long, V)]: immutable.Set[B]`                            ###
...
### `def flatten[B](implicit asTraversable: ((Long, V)) ⇒ GenTraversableOnce[B]): Iterable[B]` ###
...
### `def transpose[B](implicit asTraversable: ((Long, V)) ⇒ GenTraversableOnce[B]): Iterable[Iterable[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: ((Long, V)) ⇒ (A1, A2, A3)): (Iterable[A1], Iterable[A2], Iterable[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: ((Long, V)) ⇒ (A1, A2)): (Iterable[A1], Iterable[A2])` ###
...
### `def ++=(xs: TraversableOnce[(Long, V)]): LongMap.this.type`             ###
...
### `def +=(elem1: (Long, V), elem2: (Long, V), elems: (Long, V)*): LongMap.this.type` ###
...
### `def --=(xs: TraversableOnce[Long]): LongMap.this.type`                  ###
...
### `def -=(elem1: Long, elem2: Long, elems: Long*): LongMap.this.type`      ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.LongMap
--------------------------------------------------------------------------------


### `new LongMap(defaultEntry: (Long) ⇒ V, initialBufferSize: Int)`          ###
...
### `def ++[V1 >: V](xs: GenTraversableOnce[(Long, V1)]): LongMap[V1]`       ###
...
* Definition Classes
  * LongMap → MapLike → MapLike

(defined at scala.collection.mutable.LongMap)


### `def +=(kv: (Long, V)): LongMap.this.type`                               ###
...
* Definition Classes
  * LongMap → MapLike → Builder → Growable

(defined at scala.collection.mutable.LongMap)


### `def +=(key: Long, value: V): LongMap.this.type`                         ###
...
### `def +[V1 >: V](kv: (Long, V1)): LongMap[V1]`                            ###
...
* Definition Classes
  * LongMap → MapLike → MapLike → GenMapLike

(defined at scala.collection.mutable.LongMap)


### `def -=(key: Long): LongMap.this.type`                                   ###
...
the `defaultEntry` for that key will be returned instead.
...
* Definition Classes
  * LongMap → MapLike → Cloneable → AnyRef
* Note
...
### `def default(key: Long): V`                                              ###

The user-supplied default value for the key. Throws an exception if no other
default behavior was specified.
...
Creates a new `LongMap` with different values. Unlike `mapValues` , this method
generates a new collection immediately.
...
### `def transformValues(f: (V) ⇒ V): LongMap.this.type`                     ###
...
* Definition Classes
  * LongMap → MapLike → MapLike → GenMap
...
### `def withDefault(d: (Long) ⇒ V): Map[Long, V]`                           ###
...
### `def withDefaultValue(d: V): Map[Long, V]`                               ###
...
### `def +[B1 >: V](elem1: (Long, B1), elem2: (Long, B1), elems: (Long, B1)*): Map[Long, B1]` ###
...
### `def -(elem1: Long, elem2: Long, elems: Long*): LongMap[V]`              ###
...
### `def --(xs: GenTraversableOnce[Long]): LongMap[V]`                       ###
...
### `def newBuilder: Builder[(Long, V), LongMap[V]]`                         ###
...
### `def parCombiner: Combiner[(Long, V), ParMap[Long, V]]`                  ###
...
### `def retain(p: (Long, V) ⇒ Boolean): LongMap.this.type`                  ###
...
### `def toSeq: collection.Seq[(Long, V)]`                                   ###
...
### `def transform(f: (Long, V) ⇒ V): LongMap.this.type`                     ###
...
* Implicit information
  * This member is added by an implicit conversion from LongMap [V] to
    CollectionsHaveToParArray [LongMap [V], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (LongMap [V]) ⇒
