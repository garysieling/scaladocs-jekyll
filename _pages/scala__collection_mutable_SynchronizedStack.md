...
* returns
  * the index of the first element of this synchronized stack that is equal (as
...
Finds index of first occurrence of some value in this synchronized stack after
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this synchronized stack that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this synchronized stack that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this synchronized stack before
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this synchronized stack that is
    equal (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this synchronized stack to an array. Fills the given
array `xs` with at most `len` elements of this synchronized stack, starting at
position `start` . Copying will stop once either the end of the current
synchronized stack is reached, or the end of the target array is reached, or
...
order as this synchronized stack.
...
Returns a synchronized stack formed from this synchronized stack and another
...
* thisElem
  * the element to be used to fill up the result if this synchronized stack is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    synchronized stack.
* returns
  * a new synchronized stack containing pairs consisting of corresponding
    elements of this synchronized stack and `that` . The length of the returned
    collection is the maximum of the lengths of this synchronized stack and
     `that` . If this synchronized stack is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this
    synchronized stack, `thatElem` values are used to pad the result.
...
* returns
  * A new synchronized stack containing pairs consisting of all elements of this
    synchronized stack paired with their index. Indices start at `0` .
...
* returns
  * a new synchronized stack containing pairs consisting of corresponding
    elements of this synchronized stack and `that` . The length of the returned
    collection is the minimum of the lengths of this synchronized stack and
     `that` .
...
A copy of the synchronized stack with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original synchronized stack is not modified, so you will want to
capture the result.
...
* returns
  * a new synchronized stack consisting of `elem` followed by all elements of
    this synchronized stack.
...
A copy of this synchronized stack with an element appended.
...
* returns
  * a new synchronized stack consisting of all elements of this synchronized
    stack followed by `elem` .
...
Computes the multiset difference between this synchronized stack and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new synchronized stack which contains all elements of this synchronized
    stack except some of occurrences of elements that also appear in `that` . If
...
Computes the multiset intersection between this synchronized stack and another
sequence.
...
* returns
  * a new synchronized stack which contains all elements of this synchronized
    stack which also appear in `that` . If an element value `x` appears _n_
...
A copy of this synchronized stack with an element value appended until a given
...
* returns
  * a new synchronized stack consisting of all elements of this synchronized
    stack followed by the minimal number of occurrences of `elem` so that the
    resulting synchronized stack has a length of at least `len` .
...
Produces a new synchronized stack where a slice of elements in this synchronized
stack is replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original synchronized stack
* returns
  * a new synchronized stack consisting of all elements of this synchronized
    stack except that `replaced` elements starting from `from` are replaced by
     `patch` .
...
synchronized stack and collecting the results in reversed order.
...
* returns
  * a new synchronized stack resulting from applying the given function `f` to
    each element of this synchronized stack and collecting the results in
    reversed order.
...
Produces a new sequence which contains all elements of this synchronized stack
...
* returns
  * a new synchronized stack which contains all elements of this synchronized
    stack followed by all elements of `that` .
...
* returns
  * a copy of this synchronized stack with the element at position `index`
    replaced by `elem` .
...
Returns a new synchronized stack containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the synchronized stack is the most specific superclass encompassing the
...
synchronized stack on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the synchronized stack.
* returns
  * a new synchronized stack resulting from applying the given partial function
...
Builds a new collection by applying a function to all elements of this
synchronized stack and using the elements of the resulting collections.
...
synchronized stack. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new synchronized stack resulting from applying the given collection-valued
    function `f` to each element of this synchronized stack and concatenating
    the results.
...
* returns
  * a new synchronized stack resulting from applying the given function `f` to
    each element of this synchronized stack and collecting the results.
...
Copies the elements of this synchronized stack to an array. Fills the given
array `xs` with values of this synchronized stack. Copying will stop once either
the end of the current synchronized stack is reached, or the end of the target
array is reached.
...
Copies the elements of this synchronized stack to an array. Fills the given
array `xs` with values of this synchronized stack, beginning at index `start` .
Copying will stop once either the end of the current synchronized stack is
...
* returns
  * the first element of this synchronized stack with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this synchronized stack with the smallest value
...
Converts this synchronized stack to a map. This method is unavailable unless the
...
* returns
  * a map of type `immutable.Map[T, U]` containing all key/value pairs of type
     `(T, U)` of this synchronized stack.
...
Converts this synchronized stack of traversable collections into a synchronized
stack formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of
synchronized stack. For example:
...
* returns
  * a new synchronized stack resulting from concatenating all element
    synchronized stacks.
...
### `def transform(f: (A) ⇒ A): SynchronizedStack.this.type`                 ###
...
Applies a function `f` to all elements of this synchronized stack.
...
--------------------------------------------------------------------------------
     Instance Constructors From scala.collection.mutable.SynchronizedStack
--------------------------------------------------------------------------------


### `new SynchronizedStack()`                                                ###

(defined at scala.collection.mutable.SynchronizedStack)


--------------------------------------------------------------------------------
         Value Members From scala.collection.mutable.SynchronizedStack
--------------------------------------------------------------------------------


### `def push(elem: A): SynchronizedStack.this.type`                         ###
...
### `def push(elem1: A, elem2: A, elems: A*): SynchronizedStack.this.type`   ###

Push two or more elements onto the stack. The last element of the sequence will
be on top of the new stack.

* elem1
  * the first element to push.
* elem2
  * the second element to push.
* elems
  * the element sequence that will be pushed.
* returns
...
### `def pushAll(xs: TraversableOnce[A]): SynchronizedStack.this.type`       ###

Pushes all elements provided by a traversable object on top of the stack. The
elements are pushed in the order the traversable object is traversed.
...
* Implicit information
  * This member is added by an implicit conversion from SynchronizedStack [A] to
    CollectionsHaveToParArray [SynchronizedStack [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (SynchronizedStack [A]) ⇒
