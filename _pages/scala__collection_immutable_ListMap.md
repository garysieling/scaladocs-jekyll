...
### `def dropRight(n: Int): ListMap[A, B]`                                   ###
...
### `def grouped(size: Int): Iterator[ListMap[A, B]]`                        ###
...
### `def slice(from: Int, until: Int): ListMap[A, B]`                        ###
...
### `def sliding(size: Int): Iterator[ListMap[A, B]]`                        ###
...
### `def sliding(size: Int, step: Int): Iterator[ListMap[A, B]]`             ###
...
### `def takeRight(n: Int): ListMap[A, B]`                                   ###
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): ListMap[A, B]`                    ###
...
### `def toCollection(repr: ListMap[A, B]): collection.Iterable[(A, B)]`     ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), ListMap[A, B]]`   ###
...
### `def view: IterableView[(A, B), ListMap[A, B]]`                          ###
...
### `def zipAll[B, A1 >: (A, B), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[ListMap[A, B], (A1, B), That]): That` ###
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[ListMap[A, B], (A1, Int), That]): That` ###
...
### `def zip[A1 >: (A, B), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[ListMap[A, B], (A1, B), That]): That` ###
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): ListMap[A, B]`                    ###
...
### `def newBuilder: Builder[(A, B), ListMap[A, B]]`                         ###
...
### `def ++:[B >: (A, B), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def ++:[B >: (A, B), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def ++[B >: (A, B), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def collect[B, That](pf: PartialFunction[(A, B), B])(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def dropWhile(p: ((A, B)) ⇒ Boolean): ListMap[A, B]`                    ###
...
### `def flatMap[B, That](f: ((A, B)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def groupBy[K](f: ((A, B)) ⇒ K): Map[K, ListMap[A, B]]`                 ###
...
### `def inits: Iterator[ListMap[A, B]]`                                     ###
...
### `def map[B, That](f: ((A, B)) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def partition(p: ((A, B)) ⇒ Boolean): (ListMap[A, B], ListMap[A, B])`   ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[ListMap[A, B], B, That]): That` ###
...
### `def span(p: ((A, B)) ⇒ Boolean): (ListMap[A, B], ListMap[A, B])`        ###
...
### `def splitAt(n: Int): (ListMap[A, B], ListMap[A, B])`                    ###
...
### `def tails: Iterator[ListMap[A, B]]`                                     ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), ListMap[A, B]]` ###
...
### `def -(elem1: A, elem2: A, elems: A*): ListMap[A, B]`                    ###
...
### `def --(xs: GenTraversableOnce[A]): ListMap[A, B]`                       ###
...
--------------------------------------------------------------------------------
         Instance Constructors From scala.collection.immutable.ListMap
--------------------------------------------------------------------------------
...
### `def ++[B1 >: B](xs: GenTraversableOnce[(A, B1)]): ListMap[A, B1]`       ###
...
### `def +[B1 >: B](elem1: (A, B1), elem2: (A, B1), elems: (A, B1)*): ListMap[A, B1]` ###
...
This creates a new mapping without the given `key` . If the map does not contain
a mapping for the given key, the method returns the same map.

* key
  * a map without a mapping for the given key.
* returns
  * a new map without a binding for `key`

* Definition Classes
  * ListMap → MapLike → Subtractable → GenMapLike
...
* Definition Classes
  * ListMap → MapLike → GenMapLike

(defined at scala.collection.immutable.ListMap)


### `def next: ListMap[A, B]`                                                ###
...
### `def updated[B1 >: B](key: A, value: B1): ListMap[A, B1]`                ###
...
* key
  * the key element of the updated entry.
* value
  * the value element of the updated entry.
* returns
  * A new map with the new binding added to this map
...
### `def transform[C, That](f: (A, B) ⇒ C)(implicit bf: CanBuildFrom[ListMap[A, B], (A, C), That]): That` ###
...
* Implicit information
  * This member is added by an implicit conversion from ListMap [A, B] to
    CollectionsHaveToParArray [ListMap [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (ListMap [A, B]) ⇒
