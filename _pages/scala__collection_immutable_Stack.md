...
Finds index of first occurrence of some value in this immutable stack.

* elem
  * the element value to search for.
* returns
  * the index of the first element of this immutable stack that is equal (as
...
Finds index of first occurrence of some value in this immutable stack after or
at some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this immutable stack that is
...
Finds index of last occurrence of some value in this immutable stack.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this immutable stack that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this immutable stack before or
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this immutable stack that is equal
...
Copies the elements of this immutable stack to an array. Fills the given array
 `xs` with at most `len` elements of this immutable stack, starting at position
 `start` . Copying will stop once either the end of the current immutable stack
...
Returns a immutable stack formed from this immutable stack and another iterable
...
* thisElem
  * the element to be used to fill up the result if this immutable stack is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable stack.
* returns
  * a new immutable stack containing pairs consisting of corresponding elements
    of this immutable stack and `that` . The length of the returned collection
    is the maximum of the lengths of this immutable stack and `that` . If this
    immutable stack is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this immutable stack, `thatElem`
...
Zips this immutable stack with its indices.

* returns
  * A new immutable stack containing pairs consisting of all elements of this
    immutable stack paired with their index. Indices start at `0` .
...
* returns
  * a new immutable stack containing pairs consisting of corresponding elements
    of this immutable stack and `that` . The length of the returned collection
    is the minimum of the lengths of this immutable stack and `that` .
...
### `def toCollection(repr: Stack[A]): collection.LinearSeq[A]`              ###
...
Applies a function `f` to all elements of this immutable stack.
...
A copy of the immutable stack with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original immutable stack is not modified, so you will want to capture
the result.
...
* returns
  * a new immutable stack consisting of `elem` followed by all elements of this
    immutable stack.
...
A copy of this immutable stack with an element appended.
...
* returns
  * a new immutable stack consisting of all elements of this immutable stack
    followed by `elem` .
...
Computes the multiset difference between this immutable stack and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new immutable stack which contains all elements of this immutable stack
...
Computes the multiset intersection between this immutable stack and another
sequence.
...
A copy of this immutable stack with an element value appended until a given
...
* returns
  * a new immutable stack consisting of all elements of this immutable stack
    ...
    resulting immutable stack has a length of at least `len` .
...
Produces a new immutable stack where a slice of elements in this immutable stack
is replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original immutable stack
* returns
  * a new immutable stack consisting of all elements of this immutable stack
...
* returns
  * a new immutable stack resulting from applying the given function `f` to each
    element of this immutable stack and collecting the results in reversed
    order.
...
Produces a new sequence which contains all elements of this immutable stack and
...
A copy of this immutable stack with one single replaced element.
...
* returns
  * a copy of this immutable stack with the element at position `index` replaced
    by `elem` .
...
Returns a new immutable stack containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
immutable stack is the most specific superclass encompassing the element types
of the two operands.
...
immutable stack on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the immutable stack.
* returns
  * a new immutable stack resulting from applying the given partial function
...
* f
  * the function to apply to each element.
* returns
  * a new immutable stack resulting from applying the given collection-valued
    function `f` to each element of this immutable stack and concatenating the
    results.
...
* returns
  * a new immutable stack resulting from applying the given function `f` to each
    element of this immutable stack and collecting the results.
...
Copies the elements of this immutable stack to an array. Fills the given array
 `xs` with values of this immutable stack. Copying will stop once either the end
of the current immutable stack is reached, or the end of the target array is
reached.
...
Copies the elements of this immutable stack to an array. Fills the given array
 `xs` with values of this immutable stack, beginning at index `start` . Copying
will stop once either the end of the current immutable stack is reached, or the
...
* returns
  * the first element of this immutable stack with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this immutable stack with the smallest value measured
    by function f.
...
Converts this immutable stack to a map. This method is unavailable unless the
...
Converts this immutable stack of traversable collections into a immutable stack
...
* returns
  * a new immutable stack resulting from concatenating all element immutable
    stacks.
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.immutable.Stack
--------------------------------------------------------------------------------
...
### `def companion: GenericCompanion[Stack]`                                 ###

The factory companion object that builds instances of class `immutable.Stack` .
(or its `Iterable` superclass where class `immutable.Stack` is not a `Seq` .)

* Definition Classes
  * Stack → LinearSeq → LinearSeq → Seq → Iterable → Traversable → Seq → GenSeq
...
Removes the top element from the stack. Note: should return `(A, Stack[A])` as
for queues (mics)
...
Push all elements provided by the given traversable object onto the stack. The
last element returned by the traversable object will be on top of the new stack.
...
Push a sequence of elements onto the stack. The last element of the sequence
...
Returns new immutable stack with elements in reversed order.

* returns
  * A new immutable stack with all elements of this immutable stack in reversed
    order.
...
* returns
  * a immutable stack consisting of all elements of this immutable stack except
    the first one.

* Definition Classes
  * Stack → LinearSeqOptimized → TraversableLike → GenTraversableLike
* Exceptions thrown
  * UnsupportedOperationException if the immutable stack is empty.
...
* Implicit information
  * This member is added by an implicit conversion from Stack [A] to
    CollectionsHaveToParArray [Stack [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (Stack [A]) ⇒
