...
### `type Self = collection.IndexedSeq[T]`                                   ###
...
Finds index of first occurrence of some value in this paged sequence.

* elem
  * the element value to search for.
* returns
  * the index of the first element of this paged sequence that is equal (as
...
Finds index of first occurrence of some value in this paged sequence after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this paged sequence that is
...
Finds index of last occurrence of some value in this paged sequence.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this paged sequence that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this paged sequence before or at
a given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this paged sequence that is equal
...
### `def companion: GenericCompanion[collection.IndexedSeq]`                 ###
...
* Definition Classes
  * IndexedSeq → Seq → GenSeq → Iterable → GenIterable → Traversable →
    GenTraversable → GenericTraversableTemplate

(defined at scala.collection.IndexedSeq)


### `def seq: collection.IndexedSeq[T]`                                      ###
...
* Definition Classes
  * IndexedSeq → IndexedSeqLike → Seq → GenSeq → GenSeqLike → Iterable →
...
### `def toCollection(repr: collection.IndexedSeq[T]): collection.IndexedSeq[T]` ###
...
Copies the elements of this paged sequence to an array. Fills the given array
 `xs` with at most `len` elements of this paged sequence, starting at position
 `start` . Copying will stop once either the end of the current paged sequence
...
### `def drop(n: Int): collection.IndexedSeq[T]`                             ###
...
### `def dropRight(n: Int): collection.IndexedSeq[T]`                        ###
...
Applies a function `f` to all elements of this paged sequence.
...
### `def grouped(size: Int): Iterator[collection.IndexedSeq[T]]`             ###
...
### `def sliding(size: Int): Iterator[collection.IndexedSeq[T]]`             ###
...
### `def sliding(size: Int, step: Int): Iterator[collection.IndexedSeq[T]]`  ###
...
### `def take(n: Int): collection.IndexedSeq[T]`                             ###
...
### `def takeRight(n: Int): collection.IndexedSeq[T]`                        ###
...
### `def takeWhile(p: (T) ⇒ Boolean): collection.IndexedSeq[T]`              ###
...
### `def zipAll[B, A1 >: T, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], (A1, B), That]): That` ###

[use case]

Returns a paged sequence formed from this paged sequence and another iterable
...
* thisElem
  * the element to be used to fill up the result if this paged sequence is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    paged sequence.
* returns
  * a new paged sequence containing pairs consisting of corresponding elements
    of this paged sequence and `that` . The length of the returned collection is
    the maximum of the lengths of this paged sequence and `that` . If this paged
    sequence is shorter than `that` , `thisElem` values are used to pad the
    result. If `that` is shorter than this paged sequence, `thatElem` values are
...
### `def zipWithIndex[A1 >: T, That](implicit bf: CanBuildFrom[collection.IndexedSeq[T], (A1, Int), That]): That` ###

[use case]

Zips this paged sequence with its indices.

* returns
  * A new paged sequence containing pairs consisting of all elements of this
    paged sequence paired with their index. Indices start at `0` .
...
### `def zip[A1 >: T, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], (A1, B), That]): That` ###
...
* returns
  * a new paged sequence containing pairs consisting of corresponding elements
    of this paged sequence and `that` . The length of the returned collection is
    the minimum of the lengths of this paged sequence and `that` .
...
### `def +:[B >: T, That](elem: B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

A copy of the paged sequence with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original paged sequence is not modified, so you will want to capture
the result.
...
* returns
  * a new paged sequence consisting of `elem` followed by all elements of this
    paged sequence.
...
A copy of this paged sequence with an element appended.
...
* returns
  * a new paged sequence consisting of all elements of this paged sequence
    followed by `elem` .
...
### `def combinations(n: Int): Iterator[collection.IndexedSeq[T]]`           ###
...
### `def diff[B >: T](that: GenSeq[B]): collection.IndexedSeq[T]`            ###

[use case]

Computes the multiset difference between this paged sequence and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new paged sequence which contains all elements of this paged sequence
...
### `def distinct: collection.IndexedSeq[T]`                                 ###
...
### `def intersect[B >: T](that: GenSeq[B]): collection.IndexedSeq[T]`       ###

[use case]

Computes the multiset intersection between this paged sequence and another
sequence.
...
### `def padTo[B >: T, That](len: Int, elem: B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

A copy of this paged sequence with an element value appended until a given
...
* returns
  * a new paged sequence consisting of all elements of this paged sequence
    ...
    resulting paged sequence has a length of at least `len` .
...
### `def patch[B >: T, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

Produces a new paged sequence where a slice of elements in this paged sequence
is replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original paged sequence
* returns
  * a new paged sequence consisting of all elements of this paged sequence
...
### `def permutations: Iterator[collection.IndexedSeq[T]]`                   ###
...
### `def reverse: collection.IndexedSeq[T]`                                  ###
...
### `def reverseMap[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this paged
...
* returns
  * a new paged sequence resulting from applying the given function `f` to each
    element of this paged sequence and collecting the results in reversed order.
...
### `def sortBy[B](f: (T) ⇒ B)(implicit ord: math.Ordering[B]): collection.IndexedSeq[T]` ###
...
### `def sortWith(lt: (T, T) ⇒ Boolean): collection.IndexedSeq[T]`           ###
...
### `def sorted[B >: T](implicit ord: math.Ordering[B]): collection.IndexedSeq[T]` ###
...
### `def union[B >: T, That](that: GenSeq[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this paged sequence and
...
### `def updated[B >: T, That](index: Int, elem: B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

A copy of this paged sequence with one single replaced element.
...
* returns
  * a copy of this paged sequence with the element at position `index` replaced
    by `elem` .
...
### `def view(from: Int, until: Int): SeqView[T, collection.IndexedSeq[T]]`  ###
...
### `def view: SeqView[T, collection.IndexedSeq[T]]`                         ###
...
### `def ++:[B >: T, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
### `def ++:[B >: T, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
### `def ++[B >: T, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

Returns a new paged sequence containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
paged sequence is the most specific superclass encompassing the element types of
the two operands.
...
### `def collect[B, That](pf: PartialFunction[T, B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
paged sequence on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the paged sequence.
* returns
  * a new paged sequence resulting from applying the given partial function
...
### `def dropWhile(p: (T) ⇒ Boolean): collection.IndexedSeq[T]`              ###
...
### `def filter(p: (T) ⇒ Boolean): collection.IndexedSeq[T]`                 ###
...
### `def filterNot(p: (T) ⇒ Boolean): collection.IndexedSeq[T]`              ###
...
### `def flatMap[B, That](f: (T) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
The type of the resulting collection is guided by the static type of paged
...
* f
  * the function to apply to each element.
* returns
  * a new paged sequence resulting from applying the given collection-valued
    function `f` to each element of this paged sequence and concatenating the
    results.
...
### `def groupBy[K](f: (T) ⇒ K): Map[K, collection.IndexedSeq[T]]`           ###
...
### `def init: collection.IndexedSeq[T]`                                     ###
...
### `def inits: Iterator[collection.IndexedSeq[T]]`                          ###
...
### `def map[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
* returns
  * a new paged sequence resulting from applying the given function `f` to each
    element of this paged sequence and collecting the results.
...
### `def partition(p: (T) ⇒ Boolean): (collection.IndexedSeq[T], collection.IndexedSeq[T])` ###
...
### `def repr: collection.IndexedSeq[T]`                                     ###
...
### `def scanLeft[B, That](z: B)(op: (B, T) ⇒ B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (T, B) ⇒ B)(implicit bf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
### `def scan[B >: T, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[collection.IndexedSeq[T], B, That]): That` ###
...
### `def span(p: (T) ⇒ Boolean): (collection.IndexedSeq[T], collection.IndexedSeq[T])` ###
...
### `def splitAt(n: Int): (collection.IndexedSeq[T], collection.IndexedSeq[T])` ###
...
### `def tail: collection.IndexedSeq[T]`                                     ###
...
### `def tails: Iterator[collection.IndexedSeq[T]]`                          ###
...
### `def withFilter(p: (T) ⇒ Boolean): FilterMonadic[T, collection.IndexedSeq[T]]` ###
...
Copies the elements of this paged sequence to an array. Fills the given array
 `xs` with values of this paged sequence. Copying will stop once either the end
of the current paged sequence is reached, or the end of the target array is
reached.
...
Copies the elements of this paged sequence to an array. Fills the given array
 `xs` with values of this paged sequence, beginning at index `start` . Copying
will stop once either the end of the current paged sequence is reached, or the
...
* returns
  * the first element of this paged sequence with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this paged sequence with the smallest value measured by
    function f.
...
Converts this paged sequence to a map. This method is unavailable unless the
...
### `def flatten[B](implicit asTraversable: (T) ⇒ GenTraversableOnce[B]): collection.IndexedSeq[B]` ###

[use case]

Converts this paged sequence of traversable collections into a paged sequence
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of paged
...
* returns
  * a new paged sequence resulting from concatenating all element paged
    sequences.
...
### `def genericBuilder[B]: Builder[B, collection.IndexedSeq[B]]`            ###
...
### `def newBuilder: Builder[T, collection.IndexedSeq[T]]`                   ###
...
### `def transpose[B](implicit asTraversable: (T) ⇒ GenTraversableOnce[B]): collection.IndexedSeq[collection.IndexedSeq[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (T) ⇒ (A1, A2, A3)): (collection.IndexedSeq[A1], collection.IndexedSeq[A2], collection.IndexedSeq[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (T) ⇒ (A1, A2)): (collection.IndexedSeq[A1], collection.IndexedSeq[A2])` ###
...
--------------------------------------------------------------------------------
         Instance Constructors From scala.collection.immutable.PagedSeq
--------------------------------------------------------------------------------


### `new PagedSeq(more: (Array[T], Int, Int) ⇒ Int)(implicit arg0: ClassTag[T])` ###
...
* returns
  * the element of this paged sequence at index `idx` , where `0` indicates the
    first element.

* Definition Classes
  * PagedSeq → SeqLike → GenSeqLike → Function1
...
Predicate method to check if an element is defined at position `index` of the
current sequence. Unlike `length` this operation does not force reading a lazy
sequence to the end.

* returns
  * `true` if this paged sequence contains an element at position `idx` ,
     `false` otherwise.

* Definition Classes
  * PagedSeq → GenSeqLike → PartialFunction
...
### `def slice(_start: Int, _end: Int): PagedSeq[T]`                         ###

The subsequence from index `start` up to `end -1` if `end` is lesser than the
length of the current sequence and up to length of the sequence otherwise. This
is limited up to the length of the current sequence if `end` is larger than its
length.

* returns
  * a paged sequence containing the elements greater than or equal to index
     `from` extending up to (but not including) index `until` of this paged
    sequence.

* Definition Classes
  * PagedSeq → IterableLike → TraversableLike → GenTraversableLike
...
* Implicit information
  * This member is added by an implicit conversion from PagedSeq [T] to
...
* Implicit information
  * This member is added by an implicit conversion from PagedSeq [T] to
    CollectionsHaveToParArray [PagedSeq [T], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (PagedSeq [T]) ⇒
