...
### `def toCollection(repr: SortedMap[A, B]): Iterable[(A, B)]`              ###
...
--------------------------------------------------------------------------------
             Concrete Value Members From scala.collection.SortedMap
--------------------------------------------------------------------------------
...
* Attributes
  * protected[this]
* Definition Classes
  * SortedMap → MapLike → GenericTraversableTemplate → TraversableLike →
    HasNewBuilder
...
### `def ++:[B >: (A, B), That](that: Traversable[B])(implicit bf: CanBuildFrom[SortedMap[A, B], B, That]): That` ###
