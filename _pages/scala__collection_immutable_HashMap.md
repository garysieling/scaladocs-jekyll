...
### `def sliding(size: Int): Iterator[HashMap[A, B]]`                        ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), HashMap[A, B]]`   ###
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[HashMap[A, B], (A1, Int), That]): That` ###
...
--------------------------------------------------------------------------------
         Instance Constructors From scala.collection.immutable.HashMap
--------------------------------------------------------------------------------
...
### `def filter0(p: ((A, B)) ⇒ Boolean, negate: Boolean, level: Int, buffer: Array[HashMap[A, B]], offset0: Int): HashMap[A, B]` ###
...
* Definition Classes
  * HashMap → IterableLike → GenericTraversableTemplate → TraversableLike →
...
### `def merge0[B1 >: B](that: HashMap[A, B1], level: Int, merger: Merger[A, B1]): HashMap[A, B1]` ###
...
### `def removed0(key: A, hash: Int, level: Int): HashMap[A, B]`             ###
...
* Implicit information
  * This member is added by an implicit conversion from HashMap [A, B] to
    CollectionsHaveToParArray [HashMap [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (HashMap [A, B]) ⇒
