...
### `type Filter = (Message[A] with Undoable) ⇒ Boolean`                     ###

* Definition Classes
  * Publisher


### `abstract type Pub <: ObservableBuffer[A]`                               ###

* Definition Classes
  * ObservableBuffer → Publisher
...
### `type Sub = Subscriber[Message[A] with Undoable, Pub]`                   ###
...
* returns
  * the index of the first element of this buffer that is equal (as determined
...
Finds index of first occurrence of some value in this buffer after or at some
start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this buffer that is equal (as
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this buffer that is equal (as determined by
...
Finds index of last occurrence of some value in this buffer before or at a given
end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this buffer that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this buffer to an array. Fills the given array `xs` with
at most `len` elements of this buffer, starting at position `start` . Copying
will stop once either the end of the current buffer is reached, or the end of
...
Returns a buffer formed from this buffer and another iterable collection by
...
* thisElem
  * the element to be used to fill up the result if this buffer is shorter than
     `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    buffer.
* returns
  * a new buffer containing pairs consisting of corresponding elements of this
    buffer and `that` . The length of the returned collection is the maximum of
    the lengths of this buffer and `that` . If this buffer is shorter than
     `that` , `thisElem` values are used to pad the result. If `that` is shorter
    than this buffer, `thatElem` values are used to pad the result.
...
* returns
  * A new buffer containing pairs consisting of all elements of this buffer
...
* returns
  * a new buffer containing pairs consisting of corresponding elements of this
    buffer and `that` . The length of the returned collection is the minimum of
    the lengths of this buffer and `that` .
...
A copy of the buffer with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original buffer is not modified, so you will want to capture the
result.
...
A copy of this buffer with an element appended.
...
* returns
  * a new buffer which contains all elements of this buffer except some of
...
* returns
  * a new buffer which contains all elements of this buffer which also appear in
...
A copy of this buffer with an element value appended until a given target length
is reached.
...
* elem
  * the padding value
* returns
  * a new buffer consisting of all elements of this buffer followed by the
    minimal number of occurrences of `elem` so that the resulting buffer has a
...
Produces a new buffer where a slice of elements in this buffer is replaced by
another sequence.
...
* returns
  * a new buffer consisting of all elements of this buffer except that
...
* returns
  * a new buffer resulting from applying the given function `f` to each element
    of this buffer and collecting the results in reversed order.
...
Produces a new sequence which contains all elements of this buffer and also all
...
* that
  * the sequence to add.
* returns
  * a new buffer which contains all elements of this buffer followed by all
    elements of `that` .
...
* returns
  * a copy of this buffer with the element at position `index` replaced by
     `elem` .
...
Returns a new buffer containing the elements from the left hand operand followed
by the elements from the right hand operand. The element type of the buffer is
...
buffer on which the function is defined.
...
* returns
  * a new buffer resulting from applying the given partial function `pf` to each
...
* f
  * the function to apply to each element.
* returns
  * a new buffer resulting from applying the given collection-valued function
     `f` to each element of this buffer and concatenating the results.
...
* returns
  * a new buffer resulting from applying the given function `f` to each element
    of this buffer and collecting the results.
...
Copies the elements of this buffer to an array. Fills the given array `xs` with
values of this buffer. Copying will stop once either the end of the current
buffer is reached, or the end of the target array is reached.
...
Copies the elements of this buffer to an array. Fills the given array `xs` with
values of this buffer, beginning at index `start` . Copying will stop once
either the end of the current buffer is reached, or the end of the target array
is reached.
...
* returns
  * the first element of this buffer with the largest value measured by function
    f.
...
* f
  * The measuring function.
* returns
  * the first element of this buffer with the smallest value measured by
    function f.
...
Converts this buffer to a map. This method is unavailable unless the elements
...
Converts this buffer of traversable collections into a buffer formed by the
elements of these traversable collections.

The resulting collection's type will be guided by the static type of buffer. For
example:
...
### `def +=(elem1: A, elem2: A, elems: A*): ObservableBuffer.this.type`      ###
...
### `def --=(xs: TraversableOnce[A]): ObservableBuffer.this.type`            ###
...
--------------------------------------------------------------------------------
        Abstract Value Members From scala.collection.mutable.BufferLike
--------------------------------------------------------------------------------
...
* returns
  * the element of this buffer at index `idx` , where `0` indicates the first
    element.

* Definition Classes
  * BufferLike → SeqLike → GenSeqLike
* Exceptions thrown
...
### `def -=(x: A): ObservableBuffer.this.type`                               ###
...
--------------------------------------------------------------------------------
     Concrete Value Members From scala.collection.mutable.ObservableBuffer
--------------------------------------------------------------------------------
...
(defined at scala.collection.mutable.ObservableBuffer)


### `def +=(element: A): ObservableBuffer.this.type`                         ###
...
* Definition Classes
  * ObservableBuffer → BufferLike → Growable
...
--------------------------------------------------------------------------------
         Concrete Value Members From scala.collection.mutable.Publisher
--------------------------------------------------------------------------------
...
### `def publish(event: Message[A] with Undoable): Unit`                     ###
...
### `def transform(f: (A) ⇒ A): ObservableBuffer.this.type`                  ###
...
* Implicit information
  * This member is added by an implicit conversion from ObservableBuffer [A] to
    CollectionsHaveToParArray [ObservableBuffer [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (ObservableBuffer [A]) ⇒
