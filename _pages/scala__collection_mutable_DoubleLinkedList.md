...
* returns
  * the index of the first element of this double linked list that is equal (as
...
Finds index of first occurrence of some value in this double linked list after
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this double linked list that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this double linked list that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this double linked list before
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this double linked list that is
    equal (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this double linked list to an array. Fills the given
array `xs` with at most `len` elements of this double linked list, starting at
position `start` . Copying will stop once either the end of the current double
linked list is reached, or the end of the target array is reached, or `len`
...
order as this double linked list.
...
### `def zipAll[B, A1 >: A, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], (A1, B), That]): That` ###

[use case]

Returns a double linked list formed from this double linked list and another
...
* thisElem
  * the element to be used to fill up the result if this double linked list is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    double linked list.
* returns
  * a new double linked list containing pairs consisting of corresponding
    elements of this double linked list and `that` . The length of the returned
    collection is the maximum of the lengths of this double linked list and
     `that` . If this double linked list is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this double
    linked list, `thatElem` values are used to pad the result.
...
### `def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[DoubleLinkedList[A], (A1, Int), That]): That` ###
...
* returns
  * A new double linked list containing pairs consisting of all elements of this
...
### `def zip[A1 >: A, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], (A1, B), That]): That` ###
...
* returns
  * a new double linked list containing pairs consisting of corresponding
    elements of this double linked list and `that` . The length of the returned
    collection is the minimum of the lengths of this double linked list and
     `that` .
...
### `def toCollection(repr: LinearSeq[A]): collection.LinearSeq[A]`          ###
...
### `def +:[B >: A, That](elem: B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###
...
Also, the original double linked list is not modified, so you will want to
capture the result.
...
* returns
  * a new double linked list consisting of `elem` followed by all elements of
    this double linked list.
...
* returns
  * a new double linked list consisting of all elements of this double linked
    list followed by `elem` .
...
### `def diff[B >: A](that: GenSeq[B]): DoubleLinkedList[A]`                 ###

[use case]

Computes the multiset difference between this double linked list and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new double linked list which contains all elements of this double linked
    list except some of occurrences of elements that also appear in `that` . If
...
### `def intersect[B >: A](that: GenSeq[B]): DoubleLinkedList[A]`            ###

[use case]

Computes the multiset intersection between this double linked list and another
sequence.
...
* returns
  * a new double linked list which contains all elements of this double linked
    list which also appear in `that` . If an element value `x` appears _n_ times
    in `that` , then the first _n_ occurrences of `x` will be retained in the
...
### `def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

A copy of this double linked list with an element value appended until a given
...
* returns
  * a new double linked list consisting of all elements of this double linked
    list followed by the minimal number of occurrences of `elem` so that the
    resulting double linked list has a length of at least `len` .
...
### `def patch[B >: A, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Produces a new double linked list where a slice of elements in this double
linked list is replaced by another sequence.
...
* returns
  * a new double linked list consisting of all elements of this double linked
    list except that `replaced` elements starting from `from` are replaced by
     `patch` .
...
### `def reverseMap[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this double
linked list and collecting the results in reversed order.
...
* returns
  * a new double linked list resulting from applying the given function `f` to
    each element of this double linked list and collecting the results in
    reversed order.
...
### `def sortBy[B](f: (A) ⇒ B)(implicit ord: math.Ordering[B]): DoubleLinkedList[A]` ###
...
### `def sorted[B >: A](implicit ord: math.Ordering[B]): DoubleLinkedList[A]` ###
...
### `def toCollection(repr: DoubleLinkedList[A]): collection.Seq[A]`         ###
...
### `def union[B >: A, That](that: GenSeq[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this double linked list
...
* returns
  * a new double linked list which contains all elements of this double linked
    list followed by all elements of `that` .
...
### `def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###
...
* returns
  * a copy of this double linked list with the element at position `index`
    replaced by `elem` .
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###
...
### `def ++:[B >: A, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###
...
### `def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Returns a new double linked list containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the double linked list is the most specific superclass encompassing the
...
### `def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
double linked list on which the function is defined.
...
* returns
  * a new double linked list resulting from applying the given partial function
...
### `def flatMap[B, That](f: (A) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this double
linked list and using the elements of the resulting collections.
...
The type of the resulting collection is guided by the static type of double
linked list. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new double linked list resulting from applying the given collection-valued
    function `f` to each element of this double linked list and concatenating
    the results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, DoubleLinkedList[A]]`      ###
...
### `def map[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[DoubleLinkedList[A], B, That]): That` ###
...
* returns
  * a new double linked list resulting from applying the given function `f` to
    each element of this double linked list and collecting the results.
...
Copies the elements of this double linked list to an array. Fills the given
array `xs` with values of this double linked list. Copying will stop once either
the end of the current double linked list is reached, or the end of the target
array is reached.
...
Copies the elements of this double linked list to an array. Fills the given
array `xs` with values of this double linked list, beginning at index `start` .
Copying will stop once either the end of the current double linked list is
...
* returns
  * the first element of this double linked list with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this double linked list with the smallest value
...
Converts this double linked list to a map. This method is unavailable unless the
...
* returns
  * a map of type `immutable.Map[T, U]` containing all key/value pairs of type
     `(T, U)` of this double linked list.
...
### `def flatten[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): DoubleLinkedList[B]` ###

[use case]

Converts this double linked list of traversable collections into a double linked
list formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of double
...
* returns
  * a new double linked list resulting from concatenating all element double
    linked lists.
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): DoubleLinkedList[DoubleLinkedList[B]]` ###
...
--------------------------------------------------------------------------------
      Instance Constructors From scala.collection.mutable.DoubleLinkedList
--------------------------------------------------------------------------------
...
(defined at scala.collection.mutable.DoubleLinkedList)


--------------------------------------------------------------------------------
          Value Members From scala.collection.mutable.DoubleLinkedList
--------------------------------------------------------------------------------
...
* Definition Classes
  * DoubleLinkedList → LinkedListLike → Cloneable → AnyRef
...
(or its `Iterable` superclass where class `DoubleLinkedList` is not a `Seq` .)

* Definition Classes
  * DoubleLinkedList → LinearSeq → LinearSeq → Seq → Iterable → Traversable →
...
--------------------------------------------------------------------------------
        Value Members From scala.collection.mutable.DoubleLinkedListLike
--------------------------------------------------------------------------------
...
(defined at scala.collection.mutable.DoubleLinkedListLike)
...
* returns
  * the element of this double linked list at index `idx` , where `0` indicates
    the first element.
...
* returns
  * a double linked list consisting of all elements of this double linked list
    except the first `n` ones, or else the empty double linked list, if this
    double linked list has less than `n` elements.

* Definition Classes
  * DoubleLinkedListLike → LinkedListLike → IterableLike → TraversableLike →
    GenTraversableLike
...
--------------------------------------------------------------------------------
           Value Members From scala.collection.mutable.LinkedListLike
--------------------------------------------------------------------------------
...
* Implicit information
  * This member is added by an implicit conversion from DoubleLinkedList [A] to
    CollectionsHaveToParArray [DoubleLinkedList [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (DoubleLinkedList [A]) ⇒
