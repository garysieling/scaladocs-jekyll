...
* Annotations
  * @ deprecated
* Deprecated
  * _(Since version 2.12.0)_ This method will be made private
...
If the specified key is not already in the map, computes its value using the
given thunk `op` and enters it into the map.

Since concurrent maps cannot contain `null` for keys or values, a
 `NullPointerException` is thrown if the thunk `op` returns `null` .

If the specified mapping function throws an exception, that exception is
rethrown.

Note: This method will invoke op at most once. However, `op` may be invoked
without the result being added to the map if a concurrent process is also trying
to add a value corresponding to the same key `k` .
...
accessed, it is rewritten. The work of creating the snapshot is thus distributed
across subsequent updates and accesses on this TrieMap by all threads. Note that
the snapshot itself is never rewritten unlike when calling the `snapshot`
method, but the obtained snapshot cannot be modified.
...
* Definition Classes
  * TrieMap → Map → Map → GenMap → GenMapLike → Iterable → Iterable →
    GenIterable → Traversable → Traversable → GenTraversable → Parallelizable →
    TraversableOnce → GenTraversableOnce
...
The snapshot is lazily updated - the first time some branch in the snapshot or
this TrieMap are accessed, they are rewritten. This means that the work of
...
* Implicit information
  * This member is added by an implicit conversion from TrieMap [K, V] to
    CollectionsHaveToParArray [TrieMap [K, V], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (TrieMap [K, V]) ⇒
