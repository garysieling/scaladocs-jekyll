...
### `class DefaultKeySortedSet extends SortedMap.DefaultKeySortedSet with SortedSet[A]` ###
...
Copies the elements of this immutable tree map to an array. Fills the given
array `xs` with at most `len` elements of this immutable tree map, starting at
position `start` . Copying will stop once either the end of the current
immutable tree map is reached, or the end of the target array is reached, or
...
### `def grouped(size: Int): Iterator[TreeMap[A, B]]`                        ###
...
order as this immutable tree map.
...
### `def sliding(size: Int): Iterator[TreeMap[A, B]]`                        ###
...
### `def sliding(size: Int, step: Int): Iterator[TreeMap[A, B]]`             ###
...
### `def toCollection(repr: TreeMap[A, B]): collection.Iterable[(A, B)]`     ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), TreeMap[A, B]]`   ###
...
### `def view: IterableView[(A, B), TreeMap[A, B]]`                          ###
...
### `def zipAll[B, A1 >: (A, B), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[TreeMap[A, B], (A1, B), That]): That` ###

[use case]

Returns a immutable tree map formed from this immutable tree map and another
...
* thisElem
  * the element to be used to fill up the result if this immutable tree map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable tree map.
* returns
  * a new immutable tree map containing pairs consisting of corresponding
    elements of this immutable tree map and `that` . The length of the returned
    collection is the maximum of the lengths of this immutable tree map and
     `that` . If this immutable tree map is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this immutable
    tree map, `thatElem` values are used to pad the result.
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[TreeMap[A, B], (A1, Int), That]): That` ###
...
### `def zip[A1 >: (A, B), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[TreeMap[A, B], (A1, B), That]): That` ###
...
* returns
  * a new immutable tree map containing pairs consisting of corresponding
    elements of this immutable tree map and `that` . The length of the returned
    collection is the minimum of the lengths of this immutable tree map and
     `that` .
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): TreeMap[A, B]`                    ###
...
### `def ++:[B >: (A, B), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
### `def ++:[B >: (A, B), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
* that
  * the traversable to append.
* returns
  * a new immutable tree map which contains all elements of this immutable tree
...
### `def ++[B >: (A, B), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###

[use case]

Returns a new immutable tree map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the immutable tree map is the most specific superclass encompassing the
...
### `def collect[B, That](pf: PartialFunction[(A, B), B])(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
immutable tree map on which the function is defined.
...
### `def filter(p: ((A, B)) ⇒ Boolean): TreeMap[A, B]`                       ###
...
### `def flatMap[B, That](f: ((A, B)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this immutable
tree map and using the elements of the resulting collections.
...
tree map. This might cause unexpected results sometimes. For example:
...
* returns
  * a new immutable tree map resulting from applying the given collection-valued
    function `f` to each element of this immutable tree map and concatenating
    the results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): Map[K, TreeMap[A, B]]`                 ###
...
### `def inits: Iterator[TreeMap[A, B]]`                                     ###
...
### `def map[B, That](f: ((A, B)) ⇒ B)(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new immutable tree map resulting from applying the given function `f` to
    each element of this immutable tree map and collecting the results.
...
### `def partition(p: ((A, B)) ⇒ Boolean): (TreeMap[A, B], TreeMap[A, B])`   ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[TreeMap[A, B], B, That]): That` ###
...
### `def tails: Iterator[TreeMap[A, B]]`                                     ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), TreeMap[A, B]]` ###
...
Copies the elements of this immutable tree map to an array. Fills the given
array `xs` with values of this immutable tree map. Copying will stop once either
the end of the current immutable tree map is reached, or the end of the target
array is reached.
...
Copies the elements of this immutable tree map to an array. Fills the given
array `xs` with values of this immutable tree map, beginning at index `start` .
Copying will stop once either the end of the current immutable tree map is
...
* returns
  * the first element of this immutable tree map with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this immutable tree map with the smallest value
...
Converts this immutable tree map of traversable collections into a immutable
tree map formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of immutable
tree map. For example:
...
* returns
  * a new immutable tree map resulting from concatenating all element immutable
    tree maps.
...
### `def -(elem1: A, elem2: A, elems: A*): TreeMap[A, B]`                    ###
...
### `def --(xs: GenTraversableOnce[A]): TreeMap[A, B]`                       ###
...
### `def transform[C, That](f: (A, B) ⇒ C)(implicit bf: CanBuildFrom[TreeMap[A, B], (A, C), That]): That` ###
...
### `def filterKeys(p: (A) ⇒ Boolean): SortedMap[A, B]`                      ###
...
* Definition Classes
  * SortedMap → SortedMapLike → Sorted → MapLike → MapLike → GenMapLike
...
--------------------------------------------------------------------------------
         Instance Constructors From scala.collection.immutable.TreeMap
--------------------------------------------------------------------------------


### `new TreeMap()(implicit ordering: Ordering[A])`                          ###
...
### `def ++[B1 >: B](xs: GenTraversableOnce[(A, B1)]): TreeMap[A, B1]`       ###
...
* Definition Classes
  * TreeMap → SortedMap → SortedMapLike → MapLike → MapLike

(defined at scala.collection.immutable.TreeMap)


### `def +[B1 >: B](kv: (A, B1)): TreeMap[A, B1]`                            ###
...
* Definition Classes
  * TreeMap → SortedMap → SortedMapLike → Map → MapLike → MapLike → GenMapLike

(defined at scala.collection.immutable.TreeMap)


### `def +[B1 >: B](elem1: (A, B1), elem2: (A, B1), elems: (A, B1)*): TreeMap[A, B1]` ###
...
* Definition Classes
  * TreeMap → MapLike → Subtractable → GenMapLike
...
* returns
  * a immutable tree map consisting of all elements of this immutable tree map
    except the first `n` ones, or else the empty immutable tree map, if this
    immutable tree map has less than `n` elements.

* Definition Classes
  * TreeMap → IterableLike → TraversableLike → GenTraversableLike

(defined at scala.collection.immutable.TreeMap)


### `def dropRight(n: Int): TreeMap[A, B]`                                   ###
...
* returns
  * a immutable tree map consisting of all elements of this immutable tree map
    except the last `n` ones, or else the empty immutable tree map, if this
    immutable tree map has less than `n` elements.

* Definition Classes
  * TreeMap → IterableLike

(defined at scala.collection.immutable.TreeMap)


### `def dropWhile(p: ((A, B)) ⇒ Boolean): TreeMap[A, B]`                    ###

Drops longest prefix of elements that satisfy a predicate.

* returns
  * the longest suffix of this immutable tree map whose first element does not
...
* Definition Classes
  * TreeMap → SortedMap → SortedMap → Map → Map → MapLike
...
* Definition Classes
  * TreeMap → IterableLike → GenericTraversableTemplate → TraversableLike →
    GenTraversableLike → TraversableOnce → GenTraversableOnce → FilterMonadic

(defined at scala.collection.immutable.TreeMap)


### `def from(from: A): TreeMap[A, B]`                                       ###
...
* Definition Classes
  * TreeMap → TraversableLike → GenTraversableLike
* Exceptions thrown
  * UnsupportedOperationException if the immutable tree map is empty.
...
A new TreeMap with the entry added is returned, assuming that key is *not* in
the TreeMap.
...
* returns
  * a new immutable tree map with the inserted binding, if it wasn't present in
    the map
...
* Definition Classes
  * TreeMap → MapLike → PartialFunction → GenMapLike
...
### `def newBuilder: Builder[(A, B), TreeMap[A, B]]`                         ###
...
* Attributes
  * protected[this]
* Definition Classes
  * TreeMap → SortedMap → SortedMap → MapLike → GenericTraversableTemplate →
    TraversableLike → HasNewBuilder

(defined at scala.collection.immutable.TreeMap)


### `def range(from: A, until: A): TreeMap[A, B]`                            ###
...
### `def slice(from: Int, until: Int): TreeMap[A, B]`                        ###
...
* returns
  * a immutable tree map containing the elements greater than or equal to index
     `from` extending up to (but not including) index `until` of this immutable
    tree map.
...
### `def span(p: ((A, B)) ⇒ Boolean): (TreeMap[A, B], TreeMap[A, B])`        ###

Splits this immutable tree map into a prefix/suffix pair according to a
predicate.
...
### `def splitAt(n: Int): (TreeMap[A, B], TreeMap[A, B])`                    ###

Splits this immutable tree map into two at a given position. Note:
...
* returns
  * a pair of immutable tree maps consisting of the first `n` elements of this
    immutable tree map, and the other elements.
...
* returns
  * a immutable tree map consisting only of the first `n` elements of this
    immutable tree map, or else the whole immutable tree map, if it has less
    than `n` elements.
...
### `def takeRight(n: Int): TreeMap[A, B]`                                   ###
...
* n
  * the number of elements to take
* returns
  * a immutable tree map consisting only of the last `n` elements of this
    immutable tree map, or else the whole immutable tree map, if it has less
    than `n` elements.
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): TreeMap[A, B]`                    ###

Takes longest prefix of elements that satisfy a predicate.

* returns
  * the longest prefix of this immutable tree map whose elements all satisfy the
    predicate `p` .
...
A new TreeMap with the entry added is returned, if key is *not* in the TreeMap,
otherwise the key is updated with the new entry.
...
* Definition Classes
  * TreeMap → SortedMap → SortedMapLike → Map → MapLike → MapLike → GenMap
...
* Implicit information
  * This member is added by an implicit conversion from TreeMap [A, B] to
    CollectionsHaveToParArray [TreeMap [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (TreeMap [A, B]) ⇒
