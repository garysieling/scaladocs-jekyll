...
Finds index of first occurrence of some value in this buffer proxy.

* elem
  * the element value to search for.
* returns
  * the index of the first element of this buffer proxy that is equal (as
...
Finds index of first occurrence of some value in this buffer proxy after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this buffer proxy that is equal
...
Finds index of last occurrence of some value in this buffer proxy.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this buffer proxy that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this buffer proxy before or at a
given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this buffer proxy that is equal
    (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this buffer proxy to an array. Fills the given array
 `xs` with at most `len` elements of this buffer proxy, starting at position
 `start` . Copying will stop once either the end of the current buffer proxy is
...
Applies a function `f` to all elements of this buffer proxy.
...
Returns a buffer proxy formed from this buffer proxy and another iterable
...
* thisElem
  * the element to be used to fill up the result if this buffer proxy is shorter
    than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    buffer proxy.
* returns
  * a new buffer proxy containing pairs consisting of corresponding elements of
    this buffer proxy and `that` . The length of the returned collection is the
    maximum of the lengths of this buffer proxy and `that` . If this buffer
    proxy is shorter than `that` , `thisElem` values are used to pad the result.
    If `that` is shorter than this buffer proxy, `thatElem` values are used to
    pad the result.
...
Zips this buffer proxy with its indices.

* returns
  * A new buffer proxy containing pairs consisting of all elements of this
    buffer proxy paired with their index. Indices start at `0` .
...
* returns
  * a new buffer proxy containing pairs consisting of corresponding elements of
    this buffer proxy and `that` . The length of the returned collection is the
    minimum of the lengths of this buffer proxy and `that` .
...
A copy of the buffer proxy with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original buffer proxy is not modified, so you will want to capture the
result.
...
* returns
  * a new buffer proxy consisting of `elem` followed by all elements of this
    buffer proxy.
...
A copy of this buffer proxy with an element appended.
...
* returns
  * a new buffer proxy consisting of all elements of this buffer proxy followed
    by `elem` .
...
Computes the multiset difference between this buffer proxy and another sequence.

* that
  * the sequence of elements to remove
* returns
  * a new buffer proxy which contains all elements of this buffer proxy except
...
Computes the multiset intersection between this buffer proxy and another
sequence.

* that
  * the sequence of elements to intersect with.
* returns
  * a new buffer proxy which contains all elements of this buffer proxy which
...
A copy of this buffer proxy with an element value appended until a given target
length is reached.
...
* returns
  * a new buffer proxy consisting of all elements of this buffer proxy followed
    by the minimal number of occurrences of `elem` so that the resulting buffer
    proxy has a length of at least `len` .
...
Produces a new buffer proxy where a slice of elements in this buffer proxy is
replaced by another sequence.

* from
  * the index of the first replaced element
* replaced
  * the number of elements to drop in the original buffer proxy
* returns
  * a new buffer proxy consisting of all elements of this buffer proxy except
...
proxy and collecting the results in reversed order.
...
* returns
  * a new buffer proxy resulting from applying the given function `f` to each
    element of this buffer proxy and collecting the results in reversed order.
...
Produces a new sequence which contains all elements of this buffer proxy and
...
* that
  * the sequence to add.
* returns
  * a new buffer proxy which contains all elements of this buffer proxy followed
...
A copy of this buffer proxy with one single replaced element.
...
* returns
  * a copy of this buffer proxy with the element at position `index` replaced by
     `elem` .
...
Returns a new buffer proxy containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
buffer proxy is the most specific superclass encompassing the element types of
the two operands.
...
buffer proxy on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the buffer proxy.
* returns
  * a new buffer proxy resulting from applying the given partial function `pf`
...
Builds a new collection by applying a function to all elements of this buffer
proxy and using the elements of the resulting collections.
...
The type of the resulting collection is guided by the static type of buffer
proxy. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new buffer proxy resulting from applying the given collection-valued
    function `f` to each element of this buffer proxy and concatenating the
    results.
...
* returns
  * a new buffer proxy resulting from applying the given function `f` to each
    element of this buffer proxy and collecting the results.
...
Copies the elements of this buffer proxy to an array. Fills the given array
 `xs` with values of this buffer proxy. Copying will stop once either the end of
the current buffer proxy is reached, or the end of the target array is reached.
...
Copies the elements of this buffer proxy to an array. Fills the given array
 `xs` with values of this buffer proxy, beginning at index `start` . Copying
will stop once either the end of the current buffer proxy is reached, or the end
...
* returns
  * the first element of this buffer proxy with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this buffer proxy with the smallest value measured by
    function f.
...
Converts this buffer proxy to a map. This method is unavailable unless the
...
Converts this buffer proxy of traversable collections into a buffer proxy formed
by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of buffer
proxy. For example:
...
* returns
  * a new buffer proxy resulting from concatenating all element buffer proxys.
...
### `def +=(elem1: A, elem2: A, elems: A*): BufferProxy.this.type`           ###
...
### `def --=(xs: TraversableOnce[A]): BufferProxy.this.type`                 ###
...
--------------------------------------------------------------------------------
          Concrete Value Members From scala.collection.mutable.Buffer
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
        Concrete Value Members From scala.collection.mutable.BufferLike
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
        Abstract Value Members From scala.collection.mutable.BufferProxy
--------------------------------------------------------------------------------
...
(defined at scala.collection.mutable.BufferProxy)


--------------------------------------------------------------------------------
        Concrete Value Members From scala.collection.mutable.BufferProxy
--------------------------------------------------------------------------------
...
* xs
  * the traversable object.
* returns
  * a reference to this buffer proxy.
...
Selects an element by its index in the buffer proxy.
...
* returns
  * the element of this buffer proxy at index `idx` , where `0` indicates the
    first element.

* Definition Classes
  * BufferProxy → BufferLike → SeqLike → GenSeqLike → Function1
* Exceptions thrown
...
* Definition Classes
  * BufferProxy → BufferLike → Cloneable → AnyRef
...
### `def insertAll(n: Int, iter: collection.Iterable[A]): Unit`              ###
...
### `def insertAll(n: Int, iter: collection.Traversable[A]): Unit`           ###
...
--------------------------------------------------------------------------------
       Deprecated Value Members From scala.collection.mutable.BufferProxy
--------------------------------------------------------------------------------
...
### `def transform(f: (A) ⇒ A): BufferProxy.this.type`                       ###
...
* Implicit information
  * This member is added by an implicit conversion from BufferProxy [A] to
    CollectionsHaveToParArray [BufferProxy [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (BufferProxy [A]) ⇒
