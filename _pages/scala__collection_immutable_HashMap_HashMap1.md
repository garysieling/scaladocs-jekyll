...
### `def parCombiner: Combiner[(A, B), ParHashMap[A, B]]`                    ###
...
Copies the elements of this immutable hash map to an array. Fills the given
array `xs` with at most `len` elements of this immutable hash map, starting at
position `start` . Copying will stop once either the end of the current
immutable hash map is reached, or the end of the target array is reached, or
...
### `def grouped(size: scala.Int): Iterator[HashMap[A, B]]`                  ###
...
order as this immutable hash map.
...
### `def slice(from: scala.Int, until: scala.Int): HashMap[A, B]`            ###
...
### `def sliding(size: scala.Int, step: scala.Int): Iterator[HashMap[A, B]]` ###
...
### `def takeRight(n: scala.Int): HashMap[A, B]`                             ###
...
### `def toCollection(repr: HashMap[A, B]): collection.Iterable[(A, B)]`     ###
...
### `def view(from: scala.Int, until: scala.Int): IterableView[(A, B), HashMap[A, B]]` ###
...
Returns a immutable hash map formed from this immutable hash map and another
...
* thisElem
  * the element to be used to fill up the result if this immutable hash map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable hash map.
* returns
  * a new immutable hash map containing pairs consisting of corresponding
    elements of this immutable hash map and `that` . The length of the returned
    collection is the maximum of the lengths of this immutable hash map and
     `that` . If this immutable hash map is shorter than `that` , `thisElem`
...
* returns
  * A new immutable hash map containing pairs consisting of all elements of this
    immutable hash map paired with their index. Indices start at `0` .
...
* returns
  * a new immutable hash map containing pairs consisting of corresponding
    elements of this immutable hash map and `that` . The length of the returned
    collection is the minimum of the lengths of this immutable hash map and
     `that` .
...
### `def newBuilder: Builder[(A, B), HashMap[A, B]]`                         ###
...
* that
  * the traversable to append.
* returns
  * a new immutable hash map which contains all elements of this immutable hash
...
Returns a new immutable hash map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the immutable hash map is the most specific superclass encompassing the
...
immutable hash map on which the function is defined.
...
* returns
  * a new immutable hash map resulting from applying the given partial function
...
* returns
  * a new immutable hash map resulting from applying the given collection-valued
    function `f` to each element of this immutable hash map and concatenating
    the results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): Map[K, HashMap[A, B]]`                 ###
...
* f
  * the function to apply to each element.
* returns
  * a new immutable hash map resulting from applying the given function `f` to
    each element of this immutable hash map and collecting the results.
...
### `def splitAt(n: scala.Int): (HashMap[A, B], HashMap[A, B])`              ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), HashMap[A, B]]` ###
...
Copies the elements of this immutable hash map to an array. Fills the given
array `xs` with values of this immutable hash map. Copying will stop once either
the end of the current immutable hash map is reached, or the end of the target
array is reached.
...
Copies the elements of this immutable hash map to an array. Fills the given
array `xs` with values of this immutable hash map, beginning at index `start` .
Copying will stop once either the end of the current immutable hash map is
...
* returns
  * the first element of this immutable hash map with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this immutable hash map with the smallest value
...
Converts this immutable hash map of traversable collections into a immutable
hash map formed by the elements of these traversable collections.
...
* returns
  * a new immutable hash map resulting from concatenating all element immutable
    hash maps.
...
* Definition Classes
  * HashMap → Map → MapLike → MapLike → GenMapLike
...
* Definition Classes
  * HashMap → MapLike → Subtractable → GenMapLike
...
* p
  * the predicate used to test elements.
* returns
  * a new immutable hash map consisting of all elements of this immutable hash
    map that satisfy the given predicate `p` . The order of the elements is
    preserved.

* Definition Classes
  * HashMap → TraversableLike → GenTraversableLike
...
* Definition Classes
  * HashMap → MapLike → TraversableLike → GenTraversableLike
...
### `final def improve(hcode: scala.Int): scala.Int`                         ###
...
Uses the specified collision resolution function if two keys are the same. The
collision resolution function will always take the first argument from `this`
hash map and the second from `that` .

The `merged` method is on average more performant than doing a traversal and
reconstructing a new immutable hash map from scratch, or `++` .
...
* Definition Classes
  * HashMap → Map → MapLike → MapLike → GenMap

(defined at scala.collection.immutable.HashMap)


--------------------------------------------------------------------------------
     Instance Constructors From scala.collection.immutable.HashMap.HashMap1
--------------------------------------------------------------------------------


### `new HashMap1(key: A, hash: HashMap.Int, value: B, kv: (A, B))`          ###

(defined at scala.collection.immutable.HashMap.HashMap1)


--------------------------------------------------------------------------------
         Value Members From scala.collection.immutable.HashMap.HashMap1
--------------------------------------------------------------------------------
...
* Definition Classes
  * HashMap1 → HashMap → IterableLike → GenericTraversableTemplate →
...
### `def get0(key: A, hash: HashMap.Int, level: HashMap.Int): Option[B]`     ###
...
### `def removed0(key: A, hash: HashMap.Int, level: HashMap.Int): HashMap[A, B]` ###
...
* Definition Classes
  * HashMap1 → HashMap → GenTraversableLike → TraversableOnce →
    GenTraversableOnce
...
### `def transform[C, That](f: (A, B) ⇒ C)(implicit bf: CanBuildFrom[HashMap[A, B], (A, C), That]): That` ###
...
* Implicit information
  * This member is added by an implicit conversion from HashMap1 [A, B] to
    CollectionsHaveToParArray [HashMap1 [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (HashMap1 [A, B]) ⇒
