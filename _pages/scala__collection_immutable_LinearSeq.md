...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[LinearSeq[A], B, That]): That` ###
...
### `def scanLeft[B, That](z: B)(op: (B, A) ⇒ B)(implicit bf: CanBuildFrom[LinearSeq[A], B, That]): That` ###
...
### `def flatten[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): LinearSeq[B]` ###
...
* Definition Classes
  * LinearSeq → LinearSeq → Seq → Seq → GenSeq → Iterable → Iterable →
