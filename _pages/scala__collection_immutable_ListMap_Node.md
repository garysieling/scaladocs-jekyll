...
```scala
class Node[B1 >: B] extends ListMap[A, B1] with Serializable
```
...
### `def andThen[C](k: (B1) ⇒ C): PartialFunction[A, C]`                     ###
...
### `def applyOrElse[A1 <: A, B1 >: B1](x: A1, default: (A1) ⇒ B1): B1`      ###
...
### `def orElse[A1 <: A, B1 >: B1](that: PartialFunction[A1, B1]): PartialFunction[A1, B1]` ###
...
### `def runWith[U](action: (B1) ⇒ U): (A) ⇒ Boolean`                        ###
...
### `def copyToArray[B >: (A, B1)](xs: Array[B], start: Int, len: Int): Unit` ###

[use case]

Copies the elements of this immutable list map to an array. Fills the given
array `xs` with at most `len` elements of this immutable list map, starting at
position `start` . Copying will stop once either the end of the current
immutable list map is reached, or the end of the target array is reached, or
...
### `def dropRight(n: Int): ListMap[A, B1]`                                  ###
...
### `def exists(p: ((A, B1)) ⇒ Boolean): Boolean`                            ###
...
### `def find(p: ((A, B1)) ⇒ Boolean): Option[(A, B1)]`                      ###
...
### `def foldRight[B](z: B)(op: ((A, B1), B) ⇒ B): B`                        ###
...
### `def forall(p: ((A, B1)) ⇒ Boolean): Boolean`                            ###
...
### `def foreach[U](f: ((A, B1)) ⇒ U): Unit`                                 ###

[use case]

Applies a function `f` to all elements of this immutable list map.
...
### `def grouped(size: Int): Iterator[ListMap[A, B1]]`                       ###
...
### `def reduceRight[B >: (A, B1)](op: ((A, B1), B) ⇒ B): B`                 ###
...
### `def sameElements[B >: (A, B1)](that: GenIterable[B]): Boolean`          ###

[use case]

Checks if the other iterable collection contains the same elements in the same
order as this immutable list map.
...
### `def slice(from: Int, until: Int): ListMap[A, B1]`                       ###
...
### `def sliding(size: Int): Iterator[ListMap[A, B1]]`                       ###
...
### `def sliding(size: Int, step: Int): Iterator[ListMap[A, B1]]`            ###
...
### `def takeRight(n: Int): ListMap[A, B1]`                                  ###
...
### `def takeWhile(p: ((A, B1)) ⇒ Boolean): ListMap[A, B1]`                  ###
...
### `def thisCollection: collection.Iterable[(A, B1)]`                       ###
...
### `def toCollection(repr: ListMap[A, B1]): collection.Iterable[(A, B1)]`   ###
...
### `def toIterable: collection.Iterable[(A, B1)]`                           ###
...
### `def view(from: Int, until: Int): IterableView[(A, B1), ListMap[A, B1]]` ###
...
### `def view: IterableView[(A, B1), ListMap[A, B1]]`                        ###
...
### `def zipAll[B, A1 >: (A, B1), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[ListMap[A, B1], (A1, B), That]): That` ###

[use case]

Returns a immutable list map formed from this immutable list map and another
...
* thisElem
  * the element to be used to fill up the result if this immutable list map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable list map.
* returns
  * a new immutable list map containing pairs consisting of corresponding
    elements of this immutable list map and `that` . The length of the returned
    collection is the maximum of the lengths of this immutable list map and
     `that` . If this immutable list map is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this immutable
    list map, `thatElem` values are used to pad the result.
...
### `def zipWithIndex[A1 >: (A, B1), That](implicit bf: CanBuildFrom[ListMap[A, B1], (A1, Int), That]): That` ###

[use case]

Zips this immutable list map with its indices.

Note: might return different results for different runs, unless the underlying
collection type is ordered.

* returns
  * A new immutable list map containing pairs consisting of all elements of this
    immutable list map paired with their index. Indices start at `0` .
...
### `def zip[A1 >: (A, B1), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[ListMap[A, B1], (A1, B), That]): That` ###
...
* returns
  * a new immutable list map containing pairs consisting of corresponding
    elements of this immutable list map and `that` . The length of the returned
    collection is the minimum of the lengths of this immutable list map and
     `that` .
...
### `def filterNot(p: ((A, B1)) ⇒ Boolean): ListMap[A, B1]`                  ###
...
### `def getOrElse[B1 >: B1](key: A, default: ⇒ B1): B1`                     ###
...
### `def newBuilder: Builder[(A, B1), ListMap[A, B1]]`                       ###
...
### `def values: collection.Iterable[B1]`                                    ###
...
### `def valuesIterator: Iterator[B1]`                                       ###
...
### `def ++:[B >: (A, B1), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
### `def ++:[B >: (A, B1), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
* that
  * the traversable to append.
* returns
  * a new immutable list map which contains all elements of this immutable list
...
### `def ++[B >: (A, B1), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###

[use case]

Returns a new immutable list map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the immutable list map is the most specific superclass encompassing the
...
### `def collect[B, That](pf: PartialFunction[(A, B1), B])(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
immutable list map on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the immutable list map.
* returns
  * a new immutable list map resulting from applying the given partial function
...
### `def dropWhile(p: ((A, B1)) ⇒ Boolean): ListMap[A, B1]`                  ###
...
### `def filter(p: ((A, B1)) ⇒ Boolean): ListMap[A, B1]`                     ###
...
### `def flatMap[B, That](f: ((A, B1)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this immutable
list map and using the elements of the resulting collections.
...
list map. This might cause unexpected results sometimes. For example:
...
* returns
  * a new immutable list map resulting from applying the given collection-valued
    function `f` to each element of this immutable list map and concatenating
    the results.
...
### `def groupBy[K](f: ((A, B1)) ⇒ K): Map[K, ListMap[A, B1]]`               ###
...
### `def inits: Iterator[ListMap[A, B1]]`                                    ###
...
### `def map[B, That](f: ((A, B1)) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new immutable list map resulting from applying the given function `f` to
    each element of this immutable list map and collecting the results.
...
### `def partition(p: ((A, B1)) ⇒ Boolean): (ListMap[A, B1], ListMap[A, B1])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B1)) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B1), B) ⇒ B)(implicit bf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
### `def scan[B >: (A, B1), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[ListMap[A, B1], B, That]): That` ###
...
### `def span(p: ((A, B1)) ⇒ Boolean): (ListMap[A, B1], ListMap[A, B1])`     ###
...
### `def splitAt(n: Int): (ListMap[A, B1], ListMap[A, B1])`                  ###
...
### `def tails: Iterator[ListMap[A, B1]]`                                    ###
...
### `def toTraversable: collection.Traversable[(A, B1)]`                     ###
...
### `def withFilter(p: ((A, B1)) ⇒ Boolean): FilterMonadic[(A, B1), ListMap[A, B1]]` ###
...
### `def aggregate[B](z: ⇒ B)(seqop: (B, (A, B1)) ⇒ B, combop: (B, B) ⇒ B): B` ###
...
### `def collectFirst[B](pf: PartialFunction[(A, B1), B]): Option[B]`        ###
...
### `def copyToArray[B >: (A, B1)](xs: Array[B]): Unit`                      ###

[use case]

Copies the elements of this immutable list map to an array. Fills the given
array `xs` with values of this immutable list map. Copying will stop once either
the end of the current immutable list map is reached, or the end of the target
array is reached.
...
### `def copyToArray[B >: (A, B1)](xs: Array[B], start: Int): Unit`          ###

[use case]

Copies the elements of this immutable list map to an array. Fills the given
array `xs` with values of this immutable list map, beginning at index `start` .
Copying will stop once either the end of the current immutable list map is
...
### `def copyToBuffer[B >: (A, B1)](dest: Buffer[B]): Unit`                  ###
...
### `def foldLeft[B](z: B)(op: (B, (A, B1)) ⇒ B): B`                         ###
...
### `def fold[A1 >: (A, B1)](z: A1)(op: (A1, A1) ⇒ A1): A1`                  ###
...
### `def maxBy[B](f: ((A, B1)) ⇒ B)(implicit cmp: Ordering[B]): (A, B1)`     ###
...
* returns
  * the first element of this immutable list map with the largest value measured
    by function f.
...
### `def minBy[B](f: ((A, B1)) ⇒ B)(implicit cmp: Ordering[B]): (A, B1)`     ###
...
* f
  * The measuring function.
* returns
  * the first element of this immutable list map with the smallest value
...
### `def reduceLeftOption[B >: (A, B1)](op: (B, (A, B1)) ⇒ B): Option[B]`    ###
...
### `def reduceLeft[B >: (A, B1)](op: (B, (A, B1)) ⇒ B): B`                  ###
...
### `def reduceOption[A1 >: (A, B1)](op: (A1, A1) ⇒ A1): Option[A1]`         ###
...
### `def reduceRightOption[B >: (A, B1)](op: ((A, B1), B) ⇒ B): Option[B]`   ###
...
### `def reduce[A1 >: (A, B1)](op: (A1, A1) ⇒ A1): A1`                       ###
...
### `def toIndexedSeq: IndexedSeq[(A, B1)]`                                  ###
...
### `def toVector: scala.Vector[(A, B1)]`                                    ###
...
### `def flatten[B](implicit asTraversable: ((A, B1)) ⇒ GenTraversableOnce[B]): Iterable[B]` ###

[use case]

Converts this immutable list map of traversable collections into a immutable
list map formed by the elements of these traversable collections.
...
* returns
  * a new immutable list map resulting from concatenating all element immutable
    list maps.
...
### `def transpose[B](implicit asTraversable: ((A, B1)) ⇒ GenTraversableOnce[B]): Iterable[Iterable[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: ((A, B1)) ⇒ (A1, A2, A3)): (Iterable[A1], Iterable[A2], Iterable[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: ((A, B1)) ⇒ (A1, A2)): (Iterable[A1], Iterable[A2])` ###
...
### `def -(elem1: A, elem2: A, elems: A*): ListMap[A, B1]`                   ###
...
### `def --(xs: GenTraversableOnce[A]): ListMap[A, B1]`                      ###
...
### `def ++[B1 >: B1](xs: GenTraversableOnce[(A, B1)]): ListMap[A, B1]`      ###
...
* Definition Classes
  * ListMap → MapLike → MapLike

(defined at scala.collection.immutable.ListMap)


### `def +[B1 >: B1](kv: (A, B1)): ListMap[A, B1]`                           ###
...
* Definition Classes
  * ListMap → Map → MapLike → MapLike → GenMapLike

(defined at scala.collection.immutable.ListMap)


### `def +[B1 >: B1](elem1: (A, B1), elem2: (A, B1), elems: (A, B1)*): ListMap[A, B1]` ###
...
* Definition Classes
  * ListMap → Map → Map → MapLike

(defined at scala.collection.immutable.ListMap)


### `def iterator: Iterator[(A, B1)]`                                        ###
...
* Definition Classes
  * ListMap → MapLike → IterableLike → GenIterableLike

(defined at scala.collection.immutable.ListMap)


--------------------------------------------------------------------------------
       Instance Constructors From scala.collection.immutable.ListMap.Node
--------------------------------------------------------------------------------


### `new Node(key: A, value: B1)`                                            ###

(defined at scala.collection.immutable.ListMap.Node)


--------------------------------------------------------------------------------
           Value Members From scala.collection.immutable.ListMap.Node
--------------------------------------------------------------------------------


### `def -(k: A): ListMap[A, B1]`                                            ###

Creates a new mapping without the given `key` . If the map does not contain a
mapping for the given key, the method returns the same map.

* returns
  * a new map without a binding for `key`

* Definition Classes
  * Node → ListMap → MapLike → Subtractable → GenMapLike
...
Retrieves the value which is associated with the given key. This method throws
an exception if there is no mapping from the given key to a value.
...
* Definition Classes
  * Node → MapLike → Function1 → GenMapLike
...
* Definition Classes
  * Node → MapLike → GenMapLike

(defined at scala.collection.immutable.ListMap.Node)


### `def get(k: A): Option[B1]`                                              ###
...
* Definition Classes
  * Node → ListMap → MapLike → GenMapLike

(defined at scala.collection.immutable.ListMap.Node)


### `def next: ListMap[A, B1]`                                               ###
...
### `def updated[B2 >: B1](k: A, v: B2): ListMap[A, B2]`                     ###

This method allows one to create a new map with an additional mapping from
 `key` to `value` . If the map contains already a mapping for `key` , it will be
...
* Definition Classes
  * Node → ListMap → Map → MapLike → MapLike → GenMap
...
### `def toMap[T, U](implicit ev: <:<[(A, B1), (T, U)]): Map[T, U]`          ###
...
### `def withDefaultValue[B1 >: B1](d: B1): Map[A, B1]`                      ###
...
### `def withDefault[B1 >: B1](d: (A) ⇒ B1): Map[A, B1]`                     ###
...
### `def filterKeys(p: (A) ⇒ Boolean): Map[A, B1]`                           ###
...
### `def mapValues[C](f: (B1) ⇒ C): Map[A, C]`                               ###
...
### `def parCombiner: Combiner[(A, B1), ParMap[A, B1]]`                      ###
...
### `def transform[C, That](f: (A, B1) ⇒ C)(implicit bf: CanBuildFrom[ListMap[A, B1], (A, C), That]): That` ###
...
* Implicit information
  * This member is added by an implicit conversion from Node [B1] to
    CollectionsHaveToParArray [Node [B1], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (Node [B1]) ⇒
