...
### `class DefaultKeySet extends LinkedHashMap.DefaultKeySet`                ###
...
### `class FilteredKeys extends LinkedHashMap.FilteredKeys`                  ###
...
### `class MappedValues[C] extends LinkedHashMap.MappedValues[C]`            ###
...
Copies the elements of this linked hash map to an array. Fills the given array
 `xs` with at most `len` elements of this linked hash map, starting at position
 `start` . Copying will stop once either the end of the current linked hash map
...
### `def drop(n: Int): LinkedHashMap[A, B]`                                  ###
...
### `def dropRight(n: Int): LinkedHashMap[A, B]`                             ###
...
### `def grouped(size: Int): Iterator[LinkedHashMap[A, B]]`                  ###
...
### `def slice(from: Int, until: Int): LinkedHashMap[A, B]`                  ###
...
### `def sliding(size: Int): Iterator[LinkedHashMap[A, B]]`                  ###
...
### `def sliding(size: Int, step: Int): Iterator[LinkedHashMap[A, B]]`       ###
...
### `def take(n: Int): LinkedHashMap[A, B]`                                  ###
...
### `def takeRight(n: Int): LinkedHashMap[A, B]`                             ###
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): LinkedHashMap[A, B]`              ###
...
### `def toCollection(repr: LinkedHashMap[A, B]): collection.Iterable[(A, B)]` ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), LinkedHashMap[A, B]]` ###
...
### `def view: IterableView[(A, B), LinkedHashMap[A, B]]`                    ###
...
### `def zipAll[B, A1 >: (A, B), That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[LinkedHashMap[A, B], (A1, B), That]): That` ###

[use case]

Returns a linked hash map formed from this linked hash map and another iterable
...
* thisElem
  * the element to be used to fill up the result if this linked hash map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    linked hash map.
* returns
  * a new linked hash map containing pairs consisting of corresponding elements
    of this linked hash map and `that` . The length of the returned collection
    is the maximum of the lengths of this linked hash map and `that` . If this
    linked hash map is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this linked hash map, `thatElem`
...
### `def zipWithIndex[A1 >: (A, B), That](implicit bf: CanBuildFrom[LinkedHashMap[A, B], (A1, Int), That]): That` ###
...
* returns
  * A new linked hash map containing pairs consisting of all elements of this
...
### `def zip[A1 >: (A, B), B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], (A1, B), That]): That` ###
...
* returns
  * a new linked hash map containing pairs consisting of corresponding elements
    of this linked hash map and `that` . The length of the returned collection
    is the minimum of the lengths of this linked hash map and `that` .
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): LinkedHashMap[A, B]`              ###
...
### `def ++:[B >: (A, B), That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
### `def ++:[B >: (A, B), That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
* that
  * the traversable to append.
* returns
  * a new linked hash map which contains all elements of this linked hash map
...
### `def ++[B >: (A, B), That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###

[use case]

Returns a new linked hash map containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
linked hash map is the most specific superclass encompassing the element types
of the two operands.
...
### `def collect[B, That](pf: PartialFunction[(A, B), B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
linked hash map on which the function is defined.
...
### `def dropWhile(p: ((A, B)) ⇒ Boolean): LinkedHashMap[A, B]`              ###
...
### `def filter(p: ((A, B)) ⇒ Boolean): LinkedHashMap[A, B]`                 ###
...
### `def flatMap[B, That](f: ((A, B)) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this linked
hash map and using the elements of the resulting collections.
...
The type of the resulting collection is guided by the static type of linked hash
...
* returns
  * a new linked hash map resulting from applying the given collection-valued
    function `f` to each element of this linked hash map and concatenating the
    results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, LinkedHashMap[A, B]]` ###
...
### `def inits: Iterator[LinkedHashMap[A, B]]`                               ###
...
### `def map[B, That](f: ((A, B)) ⇒ B)(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new linked hash map resulting from applying the given function `f` to each
    element of this linked hash map and collecting the results.
...
### `def partition(p: ((A, B)) ⇒ Boolean): (LinkedHashMap[A, B], LinkedHashMap[A, B])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[LinkedHashMap[A, B], B, That]): That` ###
...
### `def span(p: ((A, B)) ⇒ Boolean): (LinkedHashMap[A, B], LinkedHashMap[A, B])` ###
...
### `def splitAt(n: Int): (LinkedHashMap[A, B], LinkedHashMap[A, B])`        ###
...
### `def tails: Iterator[LinkedHashMap[A, B]]`                               ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), LinkedHashMap[A, B]]` ###
...
Copies the elements of this linked hash map to an array. Fills the given array
 `xs` with values of this linked hash map. Copying will stop once either the end
of the current linked hash map is reached, or the end of the target array is
reached.
...
Copies the elements of this linked hash map to an array. Fills the given array
 `xs` with values of this linked hash map, beginning at index `start` . Copying
will stop once either the end of the current linked hash map is reached, or the
...
* returns
  * the first element of this linked hash map with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this linked hash map with the smallest value measured
    by function f.
...
Converts this linked hash map to a map. This method is unavailable unless the
...
Converts this linked hash map of traversable collections into a linked hash map
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of linked hash
map. For example:
...
* returns
  * a new linked hash map resulting from concatenating all element linked hash
    maps.
...
### `def ++=(xs: TraversableOnce[(A, B)]): LinkedHashMap.this.type`          ###
...
### `def +=(elem1: (A, B), elem2: (A, B), elems: (A, B)*): LinkedHashMap.this.type` ###
...
### `def --=(xs: TraversableOnce[A]): LinkedHashMap.this.type`               ###
...
### `def -=(elem1: A, elem2: A, elems: A*): LinkedHashMap.this.type`         ###
...
### `def mapResult[NewTo](f: (LinkedHashMap[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
### `def addEntry(e: LinkedEntry[A, B]): Unit`                               ###
...
### `def entriesIterator: Iterator[LinkedEntry[A, B]]`                       ###
...
### `def findEntry(key: A): LinkedEntry[A, B]`                               ###
...
### `def findOrAddEntry[B](key: A, value: B): LinkedEntry[A, B]`             ###
...
### `def initWithContents(c: Contents[A, LinkedEntry[A, B]]): Unit`          ###
...
### `def removeEntry(key: A): LinkedEntry[A, B]`                             ###
...
### `var table: Array[HashEntry[A, LinkedEntry[A, B]]]`                      ###
...
--------------------------------------------------------------------------------
       Instance Constructors From scala.collection.mutable.LinkedHashMap
--------------------------------------------------------------------------------
...
### `def createNewEntry[B1](key: A, value: B1): Entry`                       ###
...
* Definition Classes
  * LinkedHashMap → IterableLike → GenericTraversableTemplate → TraversableLike
...
### `def -(elem1: A, elem2: A, elems: A*): LinkedHashMap[A, B]`              ###
...
### `def --(xs: GenTraversableOnce[A]): LinkedHashMap[A, B]`                 ###
...
### `def newBuilder: Builder[(A, B), LinkedHashMap[A, B]]`                   ###
...
### `def retain(p: (A, B) ⇒ Boolean): LinkedHashMap.this.type`               ###
...
### `def transform(f: (A, B) ⇒ B): LinkedHashMap.this.type`                  ###
...
* Implicit information
  * This member is added by an implicit conversion from LinkedHashMap [A, B] to
    CollectionsHaveToParArray [LinkedHashMap [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (LinkedHashMap [A, B]) ⇒
