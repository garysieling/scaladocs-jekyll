...
* returns
  * the index of the first element of this linear sequence that is equal (as
...
Finds index of first occurrence of some value in this linear sequence after or
at some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this linear sequence that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this linear sequence that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this linear sequence before or
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this linear sequence that is equal
...
Copies the elements of this linear sequence to an array. Fills the given array
 `xs` with at most `len` elements of this linear sequence, starting at position
 `start` . Copying will stop once either the end of the current linear sequence
...
Returns a linear sequence formed from this linear sequence and another iterable
...
* thisElem
  * the element to be used to fill up the result if this linear sequence is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    linear sequence.
* returns
  * a new linear sequence containing pairs consisting of corresponding elements
    of this linear sequence and `that` . The length of the returned collection
    is the maximum of the lengths of this linear sequence and `that` . If this
    linear sequence is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this linear sequence, `thatElem`
...
* returns
  * a new linear sequence containing pairs consisting of corresponding elements
    of this linear sequence and `that` . The length of the returned collection
    is the minimum of the lengths of this linear sequence and `that` .
...
Also, the original linear sequence is not modified, so you will want to capture
the result.
...
* returns
  * a new linear sequence consisting of `elem` followed by all elements of this
    linear sequence.
...
Computes the multiset difference between this linear sequence and another
sequence.
...
Computes the multiset intersection between this linear sequence and another
sequence.
...
A copy of this linear sequence with an element value appended until a given
...
* returns
  * a new linear sequence consisting of all elements of this linear sequence
    ...
    resulting linear sequence has a length of at least `len` .
...
Produces a new linear sequence where a slice of elements in this linear sequence
...
Builds a new collection by applying a function to all elements of this linear
...
* returns
  * a new linear sequence resulting from applying the given function `f` to each
    element of this linear sequence and collecting the results in reversed
    order.
...
Produces a new sequence which contains all elements of this linear sequence and
...
* returns
  * a copy of this linear sequence with the element at position `index` replaced
    by `elem` .
...
Returns a new linear sequence containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
linear sequence is the most specific superclass encompassing the element types
of the two operands.
...
linear sequence on which the function is defined.
...
The type of the resulting collection is guided by the static type of linear
...
* f
  * the function to apply to each element.
* returns
  * a new linear sequence resulting from applying the given collection-valued
    function `f` to each element of this linear sequence and concatenating the
    results.
...
* returns
  * a new linear sequence resulting from applying the given function `f` to each
    element of this linear sequence and collecting the results.
...
Copies the elements of this linear sequence to an array. Fills the given array
 `xs` with values of this linear sequence. Copying will stop once either the end
of the current linear sequence is reached, or the end of the target array is
reached.
...
Copies the elements of this linear sequence to an array. Fills the given array
 `xs` with values of this linear sequence, beginning at index `start` . Copying
will stop once either the end of the current linear sequence is reached, or the
...
* returns
  * the first element of this linear sequence with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this linear sequence with the smallest value measured
    by function f.
...
Converts this linear sequence to a map. This method is unavailable unless the
...
Converts this linear sequence of traversable collections into a linear sequence
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of linear
...
* returns
  * a new linear sequence resulting from concatenating all element linear
    sequences.
...
The factory companion object that builds instances of class `LinearSeq` . (or
its `Iterable` superclass where class `LinearSeq` is not a `Seq` .)
...
### `def transform(f: (A) ⇒ A): LinearSeq.this.type`                         ###
