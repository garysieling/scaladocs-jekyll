...
### `def &(that: GenSet[A]): LinkedHashSet[A]`                               ###
...
### `def intersect(that: GenSet[A]): LinkedHashSet[A]`                       ###
...
Copies the elements of this linked hash set to an array. Fills the given array
 `xs` with at most `len` elements of this linked hash set, starting at position
 `start` . Copying will stop once either the end of the current linked hash set
...
### `def sliding(size: Int): Iterator[LinkedHashSet[A]]`                     ###
...
### `def sliding(size: Int, step: Int): Iterator[LinkedHashSet[A]]`          ###
...
### `def toCollection(repr: LinkedHashSet[A]): collection.Iterable[A]`       ###
...
Returns a linked hash set formed from this linked hash set and another iterable
...
* thisElem
  * the element to be used to fill up the result if this linked hash set is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    linked hash set.
* returns
  * a new linked hash set containing pairs consisting of corresponding elements
    of this linked hash set and `that` . The length of the returned collection
    is the maximum of the lengths of this linked hash set and `that` . If this
    linked hash set is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this linked hash set, `thatElem`
...
* returns
  * a new linked hash set containing pairs consisting of corresponding elements
    of this linked hash set and `that` . The length of the returned collection
    is the minimum of the lengths of this linked hash set and `that` .
...
### `def diff(that: GenSet[A]): LinkedHashSet[A]`                            ###
...
* returns
  * a new linked hash set resulting from applying the given function `f` to each
    element of this linked hash set and collecting the results.
...
### `def subsets(): Iterator[LinkedHashSet[A]]`                              ###
...
### `def subsets(len: Int): Iterator[LinkedHashSet[A]]`                      ###
...
### `def union(that: GenSet[A]): LinkedHashSet[A]`                           ###
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[LinkedHashSet[A], B, That]): That` ###
...
Returns a new linked hash set containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
linked hash set is the most specific superclass encompassing the element types
of the two operands.
...
linked hash set on which the function is defined.
...
* f
  * the function to apply to each element.
* returns
  * a new linked hash set resulting from applying the given collection-valued
    function `f` to each element of this linked hash set and concatenating the
    results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, LinkedHashSet[A]]`         ###
...
### `def inits: Iterator[LinkedHashSet[A]]`                                  ###
...
### `def scan[B >: A, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[LinkedHashSet[A], B, That]): That` ###
...
### `def tails: Iterator[LinkedHashSet[A]]`                                  ###
...
### `def withFilter(p: (A) ⇒ Boolean): FilterMonadic[A, LinkedHashSet[A]]`   ###
...
Copies the elements of this linked hash set to an array. Fills the given array
 `xs` with values of this linked hash set. Copying will stop once either the end
of the current linked hash set is reached, or the end of the target array is
reached.
...
Copies the elements of this linked hash set to an array. Fills the given array
 `xs` with values of this linked hash set, beginning at index `start` . Copying
will stop once either the end of the current linked hash set is reached, or the
...
* returns
  * the first element of this linked hash set with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this linked hash set with the smallest value measured
    by function f.
...
Converts this linked hash set to a map. This method is unavailable unless the
...
Converts this linked hash set of traversable collections into a linked hash set
...
* returns
  * a new linked hash set resulting from concatenating all element linked hash
    sets.
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): LinkedHashSet[LinkedHashSet[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (A) ⇒ (A1, A2, A3)): (LinkedHashSet[A1], LinkedHashSet[A2], LinkedHashSet[A3])` ###
...
### `def ++=(xs: TraversableOnce[A]): LinkedHashSet.this.type`               ###
...
### `def +=(elem1: A, elem2: A, elems: A*): LinkedHashSet.this.type`         ###
...
### `def addEntry(e: LinkedHashSet.Entry[A]): Unit`                          ###
...
### `def entriesIterator: Iterator[LinkedHashSet.Entry[A]]`                  ###
...
### `def findEntry(key: A): LinkedHashSet.Entry[A]`                          ###
...
### `def findOrAddEntry[B](key: A, value: B): LinkedHashSet.Entry[A]`        ###
...
### `def initWithContents(c: Contents[A, LinkedHashSet.Entry[A]]): Unit`     ###
...
### `def removeEntry(key: A): LinkedHashSet.Entry[A]`                        ###
...
### `var table: Array[HashEntry[A, LinkedHashSet.Entry[A]]]`                 ###
...
--------------------------------------------------------------------------------
       Instance Constructors From scala.collection.mutable.LinkedHashSet
--------------------------------------------------------------------------------
...
### `def companion: GenericCompanion[LinkedHashSet]`                         ###
...
* Definition Classes
  * LinkedHashSet → Set → Set → GenSet → Iterable → Traversable → Iterable →
...
* Definition Classes
  * LinkedHashSet → IterableLike → GenericTraversableTemplate → TraversableLike
...
### `def ++(xs: GenTraversableOnce[A]): LinkedHashSet[A]`                    ###
...
* Implicit information
  * This member is added by an implicit conversion from LinkedHashSet [A] to
    CollectionsHaveToParArray [LinkedHashSet [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (LinkedHashSet [A]) ⇒
