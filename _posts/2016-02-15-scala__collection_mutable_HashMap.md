...
Copies the elements of this mutable hash map to an array. Fills the given array
 `xs` with at most `len` elements of this mutable hash map, starting at position
 `start` . Copying will stop once either the end of the current mutable hash map
...
order as this mutable hash map.
...
Returns a mutable hash map formed from this mutable hash map and another
...
* thisElem
  * the element to be used to fill up the result if this mutable hash map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable hash map.
* returns
  * a new mutable hash map containing pairs consisting of corresponding elements
    of this mutable hash map and `that` . The length of the returned collection
    is the maximum of the lengths of this mutable hash map and `that` . If this
    mutable hash map is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this mutable hash map, `thatElem`
...
* returns
  * A new mutable hash map containing pairs consisting of all elements of this
    mutable hash map paired with their index. Indices start at `0` .
...
* returns
  * a new mutable hash map containing pairs consisting of corresponding elements
    of this mutable hash map and `that` . The length of the returned collection
    is the minimum of the lengths of this mutable hash map and `that` .
...
* that
  * the traversable to append.
* returns
  * a new mutable hash map which contains all elements of this mutable hash map
...
Returns a new mutable hash map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the mutable hash map is the most specific superclass encompassing the element
...
mutable hash map on which the function is defined.
...
* returns
  * a new mutable hash map resulting from applying the given partial function
...
* returns
  * a new mutable hash map resulting from applying the given collection-valued
    function `f` to each element of this mutable hash map and concatenating the
    results.
...
* f
  * the function to apply to each element.
* returns
  * a new mutable hash map resulting from applying the given function `f` to
    each element of this mutable hash map and collecting the results.
...
Copies the elements of this mutable hash map to an array. Fills the given array
 `xs` with values of this mutable hash map. Copying will stop once either the
end of the current mutable hash map is reached, or the end of the target array
is reached.
...
Copies the elements of this mutable hash map to an array. Fills the given array
 `xs` with values of this mutable hash map, beginning at index `start` . Copying
will stop once either the end of the current mutable hash map is reached, or the
...
* returns
  * the first element of this mutable hash map with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable hash map with the smallest value measured
    by function f.
...
Converts this mutable hash map to a map. This method is unavailable unless the
...
Converts this mutable hash map of traversable collections into a mutable hash
...
* returns
  * a new mutable hash map resulting from concatenating all element mutable hash
    maps.
...
### `def ++=(xs: TraversableOnce[(A, B)]): HashMap.this.type`                ###
...
### `def +=(elem1: (A, B), elem2: (A, B), elems: (A, B)*): HashMap.this.type` ###
...
### `def --=(xs: TraversableOnce[A]): HashMap.this.type`                     ###
...
### `def -=(elem1: A, elem2: A, elems: A*): HashMap.this.type`               ###
...
### `def mapResult[NewTo](f: (HashMap[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.HashMap
--------------------------------------------------------------------------------
...
### `def addEntry(e: DefaultEntry[A, B]): Unit`                              ###
...
### `def entriesIterator: Iterator[DefaultEntry[A, B]]`                      ###
...
### `def findEntry(key: A): DefaultEntry[A, B]`                              ###
...
### `def findOrAddEntry[B](key: A, value: B): DefaultEntry[A, B]`            ###
...
### `def foreachEntry[U](f: (DefaultEntry[A, B]) ⇒ U): Unit`                 ###
...
### `def initWithContents(c: Contents[A, DefaultEntry[A, B]]): Unit`         ###
...
### `def removeEntry(key: A): DefaultEntry[A, B]`                            ###
...
### `var table: Array[HashEntry[A, DefaultEntry[A, B]]]`                     ###
