...
Copies the elements of this mutable tree set to an array. Fills the given array
 `xs` with at most `len` elements of this mutable tree set, starting at position
 `start` . Copying will stop once either the end of the current mutable tree set
...
order as this mutable tree set.
...
Returns a mutable tree set formed from this mutable tree set and another
...
* thisElem
  * the element to be used to fill up the result if this mutable tree set is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    mutable tree set.
* returns
  * a new mutable tree set containing pairs consisting of corresponding elements
    of this mutable tree set and `that` . The length of the returned collection
    is the maximum of the lengths of this mutable tree set and `that` . If this
    mutable tree set is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this mutable tree set, `thatElem`
...
Zips this mutable tree set with its indices.

Note: might return different results for different runs, unless the underlying
collection type is ordered.

* returns
  * A new mutable tree set containing pairs consisting of all elements of this
    mutable tree set paired with their index. Indices start at `0` .
...
* returns
  * a new mutable tree set containing pairs consisting of corresponding elements
    of this mutable tree set and `that` . The length of the returned collection
    is the minimum of the lengths of this mutable tree set and `that` .
...
* returns
  * a new mutable tree set resulting from applying the given function `f` to
    each element of this mutable tree set and collecting the results.
...
* that
  * the traversable to append.
* returns
  * a new mutable tree set which contains all elements of this mutable tree set
...
Returns a new mutable tree set containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the mutable tree set is the most specific superclass encompassing the element
...
mutable tree set on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the mutable tree set.
* returns
  * a new mutable tree set resulting from applying the given partial function
...
* f
  * the function to apply to each element.
* returns
  * a new mutable tree set resulting from applying the given collection-valued
    function `f` to each element of this mutable tree set and concatenating the
    results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, TreeSet[A]]`               ###
...
Copies the elements of this mutable tree set to an array. Fills the given array
 `xs` with values of this mutable tree set. Copying will stop once either the
end of the current mutable tree set is reached, or the end of the target array
is reached.
...
Copies the elements of this mutable tree set to an array. Fills the given array
 `xs` with values of this mutable tree set, beginning at index `start` . Copying
will stop once either the end of the current mutable tree set is reached, or the
...
* returns
  * the first element of this mutable tree set with the largest value measured
    by function f.
...
* f
  * The measuring function.
* returns
  * the first element of this mutable tree set with the smallest value measured
    by function f.
...
Converts this mutable tree set to a map. This method is unavailable unless the
...
Converts this mutable tree set of traversable collections into a mutable tree
...
* returns
  * a new mutable tree set resulting from concatenating all element mutable tree
    sets.
...
### `def ++=(xs: TraversableOnce[A]): TreeSet.this.type`                     ###
...
### `def +=(elem1: A, elem2: A, elems: A*): TreeSet.this.type`               ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.TreeSet
--------------------------------------------------------------------------------
...
* Definition Classes
  * TreeSet → SetLike → Builder → Growable

(defined at scala.collection.mutable.TreeSet)


### `def -=(key: A): TreeSet.this.type`                                      ###

Removes a single element from this mutable tree set.

* returns
  * the mutable tree set itself

* Definition Classes
  * TreeSet → SetLike → Shrinkable
...
Applies a function `f` to all elements of this mutable tree set.
...
* Attributes
  * protected[this]
* Definition Classes
  * TreeSet → SetLike → SetLike → GenericTraversableTemplate → TraversableLike →
    HasNewBuilder
...
Creates a ranged projection of this set. Any mutations in the ranged projection
affect will update the original set and vice versa.

Only keys between this projection's key range will ever appear as elements of
this set, independently of whether the elements are added through the original
set or through this view. That means that if one inserts an element in a view
whose key is outside the view's bounds, calls to `contains` will _not_ consider
the newly added element. Mutations are always reflected in the original set,
though.
