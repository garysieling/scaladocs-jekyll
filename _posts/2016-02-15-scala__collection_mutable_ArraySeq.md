...
### `def parCombiner: Combiner[A, ParArray[A]]`                              ###
...
* returns
  * the index of the first element of this array sequence that is equal (as
...
Finds index of first occurrence of some value in this array sequence after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this array sequence that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this array sequence that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this array sequence before or at
a given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this array sequence that is equal
...
### `def dropRight(n: Int): ArraySeq[A]`                                     ###
...
### `def dropWhile(p: (A) ⇒ Boolean): ArraySeq[A]`                           ###
...
### `def slice(from: Int, until: Int): ArraySeq[A]`                          ###
...
### `def span(p: (A) ⇒ Boolean): (ArraySeq[A], ArraySeq[A])`                 ###
...
### `def splitAt(n: Int): (ArraySeq[A], ArraySeq[A])`                        ###
...
### `def takeRight(n: Int): ArraySeq[A]`                                     ###
...
### `def takeWhile(p: (A) ⇒ Boolean): ArraySeq[A]`                           ###
...
### `def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[ArraySeq[A], (A1, Int), That]): That` ###
...
* returns
  * A new array sequence containing pairs consisting of all elements of this
    array sequence paired with their index. Indices start at `0` .
...
### `def zip[A1 >: A, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[ArraySeq[A], (A1, B), That]): That` ###

[use case]

Returns a array sequence formed from this array sequence and another iterable
...
* returns
  * a new array sequence containing pairs consisting of corresponding elements
    of this array sequence and `that` . The length of the returned collection is
    the minimum of the lengths of this array sequence and `that` .
...
### `def grouped(size: Int): Iterator[ArraySeq[A]]`                          ###
...
### `def sliding(size: Int): Iterator[ArraySeq[A]]`                          ###
...
### `def sliding(size: Int, step: Int): Iterator[ArraySeq[A]]`               ###
...
### `def zipAll[B, A1 >: A, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[ArraySeq[A], (A1, B), That]): That` ###
...
* that
  * The iterable providing the second half of each result pair
* thisElem
  * the element to be used to fill up the result if this array sequence is
    shorter than `that` .
* thatElem
...
* returns
  * a new array sequence containing pairs consisting of corresponding elements
    of this array sequence and `that` . The length of the returned collection is
    the maximum of the lengths of this array sequence and `that` . If this array
    sequence is shorter than `that` , `thisElem` values are used to pad the
    result. If `that` is shorter than this array sequence, `thatElem` values are
...
A copy of the array sequence with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original array sequence is not modified, so you will want to capture
the result.
...
* returns
  * a new array sequence consisting of `elem` followed by all elements of this
    array sequence.
...
A copy of this array sequence with an element appended.
...
* returns
  * a new array sequence consisting of all elements of this array sequence
    followed by `elem` .
...
### `def combinations(n: Int): Iterator[ArraySeq[A]]`                        ###
...
### `def diff[B >: A](that: GenSeq[B]): ArraySeq[A]`                         ###

[use case]

Computes the multiset difference between this array sequence and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new array sequence which contains all elements of this array sequence
...
### `def intersect[B >: A](that: GenSeq[B]): ArraySeq[A]`                    ###

[use case]

Computes the multiset intersection between this array sequence and another
sequence.
...
### `def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###

[use case]

A copy of this array sequence with an element value appended until a given
...
* returns
  * a new array sequence consisting of all elements of this array sequence
    ...
    resulting array sequence has a length of at least `len` .
...
### `def patch[B >: A, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###

[use case]

Produces a new array sequence where a slice of elements in this array sequence
...
### `def permutations: Iterator[ArraySeq[A]]`                                ###
...
### `def reverseMap[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
* returns
  * a new array sequence resulting from applying the given function `f` to each
    element of this array sequence and collecting the results in reversed order.
...
### `def sortBy[B](f: (A) ⇒ B)(implicit ord: math.Ordering[B]): ArraySeq[A]` ###
...
### `def sortWith(lt: (A, A) ⇒ Boolean): ArraySeq[A]`                        ###
...
### `def sorted[B >: A](implicit ord: math.Ordering[B]): ArraySeq[A]`        ###
...
### `def union[B >: A, That](that: GenSeq[B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this array sequence and
...
### `def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
* returns
  * a copy of this array sequence with the element at position `index` replaced
    by `elem` .
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
### `def ++:[B >: A, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
### `def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###

[use case]

Returns a new array sequence containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
array sequence is the most specific superclass encompassing the element types of
the two operands.
...
### `def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
array sequence on which the function is defined.
...
* returns
  * a new array sequence resulting from applying the given partial function
...
### `def filter(p: (A) ⇒ Boolean): ArraySeq[A]`                              ###
...
### `def filterNot(p: (A) ⇒ Boolean): ArraySeq[A]`                           ###
...
### `def flatMap[B, That](f: (A) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
* f
  * the function to apply to each element.
* returns
  * a new array sequence resulting from applying the given collection-valued
    function `f` to each element of this array sequence and concatenating the
    results.
...
### `def groupBy[K](f: (A) ⇒ K): immutable.Map[K, ArraySeq[A]]`              ###
...
### `def map[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
* returns
  * a new array sequence resulting from applying the given function `f` to each
    element of this array sequence and collecting the results.
...
### `def partition(p: (A) ⇒ Boolean): (ArraySeq[A], ArraySeq[A])`            ###
...
### `def scanLeft[B, That](z: B)(op: (B, A) ⇒ B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (A, B) ⇒ B)(implicit bf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
### `def scan[B >: A, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[ArraySeq[A], B, That]): That` ###
...
### `def withFilter(p: (A) ⇒ Boolean): FilterMonadic[A, ArraySeq[A]]`        ###
...
Copies the elements of this array sequence to an array. Fills the given array
 `xs` with values of this array sequence. Copying will stop once either the end
of the current array sequence is reached, or the end of the target array is
reached.
...
Copies the elements of this array sequence to an array. Fills the given array
 `xs` with values of this array sequence, beginning at index `start` . Copying
will stop once either the end of the current array sequence is reached, or the
...
* returns
  * the first element of this array sequence with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this array sequence with the smallest value measured by
    function f.
...
Converts this array sequence to a map. This method is unavailable unless the
...
### `def flatten[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): ArraySeq[B]` ###

[use case]

Converts this array sequence of traversable collections into a array sequence
...
* returns
  * a new array sequence resulting from concatenating all element array
    sequences.
...
### `def genericBuilder[B]: Builder[B, ArraySeq[B]]`                         ###
...
### `def newBuilder: Builder[A, ArraySeq[A]]`                                ###
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): ArraySeq[ArraySeq[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (A) ⇒ (A1, A2, A3)): (ArraySeq[A1], ArraySeq[A2], ArraySeq[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (A) ⇒ (A1, A2)): (ArraySeq[A1], ArraySeq[A2])` ###
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.mutable.ArraySeq
--------------------------------------------------------------------------------


### `new ArraySeq(length: Int)`                                              ###
...
* idx
  * The index to select.
* returns
  * the element of this array sequence at index `idx` , where `0` indicates the
    first element.

* Definition Classes
  * ArraySeq → SeqLike → GenSeqLike → Function1
* Exceptions thrown
...
### `def companion: GenericCompanion[ArraySeq]`                              ###

The factory companion object that builds instances of class `ArraySeq` . (or its
 `Iterable` superclass where class `ArraySeq` is not a `Seq` .)

* Definition Classes
  * ArraySeq → IndexedSeq → IndexedSeq → Seq → Iterable → Traversable → Seq →
...
current traversable is reached or `len` elements have been copied or the end of
...
* Definition Classes
  * ArraySeq → IndexedSeqOptimized → IterableLike → TraversableLike →
    TraversableOnce → GenTraversableOnce
...
* Definition Classes
  * ArraySeq → IndexedSeqOptimized → IterableLike → GenericTraversableTemplate →
...
* Definition Classes
  * ArraySeq → CustomParallelizable → Parallelizable
...
* Definition Classes
  * ArraySeq → IndexedSeqLike → SeqLike
* Exceptions thrown
...
### `def toCollection(repr: ArraySeq[A]): IndexedSeq[A]`                     ###
...
### `def view(from: Int, until: Int): IndexedSeqView[A, ArraySeq[A]]`        ###
...
### `def view: IndexedSeqView[A, ArraySeq[A]]`                               ###
...
### `def transform(f: (A) ⇒ A): ArraySeq.this.type`                          ###
...
* Implicit information
  * This member is added by an implicit conversion from ArraySeq [A] to
    SeqCharSequence performed by method SeqCharSequence in scala.Predef. This
    conversion will take place only if A is a subclass of Char (A <: Char).
* Definition Classes
...
* Implicit information
  * This member is added by an implicit conversion from ArraySeq [A] to
    CollectionsHaveToParArray [ArraySeq [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (ArraySeq [A]) ⇒
