...
Copies the elements of this immutable sorted map to an array. Fills the given
array `xs` with at most `len` elements of this immutable sorted map, starting at
position `start` . Copying will stop once either the end of the current
immutable sorted map is reached, or the end of the target array is reached, or
...
Applies a function `f` to all elements of this immutable sorted map.
...
order as this immutable sorted map.
...
Returns a immutable sorted map formed from this immutable sorted map and another
...
* thisElem
  * the element to be used to fill up the result if this immutable sorted map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable sorted map.
* returns
  * a new immutable sorted map containing pairs consisting of corresponding
    elements of this immutable sorted map and `that` . The length of the
    returned collection is the maximum of the lengths of this immutable sorted
    map and `that` . If this immutable sorted map is shorter than `that` ,
     `thisElem` values are used to pad the result. If `that` is shorter than
    this immutable sorted map, `thatElem` values are used to pad the result.
...
Zips this immutable sorted map with its indices.

Note: might return different results for different runs, unless the underlying
collection type is ordered.

* returns
  * A new immutable sorted map containing pairs consisting of all elements of
    this immutable sorted map paired with their index. Indices start at `0` .
...
### `abstract def -(key: A): SortedMap[A, B]`                                ###
...
* that
  * the traversable to append.
* returns
  * a new immutable sorted map which contains all elements of this immutable
    sorted map followed by all elements of `that` .
...
Returns a new immutable sorted map containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the immutable sorted map is the most specific superclass encompassing the
...
immutable sorted map on which the function is defined.

* B
  * the element type of the returned collection.
* pf
  * the partial function which filters and maps the immutable sorted map.
* returns
  * a new immutable sorted map resulting from applying the given partial
...
* returns
  * a new immutable sorted map resulting from applying the given
    collection-valued function `f` to each element of this immutable sorted map
...
* f
  * the function to apply to each element.
* returns
  * a new immutable sorted map resulting from applying the given function `f` to
    each element of this immutable sorted map and collecting the results.
...
Copies the elements of this immutable sorted map to an array. Fills the given
array `xs` with values of this immutable sorted map. Copying will stop once
either the end of the current immutable sorted map is reached, or the end of the
...
Copies the elements of this immutable sorted map to an array. Fills the given
array `xs` with values of this immutable sorted map, beginning at index `start` .
Copying will stop once either the end of the current immutable sorted map is
...
* returns
  * the first element of this immutable sorted map with the largest value
...
* f
  * The measuring function.
* returns
  * the first element of this immutable sorted map with the smallest value
    measured by function f.
...
Converts this immutable sorted map of traversable collections into a immutable
...
* returns
  * a new immutable sorted map resulting from concatenating all element
...
### `def transform[C, That](f: (A, B) ⇒ C)(implicit bf: CanBuildFrom[SortedMap[A, B], (A, C), That]): That` ###
...
--------------------------------------------------------------------------------
        Concrete Value Members From scala.collection.immutable.SortedMap
--------------------------------------------------------------------------------
...
* Definition Classes
  * SortedMap → SortedMapLike → Map → MapLike → MapLike → GenMapLike
* Note
...
* Attributes
  * protected[this]
* Definition Classes
  * SortedMap → SortedMap → MapLike → GenericTraversableTemplate →
...
* Definition Classes
  * SortedMap → SortedMapLike → Map → MapLike → MapLike → GenMap
