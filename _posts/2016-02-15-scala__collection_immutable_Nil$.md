...
* returns
  * the index of the first element of this list that is equal (as determined by
...
Finds index of first occurrence of some value in this list after or at some
start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this list that is equal (as
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this list that is equal (as determined by
     `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this list before or at a given
end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this list that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this list to an array. Fills the given array `xs` with at
most `len` elements of this list, starting at position `start` . Copying will
stop once either the end of the current list is reached, or the end of the
...
### `def grouped(size: Int): Iterator[List[Nothing]]`                        ###
...
### `def sliding(size: Int): Iterator[List[Nothing]]`                        ###
...
### `def sliding(size: Int, step: Int): Iterator[List[Nothing]]`             ###
...
### `def zipAll[B, A1 >: Nothing, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[List[Nothing], (A1, B), That]): That` ###
...
* thisElem
  * the element to be used to fill up the result if this list is shorter than
     `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    list.
* returns
  * a new list containing pairs consisting of corresponding elements of this
    list and `that` . The length of the returned collection is the maximum of
    the lengths of this list and `that` . If this list is shorter than `that` ,
     `thisElem` values are used to pad the result. If `that` is shorter than
    this list, `thatElem` values are used to pad the result.
...
### `def zip[A1 >: Nothing, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[List[Nothing], (A1, B), That]): That` ###
...
* returns
  * a new list containing pairs consisting of corresponding elements of this
    list and `that` . The length of the returned collection is the minimum of
    the lengths of this list and `that` .
...
### `def toCollection(repr: List[Nothing]): collection.LinearSeq[Nothing]`   ###
...
### `def dropRight(n: Int): List[Nothing]`                                   ###
...
### `def :+[B >: Nothing, That](elem: B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def combinations(n: Int): Iterator[List[Nothing]]`                      ###
...
### `def diff[B >: Nothing](that: GenSeq[B]): List[Nothing]`                 ###
...
* returns
  * a new list which contains all elements of this list except some of
...
### `def intersect[B >: Nothing](that: GenSeq[B]): List[Nothing]`            ###
...
* returns
  * a new list which contains all elements of this list which also appear in
...
### `def padTo[B >: Nothing, That](len: Int, elem: B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###

[use case]

A copy of this list with an element value appended until a given target length
is reached.
...
* elem
  * the padding value
* returns
  * a new list consisting of all elements of this list followed by the minimal
    number of occurrences of `elem` so that the resulting list has a length of
    at least `len` .
...
### `def patch[B >: Nothing, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###

[use case]

Produces a new list where a slice of elements in this list is replaced by
another sequence.
...
* returns
  * a new list consisting of all elements of this list except that `replaced`
...
### `def permutations: Iterator[List[Nothing]]`                              ###
...
### `def reverseMap[B, That](f: (Nothing) ⇒ B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
* returns
  * a new list resulting from applying the given function `f` to each element of
    this list and collecting the results in reversed order.
...
### `def sortBy[B](f: (Nothing) ⇒ B)(implicit ord: math.Ordering[B]): List[Nothing]` ###
...
### `def sortWith(lt: (Nothing, Nothing) ⇒ Boolean): List[Nothing]`          ###
...
### `def sorted[B >: Nothing](implicit ord: math.Ordering[B]): List[Nothing]` ###
...
### `def union[B >: Nothing, That](that: GenSeq[B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this list and also all
...
* that
  * the sequence to add.
* returns
  * a new list which contains all elements of this list followed by all elements
    of `that` .
...
### `def updated[B >: Nothing, That](index: Int, elem: B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def view(from: Int, until: Int): SeqView[Nothing, List[Nothing]]`       ###
...
### `def view: SeqView[Nothing, List[Nothing]]`                              ###
...
### `def ++:[B >: Nothing, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def ++:[B >: Nothing, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def filter(p: (Nothing) ⇒ Boolean): List[Nothing]`                      ###
...
### `def filterNot(p: (Nothing) ⇒ Boolean): List[Nothing]`                   ###
...
### `def groupBy[K](f: (Nothing) ⇒ K): Map[K, List[Nothing]]`                ###
...
### `def inits: Iterator[List[Nothing]]`                                     ###
...
### `def partition(p: (Nothing) ⇒ Boolean): (List[Nothing], List[Nothing])`  ###
...
### `def scanLeft[B, That](z: B)(op: (B, Nothing) ⇒ B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (Nothing, B) ⇒ B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def scan[B >: Nothing, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def tails: Iterator[List[Nothing]]`                                     ###
...
### `def withFilter(p: (Nothing) ⇒ Boolean): FilterMonadic[Nothing, List[Nothing]]` ###
...
Copies the elements of this list to an array. Fills the given array `xs` with
values of this list. Copying will stop once either the end of the current list
...
Copies the elements of this list to an array. Fills the given array `xs` with
values of this list, beginning at index `start` . Copying will stop once either
the end of the current list is reached, or the end of the target array is
reached.
...
* returns
  * the first element of this list with the largest value measured by function
    f.
...
* f
  * The measuring function.
* returns
  * the first element of this list with the smallest value measured by function
    f.
...
Converts this list to a map. This method is unavailable unless the elements are
...
### `def newBuilder: Builder[Nothing, List[Nothing]]`                        ###
...
### `def transpose[B](implicit asTraversable: (Nothing) ⇒ GenTraversableOnce[B]): List[List[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (Nothing) ⇒ (A1, A2, A3)): (List[A1], List[A2], List[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (Nothing) ⇒ (A1, A2)): (List[A1], List[A2])` ###
...
--------------------------------------------------------------------------------
               Value Members From scala.collection.immutable.List
--------------------------------------------------------------------------------


### `def ++[B >: Nothing, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###

[use case]

Returns a new list containing the elements from the left hand operand followed
by the elements from the right hand operand. The element type of the list is the
...
* returns
  * a list which contains `x` as first element and which continues with this
    list.
...
### `def :::[B >: Nothing](prefix: List[B]): List[B]`                        ###
...
* returns
  * a list resulting from the concatenation of the given list `prefix` and this
    list.
...
### `final def collect[B, That](pf: PartialFunction[Nothing, B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
* returns
  * a new list resulting from applying the given partial function `pf` to each
...
The factory companion object that builds instances of class `List` . (or its
 `Iterable` superclass where class `List` is not a `Seq` .)

* Definition Classes
  * List → LinearSeq → LinearSeq → Seq → Iterable → Traversable → Seq → GenSeq →
...
* returns
  * a list consisting of all elements of this list except the first `n` ones, or
    else the empty list, if this list has less than `n` elements.

* Definition Classes
  * List → LinearSeqOptimized → IterableLike → TraversableLike →
    GenTraversableLike

(defined at scala.collection.immutable.List)


### `final def dropWhile(p: (Nothing) ⇒ Boolean): List[Nothing]`             ###

Drops longest prefix of elements that satisfy a predicate.

* returns
  * the longest suffix of this list whose first element does not satisfy the
    predicate `p` .
...
### `final def flatMap[B, That](f: (Nothing) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
The type of the resulting collection is guided by the static type of list. This
...
* Definition Classes
  * List → TraversableLike → GenTraversableLike → FilterMonadic
...
Applies a binary operator to all elements of this list and a start value, going
right to left.
...
* op
  * the binary operator.
* returns
  * the result of inserting `op` between consecutive elements of this list,
    ...
    where `x1, ..., xn` are the elements of this list. Returns `z` if this list
    is empty.

* Definition Classes
  * List → LinearSeqOptimized → IterableLike → TraversableOnce →
    GenTraversableOnce
...
* Definition Classes
  * List → LinearSeqOptimized → IterableLike → GenericTraversableTemplate →
...
### `final def mapConserve[B >: Nothing <: AnyRef](f: (Nothing) ⇒ B): List[B]` ###

[use case]

Builds a new list by applying a function to all elements of this list. Like
 `xs map f` , but returns `xs` unchanged if function `f` maps all elements to
themselves (as determined by `eq` ).

* f
  * the function to apply to each element.
* returns
  * a list resulting from applying the given function `f` to each element of
    this list and collecting the results.
...
### `final def map[B, That](f: (Nothing) ⇒ B)(implicit bf: CanBuildFrom[List[Nothing], B, That]): That` ###
...
### `def reverse_:::[B >: Nothing](prefix: List[B]): List[B]`                ###
...
### `def slice(from: Int, until: Int): List[Nothing]`                        ###
...
### `final def span(p: (Nothing) ⇒ Boolean): (List[Nothing], List[Nothing])` ###
...
* returns
  * a pair consisting of the longest prefix of this list whose elements all
...
### `def splitAt(n: Int): (List[Nothing], List[Nothing])`                    ###

Splits this list into two at a given position. Note: `c splitAt n` is equivalent
to (but possibly more efficient than) `(c take n, c drop n)` .

* n
  * the position at which to split.
* returns
  * a pair of lists consisting of the first `n` elements of this list, and the
    other elements.
...
* returns
  * a list consisting only of the first `n` elements of this list, or else the
...
### `def takeRight(n: Int): List[Nothing]`                                   ###
...
* n
  * the number of elements to take
* returns
  * a list consisting only of the last `n` elements of this list, or else the
    whole list, if it has less than `n` elements.
...
### `final def takeWhile(p: (Nothing) ⇒ Boolean): List[Nothing]`             ###
...
--------------------------------------------------------------------------------
               Value Members From scala.collection.immutable.Nil
--------------------------------------------------------------------------------
...
* Definition Classes
  * Nil → GenSeqLike → Equals → AnyRef → Any

(defined at scala.collection.immutable.Nil)
