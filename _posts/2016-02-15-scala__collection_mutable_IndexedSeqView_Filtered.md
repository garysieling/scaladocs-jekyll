...
### `def newAppended[B >: A](that: GenTraversable[B]): Filtered.Transformed[B]` ###
...
### `def newFlatMapped[B](f: (A) ⇒ GenTraversableOnce[B]): Filtered.Transformed[B]` ###
...
### `def newForced[B](xs: ⇒ GenSeq[B]): Filtered.Transformed[B]`             ###
...
### `def newMapped[B](f: (A) ⇒ B): Filtered.Transformed[B]`                  ###
...
### `def newPatched[B >: A](_from: Int, _patch: GenSeq[B], _replaced: Int): Filtered.Transformed[B]` ###
...
### `def newPrepended[B >: A](that: GenTraversable[B]): Filtered.Transformed[B]` ###
...
### `def newZippedAll[A1 >: A, B](that: GenIterable[B], _thisElem: A1, _thatElem: B): Filtered.Transformed[(A1, B)]` ###
...
### `def newZipped[B](that: GenIterable[B]): Filtered.Transformed[(A, B)]`   ###
...
--------------------------------------------------------------------------------
  Concrete Value Members From scala.collection.mutable.IndexedSeqView.Filtered
--------------------------------------------------------------------------------
...
* Definition Classes
  * Filtered → Transformed → IndexedSeqView → IndexedSeqLike → SeqLike
* Exceptions thrown
  * IndexOutOfBoundsException if the index is not valid.

(defined at scala.collection.mutable.IndexedSeqView.Filtered)
...
### `def transform(f: (A) ⇒ A): Filtered.this.type`                          ###
