...
* returns
  * the index of the first element of this parallel array that is equal (as
...
Finds index of first occurrence of some value in this parallel array after or at
some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this parallel array that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this parallel array that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this parallel array before or at
a given end index.
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this parallel array that is equal
...
Produces a new sequence which contains all elements of this parallel array and
...
* that
  * the sequence to add.
* returns
  * a new parallel array which contains all elements of this parallel array
...
Converts this parallel array of traversable collections into a parallel array
...
* returns
  * a new parallel array resulting from concatenating all element parallel
    arrays.
...
Returns a new parallel array containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
parallel array is the most specific superclass encompassing the element types of
the two operands.
...
parallel array on which the function is defined.
...
Copies the elements of this parallel array to an array. Fills the given array
 `xs` with values of this parallel array. Copying will stop once either the end
of the current parallel array is reached, or the end of the target array is
reached.
...
Copies the elements of this parallel array to an array. Fills the given array
 `xs` with values of this parallel array, beginning at index `start` . Copying
will stop once either the end of the current parallel array is reached, or the
...
Copies the elements of this parallel array to an array. Fills the given array
 `xs` with at most `len` elements of this parallel array, starting at position
 `start` . Copying will stop once either the end of the current parallel array
...
* returns
  * a new parallel array resulting from applying the given collection-valued
    function `f` to each element of this parallel array and concatenating the
    results.
...
* returns
  * the first element of this parallel array with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this parallel array with the smallest value measured by
    function f.
...
Converts this parallel array to a map. This method is unavailable unless the
...
Returns a parallel array formed from this parallel array and another iterable
...
* thisElem
  * the element to be used to fill up the result if this parallel array is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    parallel array.
* returns
  * a new parallel array containing pairs consisting of corresponding elements
    of this parallel array and `that` . The length of the returned collection is
    the maximum of the lengths of this parallel array and `that` . If this
    parallel array is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this parallel array, `thatElem` values
...
* returns
  * A new parallel array containing pairs consisting of all elements of this
    parallel array paired with their index. Indices start at `0` .
...
Also, the original parallel array is not modified, so you will want to capture
the result.
...
* returns
  * a new parallel array consisting of `elem` followed by all elements of this
    parallel array.
...
Computes the multiset difference between this parallel array and another
sequence.
...
Computes the multiset intersection between this parallel array and another
sequence.
...
A copy of this parallel array with an element value appended until a given
...
* returns
  * a new parallel array consisting of all elements of this parallel array
    ...
    resulting parallel array has a length of at least `len` .
...
Produces a new parallel array where a slice of elements in this parallel array
...
* f
  * the function to apply to each element.
* returns
  * a new parallel array resulting from applying the given function `f` to each
    element of this parallel array and collecting the results in reversed order.
...
* returns
  * a copy of this parallel array with the element at position `index` replaced
    by `elem` .
...
* returns
  * a new parallel array containing pairs consisting of corresponding elements
    of this parallel array and `that` . The length of the returned collection is
    the minimum of the lengths of this parallel array and `that` .
...
* returns
  * the element of this parallel array at index `idx` , where `0` indicates the
    first element.
...
The factory companion object that builds instances of class `ParArray` . (or its
 `Iterable` superclass where class `ParArray` is not a `Seq` .)

* Definition Classes
  * ParArray → ParSeq → ParSeq → ParIterable → ParIterable → GenericParTemplate
...
* returns
  * a new parallel array resulting from applying the given function `f` to each
    element of this parallel array and collecting the results.
...
* returns
  * a new parallel array containing the prefix scan of the elements in this
    parallel array
...
* Definition Classes
  * ParArray → ParSeq → ParIterable → ParIterableLike → GenSeq → GenIterable →
...
* Implicit information
  * This member is added by an implicit conversion from ParArray [T] to
    CollectionsHaveToParArray [ParArray [T], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (ParArray [T]) ⇒
