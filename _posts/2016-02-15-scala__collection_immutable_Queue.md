...
* returns
  * the index of the first element of this immutable queue that is equal (as
...
Finds index of first occurrence of some value in this immutable queue after or
at some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this immutable queue that is
...
* elem
  * the element value to search for.
* returns
  * the index of the last element of this immutable queue that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this immutable queue before or
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this immutable queue that is equal
...
Copies the elements of this immutable queue to an array. Fills the given array
 `xs` with at most `len` elements of this immutable queue, starting at position
 `start` . Copying will stop once either the end of the current immutable queue
...
Returns a immutable queue formed from this immutable queue and another iterable
...
* thisElem
  * the element to be used to fill up the result if this immutable queue is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    immutable queue.
* returns
  * a new immutable queue containing pairs consisting of corresponding elements
    of this immutable queue and `that` . The length of the returned collection
    is the maximum of the lengths of this immutable queue and `that` . If this
    immutable queue is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this immutable queue, `thatElem`
...
* returns
  * a new immutable queue containing pairs consisting of corresponding elements
    of this immutable queue and `that` . The length of the returned collection
    is the minimum of the lengths of this immutable queue and `that` .
...
Computes the multiset difference between this immutable queue and another
sequence.
...
Computes the multiset intersection between this immutable queue and another
sequence.
...
A copy of this immutable queue with an element value appended until a given
...
* returns
  * a new immutable queue consisting of all elements of this immutable queue
    ...
    resulting immutable queue has a length of at least `len` .
...
Produces a new immutable queue where a slice of elements in this immutable queue
...
* returns
  * a new immutable queue resulting from applying the given function `f` to each
    element of this immutable queue and collecting the results in reversed
    order.
...
Produces a new sequence which contains all elements of this immutable queue and
...
* returns
  * a copy of this immutable queue with the element at position `index` replaced
    by `elem` .
...
Returns a new immutable queue containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
immutable queue is the most specific superclass encompassing the element types
of the two operands.
...
immutable queue on which the function is defined.
...
* f
  * the function to apply to each element.
* returns
  * a new immutable queue resulting from applying the given collection-valued
    function `f` to each element of this immutable queue and concatenating the
    results.
...
* returns
  * a new immutable queue resulting from applying the given function `f` to each
    element of this immutable queue and collecting the results.
...
Copies the elements of this immutable queue to an array. Fills the given array
 `xs` with values of this immutable queue. Copying will stop once either the end
of the current immutable queue is reached, or the end of the target array is
reached.
...
Copies the elements of this immutable queue to an array. Fills the given array
 `xs` with values of this immutable queue, beginning at index `start` . Copying
will stop once either the end of the current immutable queue is reached, or the
...
* returns
  * the first element of this immutable queue with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this immutable queue with the smallest value measured
    by function f.
...
Converts this immutable queue to a map. This method is unavailable unless the
...
Converts this immutable queue of traversable collections into a immutable queue
...
* returns
  * a new immutable queue resulting from concatenating all element immutable
    queues.
...
--------------------------------------------------------------------------------
          Instance Constructors From scala.collection.immutable.Queue
--------------------------------------------------------------------------------


### `new Queue(in: List[A], out: List[A])`                                   ###
...
Also, the original immutable queue is not modified, so you will want to capture
the result.
...
* returns
  * a new immutable queue consisting of `elem` followed by all elements of this
    immutable queue.

* Definition Classes
  * Queue → SeqLike → GenSeqLike
...
* Definition Classes
  * Queue → SeqLike → GenSeqLike → Function1
* Exceptions thrown
  * `java.util.NoSuchElementException` if the queue is too short.
...
(or its `Iterable` superclass where class `immutable.Queue` is not a `Seq` .)

* Definition Classes
  * Queue → LinearSeq → LinearSeq → Seq → Iterable → Traversable → Seq → GenSeq
...
Returns a tuple with the first element in the queue, and a new queue with this
element removed.
...
* returns
  * A tuple of the first element of the queue, and a new queue with this element
    removed. If the queue is empty, `None` is returned.
...
Returns a new queue with all elements provided by an `Iterable` object added at
...
* returns
  * a immutable queue consisting of all elements of this immutable queue except
    the first one.

* Definition Classes
  * Queue → TraversableLike → GenTraversableLike
* Exceptions thrown
  * ``UnsupportedOperationException`` if the immutable queue is empty.
