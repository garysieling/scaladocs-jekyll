...
* returns
  * the index of the first element of this unrolled buffer that is equal (as
...
Finds index of first occurrence of some value in this unrolled buffer after or
at some start index.
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this unrolled buffer that is
...
Finds index of last occurrence of some value in this unrolled buffer.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this unrolled buffer that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this unrolled buffer before or
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this unrolled buffer that is equal
...
Copies the elements of this unrolled buffer to an array. Fills the given array
 `xs` with at most `len` elements of this unrolled buffer, starting at position
 `start` . Copying will stop once either the end of the current unrolled buffer
...
### `def drop(n: Int): UnrolledBuffer[T]`                                    ###
...
### `def dropRight(n: Int): UnrolledBuffer[T]`                               ###
...
### `def grouped(size: Int): Iterator[UnrolledBuffer[T]]`                    ###
...
### `def slice(from: Int, until: Int): UnrolledBuffer[T]`                    ###
...
### `def sliding(size: Int): Iterator[UnrolledBuffer[T]]`                    ###
...
### `def sliding(size: Int, step: Int): Iterator[UnrolledBuffer[T]]`         ###
...
### `def take(n: Int): UnrolledBuffer[T]`                                    ###
...
### `def takeRight(n: Int): UnrolledBuffer[T]`                               ###
...
### `def takeWhile(p: (T) ⇒ Boolean): UnrolledBuffer[T]`                     ###
...
### `def zipAll[B, A1 >: T, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], (A1, B), That]): That` ###

[use case]

Returns a unrolled buffer formed from this unrolled buffer and another iterable
...
* thisElem
  * the element to be used to fill up the result if this unrolled buffer is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    unrolled buffer.
* returns
  * a new unrolled buffer containing pairs consisting of corresponding elements
    of this unrolled buffer and `that` . The length of the returned collection
    is the maximum of the lengths of this unrolled buffer and `that` . If this
    unrolled buffer is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this unrolled buffer, `thatElem`
...
### `def zipWithIndex[A1 >: T, That](implicit bf: CanBuildFrom[UnrolledBuffer[T], (A1, Int), That]): That` ###
...
* returns
  * A new unrolled buffer containing pairs consisting of all elements of this
    unrolled buffer paired with their index. Indices start at `0` .
...
### `def zip[A1 >: T, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], (A1, B), That]): That` ###
...
* returns
  * a new unrolled buffer containing pairs consisting of corresponding elements
    of this unrolled buffer and `that` . The length of the returned collection
    is the minimum of the lengths of this unrolled buffer and `that` .
...
### `def +:[B >: T, That](elem: B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
Also, the original unrolled buffer is not modified, so you will want to capture
the result.
...
* returns
  * a new unrolled buffer consisting of `elem` followed by all elements of this
    unrolled buffer.
...
* returns
  * a new unrolled buffer consisting of all elements of this unrolled buffer
    followed by `elem` .
...
### `def combinations(n: Int): Iterator[UnrolledBuffer[T]]`                  ###
...
### `def diff[B >: T](that: GenSeq[B]): UnrolledBuffer[T]`                   ###

[use case]

Computes the multiset difference between this unrolled buffer and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new unrolled buffer which contains all elements of this unrolled buffer
...
### `def intersect[B >: T](that: GenSeq[B]): UnrolledBuffer[T]`              ###

[use case]

Computes the multiset intersection between this unrolled buffer and another
sequence.
...
### `def padTo[B >: T, That](len: Int, elem: B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

A copy of this unrolled buffer with an element value appended until a given
...
* returns
  * a new unrolled buffer consisting of all elements of this unrolled buffer
    ...
    resulting unrolled buffer has a length of at least `len` .
...
### `def patch[B >: T, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

Produces a new unrolled buffer where a slice of elements in this unrolled buffer
...
### `def permutations: Iterator[UnrolledBuffer[T]]`                          ###
...
### `def reverseMap[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this unrolled
...
* returns
  * a new unrolled buffer resulting from applying the given function `f` to each
    element of this unrolled buffer and collecting the results in reversed
    order.
...
### `def sortBy[B](f: (T) ⇒ B)(implicit ord: math.Ordering[B]): UnrolledBuffer[T]` ###
...
### `def sortWith(lt: (T, T) ⇒ Boolean): UnrolledBuffer[T]`                  ###
...
### `def sorted[B >: T](implicit ord: math.Ordering[B]): UnrolledBuffer[T]`  ###
...
### `def thisCollection: collection.Seq[T]`                                  ###
...
### `def toCollection(repr: UnrolledBuffer[T]): collection.Seq[T]`           ###
...
### `def union[B >: T, That](that: GenSeq[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this unrolled buffer and
...
### `def updated[B >: T, That](index: Int, elem: B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
* returns
  * a copy of this unrolled buffer with the element at position `index` replaced
    by `elem` .
...
### `def view(from: Int, until: Int): SeqView[T, UnrolledBuffer[T]]`         ###
...
### `def view: SeqView[T, UnrolledBuffer[T]]`                                ###
...
### `def ++:[B >: T, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
### `def ++:[B >: T, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
### `def ++[B >: T, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

Returns a new unrolled buffer containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
unrolled buffer is the most specific superclass encompassing the element types
of the two operands.
...
### `def collect[B, That](pf: PartialFunction[T, B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
unrolled buffer on which the function is defined.
...
* returns
  * a new unrolled buffer resulting from applying the given partial function
...
### `def dropWhile(p: (T) ⇒ Boolean): UnrolledBuffer[T]`                     ###
...
### `def filter(p: (T) ⇒ Boolean): UnrolledBuffer[T]`                        ###
...
### `def filterNot(p: (T) ⇒ Boolean): UnrolledBuffer[T]`                     ###
...
### `def flatMap[B, That](f: (T) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
The type of the resulting collection is guided by the static type of unrolled
...
* f
  * the function to apply to each element.
* returns
  * a new unrolled buffer resulting from applying the given collection-valued
    function `f` to each element of this unrolled buffer and concatenating the
    results.
...
### `def groupBy[K](f: (T) ⇒ K): immutable.Map[K, UnrolledBuffer[T]]`        ###
...
### `def inits: Iterator[UnrolledBuffer[T]]`                                 ###
...
### `def map[B, That](f: (T) ⇒ B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
* returns
  * a new unrolled buffer resulting from applying the given function `f` to each
    element of this unrolled buffer and collecting the results.
...
### `def partition(p: (T) ⇒ Boolean): (UnrolledBuffer[T], UnrolledBuffer[T])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, T) ⇒ B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (T, B) ⇒ B)(implicit bf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
### `def scan[B >: T, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[UnrolledBuffer[T], B, That]): That` ###
...
### `def span(p: (T) ⇒ Boolean): (UnrolledBuffer[T], UnrolledBuffer[T])`     ###
...
### `def splitAt(n: Int): (UnrolledBuffer[T], UnrolledBuffer[T])`            ###
...
### `def tails: Iterator[UnrolledBuffer[T]]`                                 ###
...
### `def withFilter(p: (T) ⇒ Boolean): FilterMonadic[T, UnrolledBuffer[T]]`  ###
...
Copies the elements of this unrolled buffer to an array. Fills the given array
 `xs` with values of this unrolled buffer. Copying will stop once either the end
of the current unrolled buffer is reached, or the end of the target array is
reached.
...
Copies the elements of this unrolled buffer to an array. Fills the given array
 `xs` with values of this unrolled buffer, beginning at index `start` . Copying
will stop once either the end of the current unrolled buffer is reached, or the
...
* returns
  * the first element of this unrolled buffer with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this unrolled buffer with the smallest value measured
    by function f.
...
Converts this unrolled buffer to a map. This method is unavailable unless the
...
### `def classManifestCompanion: GenericClassManifestCompanion[UnrolledBuffer]` ###
...
### `def genericClassManifestBuilder[B](implicit manifest: ClassManifest[B]): Builder[B, UnrolledBuffer[B]]` ###
...
--------------------------------------------------------------------------------
 Value Members From scala.collection.generic.GenericClassTagTraversableTemplate
--------------------------------------------------------------------------------


### `def genericClassTagBuilder[B](implicit tag: ClassTag[B]): Builder[B, UnrolledBuffer[B]]` ###
...
### `def flatten[B](implicit asTraversable: (T) ⇒ GenTraversableOnce[B]): Buffer[B]` ###

[use case]

Converts this unrolled buffer of traversable collections into a unrolled buffer
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of unrolled
buffer. For example:
...
* returns
  * a new unrolled buffer resulting from concatenating all element unrolled
    buffers.
...
### `def transpose[B](implicit asTraversable: (T) ⇒ GenTraversableOnce[B]): Buffer[Buffer[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: (T) ⇒ (A1, A2, A3)): (Buffer[A1], Buffer[A2], Buffer[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: (T) ⇒ (A1, A2)): (Buffer[A1], Buffer[A2])` ###
...
### `def ++=(xs: TraversableOnce[T]): UnrolledBuffer.this.type`              ###
...
### `def +=(elem1: T, elem2: T, elems: T*): UnrolledBuffer.this.type`        ###
...
--------------------------------------------------------------------------------
               Value Members From scala.collection.mutable.Buffer
--------------------------------------------------------------------------------


### `def companion: GenericCompanion[Buffer]`                                ###

The factory companion object that builds instances of class `Buffer` . (or its
 `Iterable` superclass where class `Buffer` is not a `Seq` .)

* Definition Classes
  * Buffer → Seq → Seq → GenSeq → Iterable → Iterable → GenIterable →
    Traversable → Traversable → GenTraversable → GenericTraversableTemplate

(defined at scala.collection.mutable.Buffer)
...
### `def ++(xs: GenTraversableOnce[T]): UnrolledBuffer[T]`                   ###
...
### `def -(elem1: T, elem2: T, elems: T*): UnrolledBuffer[T]`                ###
...
### `def -=(x: T): UnrolledBuffer.this.type`                                 ###
...
### `def appendAll(xs: TraversableOnce[T]): Unit`                            ###
...
### `def prependAll(xs: TraversableOnce[T]): Unit`                           ###
...
Removes a number of elements from a given index position. Subclasses of
 `BufferLike` will typically override this method to provide better performance
than `count` successive calls to single-element `remove` .
...
### `def mapResult[NewTo](f: (UnrolledBuffer[T]) ⇒ NewTo): Builder[T, NewTo]` ###
...
### `def transform(f: (T) ⇒ T): UnrolledBuffer.this.type`                    ###
...
--------------------------------------------------------------------------------
       Instance Constructors From scala.collection.mutable.UnrolledBuffer
--------------------------------------------------------------------------------


### `new UnrolledBuffer()(implicit tag: ClassTag[T])`                        ###
...
* Definition Classes
  * UnrolledBuffer → Builder → BufferLike → Growable
...
Selects an element by its index in the unrolled buffer.
...
* idx
  * The index to select.
* returns
  * the element of this unrolled buffer at index `idx` , where `0` indicates the
    first element.

* Definition Classes
  * UnrolledBuffer → BufferLike → SeqLike → GenSeqLike → Function1
* Exceptions thrown
...
### `def classTagCompanion: UnrolledBuffer.type`                             ###

* Definition Classes
  * UnrolledBuffer → GenericClassTagTraversableTemplate

(defined at scala.collection.mutable.UnrolledBuffer)


### `def clone(): UnrolledBuffer[T]`                                         ###
...
* Definition Classes
  * UnrolledBuffer → BufferLike → Cloneable → AnyRef

(defined at scala.collection.mutable.UnrolledBuffer)


### `def concat(that: UnrolledBuffer[T]): UnrolledBuffer[T]`                 ###

Concatenates the target unrolled buffer to this unrolled buffer.

The specified buffer `that` is cleared after this operation. This is an O(1)
operation.
...
Applies a function `f` to all elements of this unrolled buffer.
...
* Definition Classes
  * UnrolledBuffer → IterableLike → GenericTraversableTemplate → TraversableLike
    → GenTraversableLike → TraversableOnce → GenTraversableOnce → FilterMonadic

(defined at scala.collection.mutable.UnrolledBuffer)


### `def insertAll(idx: Int, elems: collection.Traversable[T]): Unit`        ###
...
### `def newBuilder: UnrolledBuffer[T]`                                      ###

The builder that builds instances of type `UnrolledBuffer` [A]

* Attributes
  * protected[this]
* Definition Classes
  * UnrolledBuffer → GenericTraversableTemplate → TraversableLike →
    HasNewBuilder

(defined at scala.collection.mutable.UnrolledBuffer)


### `def newUnrolled: Unrolled[T]`                                           ###
...
* Definition Classes
  * UnrolledBuffer → BufferLike
* Exceptions thrown
  * IndexOutOfBoundsException if the if the index `n` is not in the valid range
     `0 <= n < length` .

(defined at scala.collection.mutable.UnrolledBuffer)


### `def result(): UnrolledBuffer[T]`                                        ###
...
* Definition Classes
  * UnrolledBuffer → BufferLike → SeqLike
* Exceptions thrown
...
* Implicit information
  * This member is added by an implicit conversion from UnrolledBuffer [T] to
    CollectionsHaveToParArray [UnrolledBuffer [T], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (UnrolledBuffer [T]) ⇒
