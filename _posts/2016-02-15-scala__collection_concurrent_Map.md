...
Copies the elements of this concurrent map to an array. Fills the given array
 `xs` with at most `len` elements of this concurrent map, starting at position
 `start` . Copying will stop once either the end of the current concurrent map
...
### `def drop(n: Int): mutable.Map[A, B]`                                    ###
...
### `def dropRight(n: Int): mutable.Map[A, B]`                               ###
...
### `def grouped(size: Int): Iterator[mutable.Map[A, B]]`                    ###
...
### `def slice(from: Int, until: Int): mutable.Map[A, B]`                    ###
...
### `def sliding(size: Int): Iterator[mutable.Map[A, B]]`                    ###
...
### `def sliding(size: Int, step: Int): Iterator[mutable.Map[A, B]]`         ###
...
### `def take(n: Int): mutable.Map[A, B]`                                    ###
...
### `def takeRight(n: Int): mutable.Map[A, B]`                               ###
...
### `def takeWhile(p: ((A, B)) ⇒ Boolean): mutable.Map[A, B]`                ###
...
### `def toCollection(repr: mutable.Map[A, B]): Iterable[(A, B)]`            ###
...
### `def view(from: Int, until: Int): IterableView[(A, B), mutable.Map[A, B]]` ###
...
### `def view: IterableView[(A, B), mutable.Map[A, B]]`                      ###
...
Returns a concurrent map formed from this concurrent map and another iterable
...
* thisElem
  * the element to be used to fill up the result if this concurrent map is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    concurrent map.
* returns
  * a new concurrent map containing pairs consisting of corresponding elements
    of this concurrent map and `that` . The length of the returned collection is
    the maximum of the lengths of this concurrent map and `that` . If this
    concurrent map is shorter than `that` , `thisElem` values are used to pad
    the result. If `that` is shorter than this concurrent map, `thatElem` values
...
* returns
  * a new concurrent map containing pairs consisting of corresponding elements
    of this concurrent map and `that` . The length of the returned collection is
    the minimum of the lengths of this concurrent map and `that` .
...
### `def filterNot(p: ((A, B)) ⇒ Boolean): mutable.Map[A, B]`                ###
...
### `def ++:[B >: (A, B), That](that: Traversable[B])(implicit bf: CanBuildFrom[mutable.Map[A, B], B, That]): That` ###
...
Returns a new concurrent map containing the elements from the left hand operand
followed by the elements from the right hand operand. The element type of the
concurrent map is the most specific superclass encompassing the element types of
the two operands.
...
concurrent map on which the function is defined.
...
### `def dropWhile(p: ((A, B)) ⇒ Boolean): mutable.Map[A, B]`                ###
...
### `def filter(p: ((A, B)) ⇒ Boolean): mutable.Map[A, B]`                   ###
...
concurrent map and using the elements of the resulting collections.
...
The type of the resulting collection is guided by the static type of concurrent
...
* returns
  * a new concurrent map resulting from applying the given collection-valued
    function `f` to each element of this concurrent map and concatenating the
    results.
...
### `def groupBy[K](f: ((A, B)) ⇒ K): immutable.Map[K, mutable.Map[A, B]]`   ###
...
### `def inits: Iterator[mutable.Map[A, B]]`                                 ###
...
* f
  * the function to apply to each element.
* returns
  * a new concurrent map resulting from applying the given function `f` to each
    element of this concurrent map and collecting the results.
...
### `def partition(p: ((A, B)) ⇒ Boolean): (mutable.Map[A, B], mutable.Map[A, B])` ###
...
### `def scanLeft[B, That](z: B)(op: (B, (A, B)) ⇒ B)(implicit bf: CanBuildFrom[mutable.Map[A, B], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: ((A, B), B) ⇒ B)(implicit bf: CanBuildFrom[mutable.Map[A, B], B, That]): That` ###
...
### `def scan[B >: (A, B), That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[mutable.Map[A, B], B, That]): That` ###
...
### `def span(p: ((A, B)) ⇒ Boolean): (mutable.Map[A, B], mutable.Map[A, B])` ###
...
### `def splitAt(n: Int): (mutable.Map[A, B], mutable.Map[A, B])`            ###
...
### `def tails: Iterator[mutable.Map[A, B]]`                                 ###
...
### `def withFilter(p: ((A, B)) ⇒ Boolean): FilterMonadic[(A, B), mutable.Map[A, B]]` ###
...
Copies the elements of this concurrent map to an array. Fills the given array
 `xs` with values of this concurrent map. Copying will stop once either the end
of the current concurrent map is reached, or the end of the target array is
reached.
...
Copies the elements of this concurrent map to an array. Fills the given array
 `xs` with values of this concurrent map, beginning at index `start` . Copying
will stop once either the end of the current concurrent map is reached, or the
...
* returns
  * the first element of this concurrent map with the largest value measured by
    function f.
...
* f
  * The measuring function.
* returns
  * the first element of this concurrent map with the smallest value measured by
    function f.
...
Converts this concurrent map to a map. This method is unavailable unless the
...
--------------------------------------------------------------------------------
          Abstract Value Members From scala.collection.concurrent.Map
--------------------------------------------------------------------------------


### `abstract def putIfAbsent(k: A, v: B): Option[B]`                        ###
...
* returns
  * `Some(oldvalue)` if there was a value `oldvalue` previously associated with
    the specified key, or `None` if there was no mapping for the specified key

(defined at scala.collection.concurrent.Map)


### `abstract def remove(k: A, v: B): Boolean`                               ###
...
* v
  * value expected to be associated with the specified key if the removal is to
    take place
* returns
...
### `abstract def replace(k: A, v: B): Option[B]`                            ###

Replaces the entry for the given key only if it was previously mapped to some
value.
...
* v
  * value to be associated with the specified key
* returns
  * `Some(v)` if the given key was previously mapped to some value `v` , or
     `None` otherwise

(defined at scala.collection.concurrent.Map)


### `abstract def replace(k: A, oldvalue: B, newvalue: B): Boolean`          ###

Replaces the entry for the given key only if it was previously mapped to a given
value.
...
* k
  * key for which the entry should be replaced
* oldvalue
  * value expected to be associated with the specified key if replacing is to
    happen
* newvalue
  * value to be associated with the specified key
* returns
...
--------------------------------------------------------------------------------
          Concrete Value Members From scala.collection.concurrent.Map
--------------------------------------------------------------------------------
...
Converts this concurrent map of traversable collections into a concurrent map
formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of concurrent
map. For example:
...
* returns
  * a new concurrent map resulting from concatenating all element concurrent
    maps.
...
### `def transpose[B](implicit asTraversable: ((A, B)) ⇒ GenTraversableOnce[B]): mutable.Iterable[mutable.Iterable[B]]` ###
...
### `def unzip3[A1, A2, A3](implicit asTriple: ((A, B)) ⇒ (A1, A2, A3)): (mutable.Iterable[A1], mutable.Iterable[A2], mutable.Iterable[A3])` ###
...
### `def unzip[A1, A2](implicit asPair: ((A, B)) ⇒ (A1, A2)): (mutable.Iterable[A1], mutable.Iterable[A2])` ###
...
### `def mapResult[NewTo](f: (mutable.Map[A, B]) ⇒ NewTo): Builder[(A, B), NewTo]` ###
...
### `def withDefault(d: (A) ⇒ B): mutable.Map[A, B]`                         ###
...
### `def withDefaultValue(d: B): mutable.Map[A, B]`                          ###
...
### `abstract def +=(kv: (A, B)): Map.this.type`                             ###
...
### `def ++[B1 >: B](xs: GenTraversableOnce[(A, B1)]): mutable.Map[A, B1]`   ###
...
### `def +[B1 >: B](kv: (A, B1)): mutable.Map[A, B1]`                        ###
...
### `def +[B1 >: B](elem1: (A, B1), elem2: (A, B1), elems: (A, B1)*): mutable.Map[A, B1]` ###
...
### `def -(elem1: A, elem2: A, elems: A*): mutable.Map[A, B]`                ###
...
### `def --(xs: GenTraversableOnce[A]): mutable.Map[A, B]`                   ###
...
### `def newBuilder: Builder[(A, B), mutable.Map[A, B]]`                     ###
...
### `def updated[B1 >: B](key: A, value: B1): mutable.Map[A, B1]`            ###
...
* Implicit information
  * This member is added by an implicit conversion from Map [A, B] to
    CollectionsHaveToParArray [Map [A, B], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (Map [A, B]) ⇒
