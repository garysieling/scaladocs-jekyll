...
Finds index of first occurrence of some value in this synchronized buffer.

* elem
  * the element value to search for.
* returns
  * the index of the first element of this synchronized buffer that is equal (as
...
Finds index of first occurrence of some value in this synchronized buffer after
...
* from
  * the start index
* returns
  * the index `>= from` of the first element of this synchronized buffer that is
...
Finds index of last occurrence of some value in this synchronized buffer.

* elem
  * the element value to search for.
* returns
  * the index of the last element of this synchronized buffer that is equal (as
    determined by `==` ) to `elem` , or `-1` , if none exists.
...
Finds index of last occurrence of some value in this synchronized buffer before
...
* end
  * the end index.
* returns
  * the index `<= end` of the last element of this synchronized buffer that is
    equal (as determined by `==` ) to `elem` , or `-1` , if none exists.
...
Copies the elements of this synchronized buffer to an array. Fills the given
array `xs` with at most `len` elements of this synchronized buffer, starting at
position `start` . Copying will stop once either the end of the current
synchronized buffer is reached, or the end of the target array is reached, or
...
Applies a function `f` to all elements of this synchronized buffer.
...
order as this synchronized buffer.
...
### `def zipAll[B, A1 >: A, That](that: GenIterable[B], thisElem: A1, thatElem: B)(implicit bf: CanBuildFrom[Buffer[A], (A1, B), That]): That` ###

[use case]

Returns a synchronized buffer formed from this synchronized buffer and another
...
* thisElem
  * the element to be used to fill up the result if this synchronized buffer is
    shorter than `that` .
* thatElem
  * the element to be used to fill up the result if `that` is shorter than this
    synchronized buffer.
* returns
  * a new synchronized buffer containing pairs consisting of corresponding
    elements of this synchronized buffer and `that` . The length of the returned
    collection is the maximum of the lengths of this synchronized buffer and
     `that` . If this synchronized buffer is shorter than `that` , `thisElem`
    values are used to pad the result. If `that` is shorter than this
    synchronized buffer, `thatElem` values are used to pad the result.
...
### `def zipWithIndex[A1 >: A, That](implicit bf: CanBuildFrom[Buffer[A], (A1, Int), That]): That` ###

[use case]

Zips this synchronized buffer with its indices.

* returns
  * A new synchronized buffer containing pairs consisting of all elements of
    this synchronized buffer paired with their index. Indices start at `0` .
...
### `def zip[A1 >: A, B, That](that: GenIterable[B])(implicit bf: CanBuildFrom[Buffer[A], (A1, B), That]): That` ###
...
* returns
  * a new synchronized buffer containing pairs consisting of corresponding
    elements of this synchronized buffer and `that` . The length of the returned
    collection is the minimum of the lengths of this synchronized buffer and
     `that` .
...
### `def +:[B >: A, That](elem: B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

A copy of the synchronized buffer with an element prepended.

Note that :-ending operators are right associative (see example). A mnemonic for
 `+:` vs. `:+` is: the COLon goes on the COLlection side.

Also, the original synchronized buffer is not modified, so you will want to
capture the result.
...
* returns
  * a new synchronized buffer consisting of `elem` followed by all elements of
...
A copy of this synchronized buffer with an element appended.
...
* returns
  * a new synchronized buffer consisting of all elements of this synchronized
    buffer followed by `elem` .
...
### `def diff[B >: A](that: GenSeq[B]): Buffer[A]`                           ###

[use case]

Computes the multiset difference between this synchronized buffer and another
sequence.

* that
  * the sequence of elements to remove
* returns
  * a new synchronized buffer which contains all elements of this synchronized
    buffer except some of occurrences of elements that also appear in `that` .
...
### `def intersect[B >: A](that: GenSeq[B]): Buffer[A]`                      ###

[use case]

Computes the multiset intersection between this synchronized buffer and another
sequence.
...
* returns
  * a new synchronized buffer which contains all elements of this synchronized
    buffer which also appear in `that` . If an element value `x` appears _n_
...
### `def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

A copy of this synchronized buffer with an element value appended until a given
...
* returns
  * a new synchronized buffer consisting of all elements of this synchronized
    buffer followed by the minimal number of occurrences of `elem` so that the
    resulting synchronized buffer has a length of at least `len` .
...
### `def patch[B >: A, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Produces a new synchronized buffer where a slice of elements in this
synchronized buffer is replaced by another sequence.
...
* returns
  * a new synchronized buffer consisting of all elements of this synchronized
    buffer except that `replaced` elements starting from `from` are replaced by
     `patch` .
...
### `def reverseMap[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this
synchronized buffer and collecting the results in reversed order.
...
* returns
  * a new synchronized buffer resulting from applying the given function `f` to
    each element of this synchronized buffer and collecting the results in
    reversed order.
...
### `def sortBy[B](f: (A) ⇒ B)(implicit ord: math.Ordering[B]): Buffer[A]`   ###
...
### `def sorted[B >: A](implicit ord: math.Ordering[B]): Buffer[A]`          ###
...
### `def toCollection(repr: Buffer[A]): collection.Seq[A]`                   ###
...
### `def union[B >: A, That](that: GenSeq[B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Produces a new sequence which contains all elements of this synchronized buffer
...
* returns
  * a new synchronized buffer which contains all elements of this synchronized
    buffer followed by all elements of `that` .
...
### `def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

A copy of this synchronized buffer with one single replaced element.
...
* returns
  * a copy of this synchronized buffer with the element at position `index`
    replaced by `elem` .
...
### `def view(from: Int, until: Int): SeqView[A, Buffer[A]]`                 ###
...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
### `def ++:[B >: A, That](that: TraversableOnce[B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
### `def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Returns a new synchronized buffer containing the elements from the left hand
operand followed by the elements from the right hand operand. The element type
of the synchronized buffer is the most specific superclass encompassing the
...
### `def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Builds a new collection by applying a partial function to all elements of this
synchronized buffer on which the function is defined.
...
* returns
  * a new synchronized buffer resulting from applying the given partial function
...
### `def filter(p: (A) ⇒ Boolean): Buffer[A]`                                ###
...
### `def filterNot(p: (A) ⇒ Boolean): Buffer[A]`                             ###
...
### `def flatMap[B, That](f: (A) ⇒ GenTraversableOnce[B])(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###

[use case]

Builds a new collection by applying a function to all elements of this
synchronized buffer and using the elements of the resulting collections.
...
synchronized buffer. This might cause unexpected results sometimes. For example:
...
* f
  * the function to apply to each element.
* returns
  * a new synchronized buffer resulting from applying the given
    collection-valued function `f` to each element of this synchronized buffer
...
### `def map[B, That](f: (A) ⇒ B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
* returns
  * a new synchronized buffer resulting from applying the given function `f` to
    each element of this synchronized buffer and collecting the results.
...
### `def scanLeft[B, That](z: B)(op: (B, A) ⇒ B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
### `def scanRight[B, That](z: B)(op: (A, B) ⇒ B)(implicit bf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
### `def scan[B >: A, That](z: B)(op: (B, B) ⇒ B)(implicit cbf: CanBuildFrom[Buffer[A], B, That]): That` ###
...
### `def withFilter(p: (A) ⇒ Boolean): FilterMonadic[A, Buffer[A]]`          ###
...
Copies the elements of this synchronized buffer to an array. Fills the given
array `xs` with values of this synchronized buffer. Copying will stop once
either the end of the current synchronized buffer is reached, or the end of the
...
Copies the elements of this synchronized buffer to an array. Fills the given
array `xs` with values of this synchronized buffer, beginning at index `start` .
Copying will stop once either the end of the current synchronized buffer is
...
* returns
  * the first element of this synchronized buffer with the largest value
...
* f
  * The measuring function.
* returns
  * the first element of this synchronized buffer with the smallest value
    measured by function f.
...
Converts this synchronized buffer to a map. This method is unavailable unless
...
* returns
  * a map of type `immutable.Map[T, U]` containing all key/value pairs of type
     `(T, U)` of this synchronized buffer.
...
### `def flatten[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): Buffer[B]` ###

[use case]

Converts this synchronized buffer of traversable collections into a synchronized
buffer formed by the elements of these traversable collections.

The resulting collection's type will be guided by the static type of
synchronized buffer. For example:
...
* returns
  * a new synchronized buffer resulting from concatenating all element
...
### `def newBuilder: Builder[A, Buffer[A]]`                                  ###
...
### `def transpose[B](implicit asTraversable: (A) ⇒ GenTraversableOnce[B]): Buffer[Buffer[B]]` ###
...
### `def +=(elem1: A, elem2: A, elems: A*): SynchronizedBuffer.this.type`    ###
...
### `def --=(xs: TraversableOnce[A]): SynchronizedBuffer.this.type`          ###
...
### `def --(xs: GenTraversableOnce[A]): Buffer[A]`                           ###
...
### `def -=(x: A): SynchronizedBuffer.this.type`                             ###
...
### `def transform(f: (A) ⇒ A): SynchronizedBuffer.this.type`                ###
...
--------------------------------------------------------------------------------
   Deprecated Value Members From scala.collection.mutable.SynchronizedBuffer
--------------------------------------------------------------------------------
...
* Definition Classes
  * SynchronizedBuffer → BufferLike → Scriptable
* Annotations
  * @ deprecated
* Deprecated
  * _(Since version 2.11.0)_ Scripting is deprecated.

(defined at scala.collection.mutable.SynchronizedBuffer)


--------------------------------------------------------------------------------
         Value Members From scala.collection.mutable.SynchronizedBuffer
--------------------------------------------------------------------------------


### `def ++(xs: GenTraversableOnce[A]): Self`                                ###

Appends a number of elements provided by a traversable object via its `foreach`
...
Prepends a number of elements provided by a traversable object via its
...
### `def +=(elem: A): SynchronizedBuffer.this.type`                          ###
...
* Definition Classes
  * SynchronizedBuffer → BufferLike → Growable
...
* returns
  * the element of this synchronized buffer at index `idx` , where `0` indicates
    the first element.

* Definition Classes
  * SynchronizedBuffer → BufferLike → SeqLike → GenSeqLike → Function1
* Exceptions thrown
...
* Definition Classes
  * SynchronizedBuffer → BufferLike → Cloneable → AnyRef
...
will not replace an element with a one. Instead, it will insert the new elements
at index `n` .
...
### `def insertAll(n: Int, xs: collection.Traversable[A]): Unit`             ###

Inserts new elements at the index `n` . Opposed to method `update` , this method
will not replace an element with a one. Instead, it will insert a new element at
index `n` .
...
* Definition Classes
  * SynchronizedBuffer → BufferLike → SeqLike
...
* Implicit information
  * This member is added by an implicit conversion from SynchronizedBuffer [A]
    to CollectionsHaveToParArray [SynchronizedBuffer [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (SynchronizedBuffer [A]) ⇒
