...
* elems
  * the up to 32 children of this node. the number of children must be identical
    to the number of 1 bits in bitmap
* size0
...
* Definition Classes
  * HashTrieSet → HashSet → IterableLike → GenericTraversableTemplate →
...
* Implicit information
  * This member is added by an implicit conversion from HashTrieSet [A] to
    CollectionsHaveToParArray [HashTrieSet [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (HashTrieSet [A]) ⇒
