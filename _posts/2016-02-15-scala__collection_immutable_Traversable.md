...
### `def ++:[B >: A, That](that: collection.Traversable[B])(implicit bf: CanBuildFrom[Traversable[A], B, That]): That` ###
...
### `def thisCollection: collection.Traversable[A]`                          ###
...
### `def toCollection(repr: Traversable[A]): collection.Traversable[A]`      ###
...
* Implicit information
  * This member is added by an implicit conversion from Traversable [A] to
    CollectionsHaveToParArray [Traversable [A], T] performed by method
    CollectionsHaveToParArray in scala.collection.parallel. This conversion will
    take place only if an implicit value of type (Traversable [A]) ⇒
